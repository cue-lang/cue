...
## 2. Map each issue to its commits on master
...
## 3. Order and prune the commit list
...
## 4. Determine the target release branch
...
commits picked per issue in order, issues skipped (no commits found)
or commits dropped (already backported), any conflict resolutions or
judgment calls, and the test results.
