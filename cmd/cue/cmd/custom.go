...
func doTasks(cmd *Command, command string, root *cue.Instance) error {
	...
	//
	// It should also be an error in case the command we have invoked declares
	// no tasks. It might be the command declares literally no tasks, or that
	// the CUE value it references is sufficiently incomplete:
	//
	//    input: string
