...
func runExpWritefs(cmd *Command, args []string) error {
	...
	for fp, f := range todo.Create {
		...
		case "file":
			...
			// TODO: more principled checking of whether this filetype is known to CUE.
			// Note that cuelang.org/play has a similar situation going on.
			var err error
			switch ext := filepath.Ext(fp); ext {
			case ".json":
				// Note that we marshal again, to ensure good formatting.
				var p []byte
			...
			case ".yaml":
				var p []byte
				p, err = cueyaml.Encode(v)
				buf.Write(p)
			case ".toml":
				err = cuetoml.NewEncoder(&buf).Encode(v)
