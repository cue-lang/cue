...
Arguments are interpreted as import paths (see 'cue help inputs') unless --recurse is set,
...
`,
		RunE: mkRunE(c, func(cmd *Command, args []string) error {
			...
			if !flagRecurse.Bool(cmd) { // format packages
				...
				if hasDots {
					exitOnErr(cmd, errors.New(`cannot use "..." in --recurse mode`), true)
				}
	...
	cmd.Flags().Bool(string(flagRecurse), false, "treat arguments as file paths to descend into rather than import paths")
