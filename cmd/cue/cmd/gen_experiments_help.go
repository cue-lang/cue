...
func extractExperiments() ([]Experiment, error) {
	...
	for i := 0; i < fileType.NumField(); i++ {
		...
		if fieldInfo != nil {
			...
			proposal, specChanges, needsFix = parseExperimentComment(comment)
		}
...
func parseExperimentComment(comment string) (proposal string, specChanges []string, needsFix bool) {
	lines := strings.Split(comment, " ")
	for i, line := range lines {
		if strings.Contains(line, "https://cuelang.org/issue/") {
			proposal = line
		}
		if strings.Contains(line, "https://cuelang.org/cl/") {
			specChanges = append(specChanges, line)
		}
		if strings.Contains(line, "Needs cue fix:") && i+1 < len(lines) {
			needsFix = strings.ToLower(strings.TrimSpace(lines[i+1])) == "yes"
		}
	}
	return
}
...
`)
	...
	for _, exp := range experiments {
		...
		if exp.Comment != "" {
			description := extractDescription(exp.Comment)
			if description != "" {
		...
		for _, specChange := range exp.SpecChange {
			sb.WriteString(fmt.Sprintf("    Spec change: %s\n", specChange))
		}

		if exp.NeedsFix {
			sb.WriteString("    Requires cue fix when upgrading\n")
		}
...
func extractDescription(comment string) string {
	// Extract the first sentence as description, up to the first period or newline
	parts := strings.SplitN(comment, ".", 2)
	if len(parts) > 0 {
		desc := strings.TrimSpace(parts[0])
		// Remove field name prefix if it exists
		if strings.Contains(desc, " enables ") {
			parts := strings.SplitN(desc, " enables ", 2)
			if len(parts) == 2 {
				desc = "Enables " + parts[1]
			}
		} else if strings.Contains(desc, " allows ") {
			parts := strings.SplitN(desc, " allows ", 2)
			if len(parts) == 2 {
				desc = "Allows " + parts[1]
			}
		}
		// Clean up some common patterns
		desc = strings.ReplaceAll(desc, "enables", "Enables")
		desc = strings.ReplaceAll(desc, "allows", "Allows")
