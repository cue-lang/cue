...
Otherwise, the import path P is absolute and denotes a package that may be
external, usually found in an external registry. It may also refer to a package in
cue.mod/{pkg|gen|usr}/P - this is legacy behavior and may not
be supported indefinitely.

An absolute import path is of the form P or P@vN where vN is the
major version of the module containing the package. An import
path is mapped to a registry location by
consulting the cue.mod/module.cue and the registry configuration
(see "cue help modules" and "cue help registryconfig" for more details).
...
directories are excluded from the result. "..." is not currently
supported in external import paths.
...
explicitly mentioned on the command line. Files with
names ending "_test.cue" are always ignored.
