...
// makeMappers builds a protocol.Mapper for each file in the files
...
// the editor), in a directory which did not previously exist. Even
// though the new file's directory contains no active files, the
// creation must be noticed because it matches the glob embed
// attribute of a loaded package: a package is created for the new
...
`
	I.WithOptions(I.RootURIAsDefaultFolder()).Run(t, files, func(t *testing.T, env *I.Env) {
		...
		testCasesBefore.run(t, env, mappers)
