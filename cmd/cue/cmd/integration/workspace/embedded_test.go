...
`
	WithOptions(RootURIAsDefaultFolder()).Run(t, files, func(t *testing.T, env *Env) {
		...
		env.Await(
			...
			LogMatching(protocol.Debug, fmt.Sprintf(`Package dirs=\[%v/data\] importPath=mod\.example/x/data@v0:_.+ Created`, rootURI), 1, false),
			LogMatching(protocol.Debug, fmt.Sprintf(`Package dirs=\[%v/data\] importPath=mod\.example/x/data@v0:_.+ Reloaded`, rootURI), 1, false),
		)
	})
}
...
`
	WithOptions(RootURIAsDefaultFolder()).Run(t, files, func(t *testing.T, env *Env) {
		...
		env.Await(
			...
			LogExactf(protocol.Debug, 1, false, "Module dir=%v module=mod.example/x@v0 Loading packages [mod.example/x@v0:a]", rootURI),
			LogExactf(protocol.Debug, 1, false, "Package dirs=[%v] importPath=mod.example/x@v0:a Reloaded", rootURI),
			NoLogMatching(protocol.Debug, fmt.Sprintf(`Package dirs=\[%v/data\] importPath=mod\.example/x/data@v0:_.+ Created`, rootURI)),
		)
	})
}
...
`
	WithOptions(RootURIAsDefaultFolder()).Run(t, files, func(t *testing.T, env *Env) {
		...
		env.Await(
			LogMatching(protocol.Debug, fmt.Sprintf(`Package dirs=\[%v/data\] importPath=mod\.example/x/data@v0:_.+ Deleted`, rootURI), 1, false),
			LogExactf(protocol.Debug, 2, false, "Package dirs=[%v] importPath=mod.example/x@v0:a Reloaded", rootURI),
		)
	})
}
...
`
	WithOptions(RootURIAsDefaultFolder()).Run(t, files, func(t *testing.T, env *Env) {
		...
		env.Await(
			env.DoneWithOpen(),
			LogMatching(protocol.Debug, fmt.Sprintf(`Package dirs=\[%v/data\] importPath=mod\.example/x/data@v0:_.+ Reloaded`, rootURI), 2, false),
...
`
	WithOptions(RootURIAsDefaultFolder()).Run(t, files, func(t *testing.T, env *Env) {
		...
		env.Await(
			...
			LogMatching(protocol.Debug, fmt.Sprintf(`Package dirs=\[%v/data\] importPath=mod\.example/x/data@v0:_.+ Created`, rootURI), 2, false),
			LogMatching(protocol.Debug, fmt.Sprintf(`Package dirs=\[%v/data\] importPath=mod\.example/x/data@v0:_.+ Reloaded`, rootURI), 2, false),
		)
...
([b.cue line 5](%s/b.cue#L5))`[1:],
		...
		for p, expectation := range testCases {
			...
			for i := range strLen {
				...
				fmt.Printf("hover from %v %v, got %v (rootURI: %v)\n", p.mapper.URI, pos, got, rootURI)
