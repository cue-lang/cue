...
// NB the nested files in the archive below appear to do nothing. They
// have been added to test that when FindReferences triggers loading
// of all packages within the current module, we do not accidentally
// walk down into nested modules, which could previously provoke
// infinite loops when packages are loaded.
...
func TestReferences(t *testing.T) {
	WithOptions(RootURIAsDefaultFolder()).Run(t, referencesFiles, func(t *testing.T, env *Env) {
		...
		env.Await(
			// Make sure we did not create a package x: that pkg only
			// belongs to the nested module.
			NoLogExactf(protocol.Debug, "Package dirs=[%v/nested] importPath=example.com/bar/nested@v0:x", rootURI),
		)
	})
}
