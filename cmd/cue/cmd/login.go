...
To log into a standard OCI registry, use 'docker login' or 'podman login' instead:

	https://cuelang.org/docs/reference/modules/#authorization
`[1:], code, http.StatusText(code))
					}
			...
			case tokenRegex.MatchString(tokenStr):
			...
			default:
				return fmt.Errorf("unknown token format, expected a prefix like appv1_")
			}
...
var tokenRegex = regexp.MustCompile(`^\w{2,10}v\d{1,3}_.+$`)
