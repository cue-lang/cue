...
It only reads that file; it does not look up information about the modules
involved.
...
Users should prefer 'cue get path@version' or 'go get path@none',
which make other go.mod adjustments as needed to satisfy
constraints imposed by other modules.


The --fmt flag reformats the module.cue file without making other changes.
...
The --dropsource flag removes the source field.
...
`,
	...
	addFlagVar(cmd, boolFlagFunc(editCmd.flagDropSource), "dropsource", "remove the source field")
	...
	addFlagVar(cmd, flagFunc(editCmd.flagDropRequire), "droprequire", "remove a requirement")
...
func (c *modEditCmd) flagDropSource(arg bool) error {
	if !arg {
		return fmt.Errorf("cannot set --dropSource to false")
	}
	c.addEdit(func(f *modfile.File) error {
		f.Source = nil
		return nil
