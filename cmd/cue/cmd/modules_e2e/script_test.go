	...
	p := testscript.Params{
		...
		Cmds: map[string]func(ts *testscript.TestScript, neg bool, args []string){
			...
			"create-github-repo": func(ts *testscript.TestScript, neg bool, args []string) {
				...
				repoName := fmt.Sprintf("%d", time.Now().Unix())
			...
			// env-fill expands variables in a file like cmpenv, replacing the file contents with the result.
			...
			"cue-mod-wait": func(ts *testscript.TestScript, neg bool, args []string) {
				...
				for i := 0; i < 10; i++ {
					...
					ts.Logf("waiting for HEAD %s", manifest)
