...
// placeValue wraps v in the nested struct described by the -l/--path flags;
// it is the value-side counterpart to placeOrphans. It is a no-op without -l,
// or when orphan files are present: there the flags are already consumed by
// placeOrphans, so wrapping again would double-wrap.
func (b *buildPlan) placeValue(v cue.Value) (cue.Value, error) {
