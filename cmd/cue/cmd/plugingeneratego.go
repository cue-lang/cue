...
func runPluginGenerateGo(cmd *Command, args []string) error {
	...
	// Generate _imports_gen.go files first so that "go mod tidy"
	// can see all referenced packages.
	for _, gf := range genFiles {
		if err := generateImportsForFile(gf); err != nil {
			return err
		}
	}

	// Ensure all referenced packages are available in go.mod.
	if goModDir != "" {
	...
	for _, gf := range genFiles {
		if err := generateSignaturesForFile(inst, gf, localGoPkg, pkgMap); err != nil {
			return err
		}
	}
	return nil
...
func generateImportsForFile(gf genFile) error {
	externalPkgs := map[string]bool{}
	for _, ref := range gf.refs {
		if ref.goPackage != "." {
			externalPkgs[ref.goPackage] = true
		}
	}
	...
	return os.WriteFile(goPath, goData, 0o666)
}

func generateSignaturesForFile(inst *build.Instance, gf genFile, localGoPkg string, pkgMap map[string]*packages.Package) error {
	...
	return os.WriteFile(cuePath, cueData, 0o666)
}
