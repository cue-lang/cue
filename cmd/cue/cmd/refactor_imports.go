...
func runRefactorImports(cmd *Command, args []string) error {
	...
	newImportPath := module.ParseImportPath(args[len(args)-1])
	oldImportPath := newImportPath
	if len(args) > 1 {
		oldImportPath = module.ParseImportPath(args[0])
	}
	...
	matchPkg := func(ip module.ImportPath) (_m bool) {
		log.Printf("match %q {", ip)
		defer func() {
			log.Printf("} -> %v", _m)
		}()
		...
		if !pkgIsUnderneath(ip.Path, oldImportPath.Path) {
			log.Printf("%q not underneath %q", oldImportPath.Path, ip.Path)
			return false
		}
	...
	log.Printf("running refactor, old %v; new %v", oldImportPath, newImportPath)
	...
	for _, inst := range binst {
		...
		for _, file := range inst.BuildFiles {
			...
			if !refactorImports(syntax, func(importPath, ident string) (_newIP, _newIdent string) {
				log.Printf("processImport %q %q (newIdent %q) {", importPath, ident, newIdent)
				defer func() {
					log.Printf("} -> %q %q", _newIP, _newIdent)
				}()
				oldIP := module.ParseImportPath(importPath)
				if !matchPkg(oldIP) {
					// No match: no change.
				...
				newIP = module.ParseImportPath(newIP.String())
				return newIP.String(), newIP.Qualifier
			}) {
				// Nothing changed: no need to write the file.
				continue
			}
			log.Printf("before Sanitize: %q", astinternal.AppendDebug(nil, syntax, astinternal.DebugConfig{
				OmitEmpty:       true,
			...
			log.Printf("after Sanitize: %q", data)
...
func refactorImports(f *ast.File, processImport func(importPath, ident string) (newImportPath, newIdent string)) bool {
	...
	astutil.Apply(f, func(c astutil.Cursor) bool {
		...
		case *ast.ImportSpec:
			...
			if ident == "" {
				ident = module.ParseImportPath(importPath).Qualifier
			}
			newIP := module.ParseImportPath(newImportPath)
			if newIdent == "" {
