	...
	p := testscript.Params{
		...
		Setup: func(e *testscript.Env) error {
			...
			for _, entry := range entries {
				...
				suffix, ok := strings.CutPrefix(entry.Name(), "_registry")
				if !ok {
					continue
				}
				...
				prefix := ""
				if data, err := os.ReadFile(filepath.Join(e.WorkDir, "_registry"+suffix+"_prefix")); err == nil {
