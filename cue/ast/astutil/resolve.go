...
type scope struct {
	...
	// scopePool is used to reuse scope allocations. Only set on the root scope.
	scopePool []*scope
}
...
func (s *scope) allocScope() *scope {
	root := s.rootScope()
	if n := len(root.scopePool); n > 0 {
		scope := root.scopePool[n-1]
		root.scopePool = root.scopePool[:n-1]
		return scope
	}
...
func putScope(root, s *scope) {
	...
	*s = scope{index: s.index}
	clear(s.index)
	root.scopePool = append(root.scopePool, s)
}
...
// freeScopesUntil frees all scopes from s up to (but not including) 'until'.
func (s *scope) freeScopesUntil(until *scope) {
	root := s.rootScope()
	for s != until {
		next := s.outer
		putScope(root, s)
