...
//
// Deprecated: this API means that there's an ambiguity between
// a key-value attribute containing an empty value and a
// a simple string value, and no way to tell if an attribute's
// value was quoted. Use [Attribute.Args] to obtain
// an [AttributeArg] instead, which avoids those issues.
...
func (a AttributeArg) AsString() string {
	return internal.MaybeUnquote(s.Value)
}
