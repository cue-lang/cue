...
// AllowAll returns an allow function that permits all injections.
func AllowAll() func(inst *build.Instance, name string) error {
	return func(*build.Instance, string) error { return nil }
}

// Inject returns an [Option] that registers the given [Injector]
// as an interpreter for @extern(inject) attributes.
func Inject(j *Injector) Option {
	return Option{func(r *runtime.Runtime) {
		r.SetInterpreter(&injectInterpreter{injector: j})
	}}
}

type injectInterpreter struct {
	injector *Injector
}

func (i *injectInterpreter) Kind() string { return "inject" }

func (i *injectInterpreter) NewCompiler(b *build.Instance, r *runtime.Runtime) (runtime.Compiler, errors.Error) {
	return &injectCompiler{
		injector: i.injector,
		inst:     b,
	}, nil
}

type injectCompiler struct {
	injector *Injector
	inst     *build.Instance
}

func (c *injectCompiler) Compile(name string, scope adt.Value, a *internal.Attr) (adt.Expr, errors.Error) {
