...
func (p list) Msg() (format string, args []interface{}) {
	...
	case 1:
		// Note: this is _not_ the same as returning p[0].Msg()
		// because the message does not necessarily include
		// as much information as p[0].Error() which will be invoked
		// by %s.
		return "%s", []any{p[0]}
	}
