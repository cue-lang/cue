...
// FuncOption configures a function created by [NewPureFunc1], [NewPureFunc2], etc.
...
// NewPureValidatorFunc returns a [Value] that acts as a CUE validator.
...
func NewPureValidatorFunc[T any](f func(T) error, opts ...FuncOption) Value {
	...
	return newValueRoot(ctx.runtime(), ctx.ctx(), &adt.FuncValidator{
		...
		Validate: func(opCtx *adt.OpContext, v adt.Value) *adt.Bottom {
			var t T
			// Wrap the value using the runtime that is actually evaluating
			// the unification (opCtx's runtime), not this validator's own
			// runtime, so any per-runtime state consulted during decoding is
			// correct.
			rt := opCtx.Runtime.(*runtime.Runtime)
			cueVal := newValueRoot(rt, opCtx, v)
...
func newPureFunc[Args, R any](f func(Args) (R, error), opts []FuncOption) Value {
	var cfg funcConfig
	for _, o := range opts {
		o(&cfg)
	}
	...
	return newValueRoot(ctx.runtime(), ctx.ctx(), &adt.Func{
		...
		Func: func(opCtx *adt.OpContext, argValues []adt.Value) adt.Expr {
			...
			// Wrap the argument values using the runtime that is actually
			// evaluating the call (opCtx's runtime), not this function's own
			// runtime, so any per-runtime state consulted during decoding is
			// correct.
			rt := opCtx.Runtime.(*runtime.Runtime)
			for i := range numArgs {
				argVal := newValueRoot(rt, opCtx, argValues[i])
