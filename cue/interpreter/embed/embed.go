...
func pathElems(p string) iter.Seq[string] {
	return func(yield func(string) bool) {
		for {
			elem, rest, ok := strings.Cut(p, "/")
			if !yield(elem) {
				return
			}
			if !ok {
...
// shouldMatch checks if the given file should be considered
// to match the given glob pattern, where explicit
// if the file does not exist and will check that elsewhere.
func shouldMatch(file string, pattern string) bool {
