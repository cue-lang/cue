...
// root returns the file system corresponding to the root of the current
// package as defined by the build.Instance.
func (c *compiler) pkgfs() fs.FS {
	return nil
}
...
func (c *compiler) Compile(funcName string, scope adt.Value, a *internal.Attr) (adt.Expr, errors.Error) {
	...
	case file != "":
		return c.processFile(a.Pos, file, typ, scope)

	default: // glob != "":
		return c.processGlob(a.Pos, glob, typ, scope)
	}
}

func (c *compiler) processFile(pos token.Pos, file, scope string, schema adt.Value) (adt.Expr, errors.Error) {
	file, err := c.clean(pos, file)
	if err != nil {
		return nil, err
	}

	return c.decodeFile(pos, file, scope, schema)
}

func (c *compiler) processGlob(pos token.Pos, glob, scope string, schema adt.Value) (adt.Expr, errors.Error) {
	glob, ce := c.clean(pos, glob)
	if ce != nil {
		return nil, ce
	}

	if strings.Contains(glob, "**") {
		return nil, errors.Newf(pos, "double star not supported in glob")
	}

	// Jump through some hoops to get a glob that behaves the same for Windows
	// and Unix.
	dir := os.DirFS(filepath.Dir(pos.File().Name()))
	...
	for _, f := range matches {
		expr, err := c.decodeFile(pos, f, scope, schema)
		if err != nil {
			return nil, err
		}
...
func (c *compiler) clean(pos token.Pos, s string) (string, errors.Error) {
	file := path.Clean(s)
	if file != s {
...
func (c *compiler) decodeFile(pos token.Pos, file, scope string, schema adt.Value) (adt.Expr, errors.Error) {
	...
	if d.Next(); !d.Done() {
		return nil, errors.Newf(pos, "streaming not implemented: found more than one value in file")
	}

	switch f.Encoding {
	case build.JSONL:
		return nil, errors.Newf(pos, "encoding %q not (yet) supported: requires support for streaming", f.Encoding)
	case build.BinaryProto, build.TextProto:
		return nil, errors.Newf(pos, "encoding %q not (yet) supported: requires support for schema-guided decoding", f.Encoding)
	}
