...
func TestImportPaths(t *testing.T) {
	...
	var f func(inst *build.Instance)
	f = func(inst *build.Instance) {
	...
	for _, inst := range insts {
		...
		qt.Assert(t, qt.Not(qt.CmpEquals(inst.PkgName, "_")))
		f(inst)
	}
	...
	qt.Assert(t, qt.CmpEquals(importPaths, []string{
