...
// iofsFileSystem implements [fileSystem] backed by an [io/fs.FS].
...
func newIOFSFileSystem(cfg *Config) (*iofsFileSystem, error) {
	return &iofsFileSystem{
		fsys:      cfg.FS,
		cwd:       cfg.Dir,
...
func (fsys *iofsFileSystem) readDir(p string) ([]fs.DirEntry, errors.Error) {
...
func (fsys *iofsFileSystem) stat(p string) (fs.FileInfo, errors.Error) {
...
func (fsys *iofsFileSystem) openFile(p string) (io.ReadCloser, errors.Error) {
...
func (fsys *iofsFileSystem) getSource(cfg *Config, filename string) (any, error) {
...
func (fsys *iofsFileSystem) getCUESyntax(bf *build.File, cfg parser.Config) (*ast.File, error) {
	return fsys.fileCache.getCUESyntax(bf, cfg)
}

func (fsys *iofsFileSystem) ioFS(root string, languageVersion string) fs.FS {
...
// fileSystem path p.
func (fsys *iofsFileSystem) fsPath(p string) string {
...
func (fsys *iofsFileSystem) makeAbs(p string) string {
	if path.IsAbs(p) {
		return path.Clean(p)
	}
...
// fsIOFS provides an [io/fs.FS] view for the iofsFileSystem implementation.
