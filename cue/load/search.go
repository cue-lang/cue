...
// instancesFromResolvedPackages returns the set of instances corresponding to
// the given resolved packages.
func (l *loader) instancesFromResolvedPackages(pkgs []resolvedPackageArg) []*build.Instance {
	var out []*build.Instance
	for _, pkg := range pkgs {
		p := l.newInstance(token.NoPos, pkg)
		out = append(out, l.importPkg(token.NoPos, p)...)
	}
	return out
}
...
func expandPackageArgs(c *Config, pkgArgs []string, pkgQual string) ([]resolvedPackageArg, error) {
	expanded := make([]resolvedPackageArg, 0, len(pkgArgs))
	for _, p := range pkgArgs {
		var err error
		expanded, err = appendExpandedPackageArg(c, expanded, p, pkgQual)
		if err != nil {
			return nil, err
		}
	}
...
func appendExpandedPackageArg(c *Config, pkgPaths []resolvedPackageArg, p string, pkgQual string) ([]resolvedPackageArg, error) {
	origp := p
	if filepath.IsAbs(p) {
	...
	if isLocal {
		if c.Module != "" {
			...
			// the module root.
			absPath := pathpkg.Join(c.Dir, ip.Path)
...
func appendExpandedUnqualifiedPackagePath(pkgPaths []resolvedPackageArg, origp string, ip module.ImportPath, pkgQual string, mainModRoot module.SourceLoc, mainModPath string) (_ []resolvedPackageArg, _err error) {
	...
	dir := pathpkg.Join(mainModRoot.Dir, ipRel.Path)
	info, err := fs.Stat(mainModRoot.FS, dir)
	if err != nil {
		// The package directory doesn't exist. Treat it like an
		// empty directory and let later logic deal with it.
		return append(pkgPaths, resolvedPackageArg{origp, ip.String()}), nil
	}
	if !info.IsDir() {
		return nil, fmt.Errorf("%s refers to a file not a package directory", origp)
	}
	iter := modimports.PackageFiles(mainModRoot.FS, dir, "*")

	// 1. if pkgQual is "*", it appends all the packages present in the
	// package directory.
	if pkgQual == "*" {
	...
	if foundQualifier {
		// We found the actual package that was implied
		// by the import path (or pkgQual == "_"). This takes precedence over
		// anything else.
		return append(pkgPaths, resolvedPackageArg{origp, ip.String()}), nil
	}
	if len(files) == 0 {
	...
	for _, f := range files[1:] {
		...
		if pkgName1 := f.Syntax.PackageName(); pkgName1 != pkgName {
			return nil, &MultiplePackageError{
				...
				Files: []string{
					pathpkg.Base(files[0].FilePath),
					pathpkg.Base(f.FilePath),
				},
			}
		}
	}
...
// and appends the results to pkgPaths.
// It returns an error if the pattern matching nothing.
// Note:
//
//	We know that pattern contains ...
//	We know that pattern is relative to the module root
func appendExpandedWildcardPackagePath(pkgPaths []resolvedPackageArg, pattern module.ImportPath, pkgQual string, mainModRoot module.SourceLoc, mainModPath string) (_ []resolvedPackageArg, _err error) {
	...
	dir = pathpkg.Join(mainModRoot.Dir, dir)
	...
	iter(func(f modimports.ModuleFile, err error) bool {
		...
		if modIpath.Path == "" {
			// There's no module, so make sure that the path
			// still looks like a relative import path.
		...
		if pkgQual == "" {
			...
			if prevFile.FilePath != "" && prevImportPath.Path == ip.Path && ip.Qualifier != prevImportPath.Qualifier {
				...
				_err = &MultiplePackageError{
					...
					Files: []string{
						pathpkg.Base(prevFile.FilePath),
						pathpkg.Base(f.FilePath),
					},
				}
				return false
...
// cutModulePrefix strips the given module path from p and reports
// whether p is inside mod. It returns a relative package
// path within m.
//
// If p does not contain a major version suffix but otherwise
// matches mod, it counts as a match.
