...
func (t SelectorType) String() string {
	...
	pl := bits.Len8(uint8(labelBit))
	constraintBit := t.ConstraintType()
	pc := bits.Len8(uint8(constraintBit))

	switch {
	case bits.OnesCount8(uint8(constraintBit)) > 1,
		bits.OnesCount8(uint8(labelBit)) > 1:
		fallthrough
	default:
		return fmt.Sprintf("InvalidSelectorType(%#x)", uint(t))
...
// ConstraintType returns the type of the label part of a selector.
