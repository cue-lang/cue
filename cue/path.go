...
//
// Selectors are ordered first by their underlying label
// (string, definition, hidden, index, pattern), then by value
// within the same kind, then by constraint type.
...
func compareSel(a, b selector) int {
	...
	ao, bo := selectorOrd(a), selectorOrd(b)
	if c := cmp.Compare(ao, bo); c != 0 {
		return c
	}
	var vc int
	switch a := a.(type) {
	case stringSelector:
		vc = cmp.Compare(string(a), string(b.(stringSelector)))
	case definitionSelector:
		vc = cmp.Compare(string(a), string(b.(definitionSelector)))
	case scopedSelector:
		bs := b.(scopedSelector)
		vc = cmp.Or(cmp.Compare(a.name, bs.name), cmp.Compare(a.pkg, bs.pkg))
	case indexSelector:
		vc = cmp.Compare(adt.Feature(a).Index(), adt.Feature(b.(indexSelector)).Index())
	case anySelector:
		vc = cmp.Compare(adt.Feature(a), adt.Feature(b.(anySelector)))
	case patternSelector:
		vc = cmp.Compare(a._labelType, b.(patternSelector)._labelType)
	case pathError:
		// All errors compare equal.
	}
	if vc != 0 {
		return vc
	}
