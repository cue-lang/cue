...
func (v Value) Unify(w ...Value) Value {
	// Skip missing values and duplicate vertices: unification is
	// idempotent, and this retains the fast paths of the binary form.
	all := make([]adt.Value, 0, 1+len(w))
	first := v
	add := func(x Value) {
		if x.v == nil || slices.Contains(all, adt.Value(x.v)) {
			return
		}
		if len(all) == 0 {
			first = x
