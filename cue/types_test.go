...
// TestUnifyVariadic tests the top-level Unify function,
// checking that it behaves like a chain of Value.Unify calls.
...
func TestUnifyMissingValues(t *testing.T) {
	...
	qt.Assert(t, qt.Equals(cue.Unify(), zero))
	qt.Assert(t, qt.Equals(cue.Unify(a), a))
	qt.Assert(t, qt.Equals(a.Unify(zero), a))
	qt.Assert(t, qt.Equals(zero.Unify(a), a))
	qt.Assert(t, qt.Equals(zero.Unify(zero), zero))
	qt.Assert(t, qt.Equals(cue.Unify(a, a, a), a))

	v := cue.Unify(zero, a, b)
	data, err := v.MarshalJSON()
	qt.Assert(t, qt.IsNil(err))
	qt.Assert(t, qt.Equals(string(data), `{"x":1,"y":2}`))

	// Duplicates between the incoming values are not skipped,
	// but unification is idempotent.
	v = cue.Unify(a, b, b)
	data, err = v.MarshalJSON()
	qt.Assert(t, qt.IsNil(err))
	qt.Assert(t, qt.Equals(string(data), `{"x":1,"y":2}`))
...
func TestIsClosed(t *testing.T) {
	testCases := []struct {
		...
		{
			desc:                "definition with disjunction of scalar and struct",
			input:               `#A: 1 | {a!: int}`,
			path:                "#A",
			wantClosed:          true,
			wantClosedRecursive: true,
		...
		{
			desc:                "closed list with definition elements",
			input:               `#A: {a: 1}, x: [#A, #A]`,
			path:                "x",
			wantClosed:          false,
			wantClosedRecursive: false,
		...
		{
			desc:                "list element from definition",
			input:               `#A: {a: 1}, x: [#A, #A]`,
			path:                "x[0]",
			wantClosed:          true,
			wantClosedRecursive: true,
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			cuetdtest.FullMatrix.Run(t, tc.input, func(t *testing.T, m *cuetdtest.M) {
				...
				qt.Check(t, qt.Equals(v.IsClosed(), tc.wantClosed))
				qt.Check(t, qt.Equals(v.IsClosedRecursively(), tc.wantClosedRecursive))

				if v.IsClosedRecursively() {
					qt.Check(t, qt.IsTrue(v.IsClosed()))
				}
			})
		})
	}
