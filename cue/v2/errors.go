...
func (e *valueError) Path() (a []string) {
	...
	for _, sel := range e.v.Path().Selectors() {
...
// IsIncomplete reports whether err is an incomplete error. Incomplete
// errors occur when a value cannot be fully evaluated due to missing
// information, such as unresolved references or incomplete
// disjunctions. These errors may be acceptable in non-concrete
// contexts.
...
// canceledError wraps the cancellation cause of a forcing operation so
// that errors.Is(err, context.Canceled) (or DeadlineExceeded) holds.
func canceledError(cause error) error {
	return fmt.Errorf("evaluation canceled: %w", cause)
}
