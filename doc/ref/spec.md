...
a: {
	...
	TODO: also have an example that is *not* a regular expression, to clarify
	that those can also exist.
    ["foo"]: int
    foo?: int
...
Dan: presumably you can do the same with both:
	a[b.c]
	a."\(b.c)" -- presumably valid syntax, even if not as nice
Index exprs on structs seem redundant?

TODO: another from Rog:
	x: a: 123
	y: x."a"		// OK
	y: x."\("a")"	// not OK - why?
...
import   "lib/math:math"    math.Sin TODO(FIXME): what is this colon syntax? the
										EBNF has it but it's not explained anywhere.
