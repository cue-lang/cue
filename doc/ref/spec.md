...
denotes an open struct that is unified with `x` but allows additional fields
beyond those defined in `x`.
The postfix `...` operator recursively opens closed structs,
...
- an open struct unified with `x`, if `x` is a struct
- bottom (an error), if `x` is not a struct
...
The builtin function `__closeAll` recursively converts aa partially defined,
or open, struct to fully defined, or closed, struct. It is only defined on
