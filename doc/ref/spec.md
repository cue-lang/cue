...
A validator is a value that validates the value at the position where it is
defined. A successful validation yields the original value;
...
used as validators, so we will not refer to their
...
- The first argument is a numeric constraint specifying how many schemas must match
- The second argument is a list of schemas to test against the value
...
- The first argument is a condition schema (the "if" clause)
- The second argument is the schema to apply if the condition matches (the "then" clause)
- The third argument is the schema to apply if the condition does not match (the "else" clause)
