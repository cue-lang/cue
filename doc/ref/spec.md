...
or [postfix `...` operator](#postfix-open).
...
If an `x...` expression is used within a definition, the result is "reclosed" if
the expression is unified with any other value within that definition.
...
#VolumeType: foo: int
#VolumeDisk: {foo: int, bar: int}
#Def2: {
    volume: #VolumeType... // Not unified with anything else within definition.
}
d: #Def2
d: volume: #VolumeDisk & {bar: 2) // bar is allowed.
```
