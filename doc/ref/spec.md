...
The alias is specified using a postfix `~` operator after a label.
A single identifier after `~` creates an alias referring to the field value, while the dual form
`~(K,V)` creates two aliases: one for the label and one for the field value.
Either position in the dual form can use `_` (blank identifier) to skip binding that alias.

Aliases can appear in several positions:

After a label (`label~X: value`):

- binds the identifier `X` to the field itself (not its value).

After a dynamic field (`(label)~X: value`):

- binds the identifier `X` to the field itself.

After a dynamic field with dual form (`(label)~(K,V): value`):

- binds the identifier `K` to the concrete label resulting from evaluating `label`.
- binds the identifier `V` to the field itself.

After a pattern constraint (`[expr]~X: value`):

- binds the identifier `X` to the field matched by the pattern
  within the scope of that field's value (`value`).

After a pattern constraint with dual form (`[expr]~(K,V): value`):
...
// A value alias using let and self
foo~F: {
    let V = self
    x: V.a
}
...
("my" + "Field")~(K,V): { label: K, field: V }
// outputs: myField: { label: "myField", field: <field reference> }
```
