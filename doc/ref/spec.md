...
**`otherwise` clause**: Used for all other comprehension types as a default value.
Provides a default when the comprehension yields zero items.
...
within the otherwise clause, since these bindings have no meaningful
value when zero items are yielded.

If the comprehension source or condition contains an error, the error
propagates rather than triggering the else or otherwise clause

```ebnf
Comprehension       = Clauses StructLit [ OtherwiseClause ] .
...
OtherwiseClause     = ( "else" | "otherwise" ) StructLit .
```
...
// otherwise accesses outer scope
i: {
    let threshold = 10
    for x in [] { x } otherwise { otherwiseField: threshold }
}
j: { otherwiseField: 10 }
...
// try with else: otherwise when reference doesn't resolve
m: {
    optField?: int
