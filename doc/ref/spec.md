...
An optional marker declares a field constraint as described above.
A required marker additionally requires that once a struct is output as data,
the corresponding field _must_ be defined.

A struct that has an required field constraint with a bottom value
evaluates itself to bottom.
An optional field constraints with a bottom value,
while precluding that field from becoming defined,
does _not_ invalidate the struct that contains it.

The subsumption for fields with the various markers is defined as
```
{a?: x} ⊑ {a!: x} ⊑ {a: x}
```
for any given `x`.
