...
A _dynamic field_ is a field whose label is determined by an expression.
...
In front of a dynamic field label (`(X=expr): value`):
...
In front of a pattern constraint pattern (`[X=expr]: value`):
