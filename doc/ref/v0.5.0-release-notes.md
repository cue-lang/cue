...
release workflow. In a related change, 79a476b8 moves us to a CUE-based source
...
Docker manifest latest steps. This fixes a long-standing bug where by homebrew
...
As of 45aa5c09, CUE build artefacts published on GitHub are entirely
reproducible, thanks to Go's [support for reproducible, verifiable and verified
builds](https://research.swtch.com/vgo-repro).

## Encoders

When `cue import`-ing YAML, the `-R` recursive flag now only recursively
interprets text as YAML if it contains a newline f207b0b9.
...
Some use of these deprecated types remains: #1035, #1735, #1806. These will be
tidied up in later releases as the API surface matures towards v1.0.0.

The `cue` package now support `InlineImports` for `Value.Syntax()` ae562679. This
leverages new support within the internal exporter to creating self-contained
CUE. This change also deprecates `Value.ResolveReferences()`.
...
67ace837 disallowed deprecated language features in API. These features had long
been deprecated in `cmd/cue`.  This means that API operations will now fail on
deprectaed features whereas before they were handled in compatibility mode. The
`parser.FromVersion` feature can be used to reenable these deprecated features.

## Language

This release does not introduce any language changes. cf3ba232 made some minor
adjustments to the language specification to improve readability.
...
handled. 748a6858 move the evaluator to a model of per-field comprehensions.
...
even though CUE does not allow the result of a comprehension to depend on
another comprehension within a single struct. The way this works is that for
fields with a fixed prefix path in a comprehension value, the comprehension is
assigned to these respective fields.
...
Analogous to reference cycles, as of f1daf4b8 we allow comprehensions to use the
struct in which they are defined as source, as long as they do not introduce new
fields, which would alter the source on which they iterate.
...
on export, an option that can further be refined by asking the exporter to
inline imports.  These internal changes manifest in changes to `cmd/cue def` and
...
support arbitary-precision format 9bda2205.
...
might still satisfy `MinItems` later on. 849371d9 made a similar change for
`struct.MinFields`.
...
b8703c81. The Go version used to build `cmd/cue` is included as of 9b1f2488.
...
d573e0c2 added evaluator stats support. This is mostly intended for more
reliable performance analysis in `unity`. This is an experimental feature that
is really for internal use only.
