...
release workflow. In a related change, [CL
546920](https://cuelang.org/cl/546920) moves us to a CUE-based source
configuration for GoReleaser. This allows us to define the concept of 'latest',
state which indicates whether we should perform the homebrew release and the
Docker manifest latest steps. This fixes a long-standing bug whereby homebrew
taps and Docker images were created for the latest release, regardless of
whether that was a pre-release, or indeed a patch version of a non-current minor
version.

As of [CL 547507](https://cuelang.org/cl/547507), CUE build artefacts published
on GitHub are entirely reproducible, thanks to Go's [support for reproducible,
...
541561](https://cuelang.org/cl/541561). This leverages new support within the
internal exporter to creating self-contained CUE. This change also deprecates
...
value [CL 542769](https://cuelang.org/cl/542769).
...
537264](https://cuelang.org/cl/537264) made some minor adjustments to the
language specification to improve readability.
...
handled. [CL 529524](https://cuelang.org/cl/529524) move the evaluator to a
model of per-field comprehensions.
...
Analogous to reference cycles, as of [CL 544129](https://cuelang.org/cl/544129)
we allow comprehensions to use the struct in which they are defined as source,
as long as they do not introduce new fields, which would alter the source on
which they iterate.
...
support arbitary-precision format; see [CL
...
might still satisfy `MinItems` later on. [CL
546238](https://cuelang.org/cl/546238) made a similar change for
`struct.MinFields`.
...
4432aa66; see [CL 541898](https://cuelang.org/cl/541898).

## `cmd/cue`

Version control system (VCS) information is included in `cue version` as of
b8703c81. The Go version used to build `cmd/cue` is included as of [CL
