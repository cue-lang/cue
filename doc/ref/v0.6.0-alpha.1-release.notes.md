...
Release commit: 401cab2dcdb3e7ffeec482433bbeaf88fa3130b9

---

The main focus of this release is the introduction of [required
fields](https://github.com/cue-lang/proposal/blob/main/designs/1951-required-fields-v2.md).
...
752b8e4a adds `arch` to set of injectable system variables understood by
`cue/load`. The text at `cue help injection` explains how this in more detail.

8d31dad6 adds support for zero values in `cue.Value.Float64()`, which has the
effect of fixing the error when attempting to use `strconv.FormatFloat` with a
zero value.

48207fb5 fixes a long-standing bug to make HTML escaping in JSON an opt-in. This
means that `cue export` now respects the `--escape` flag when set.
...
is like `foo?: value`, but requires a regular value be provided for `foo` (a
...
As a general rule, all data and data templating should be defined with regular
...
This is non-idiomatic, because as a general rule schema should onlys be defined
in terms of field constraints.
...
string`, which doesn't provide much actionable information to the user to help
...
c3138e36 adds support for making dynamic fields optional or required. For
...
a6055ea7 removes support for old-style `::` definitions. This also includes
deprecation support. In a similar vein, https://cuelang.org/cl/547011 removes
the last vestiges of `<foo>: T`. This was once the notation for pattern
constraints.

## Builtins

2e70ac78 reimplements `pkg/list.Sort`. The resulting reduction in the number of
allocations and other work gives rise to a ~80% reduction in running time
against CUE benchmarks.

## `cmd/cue`

56caeaf8 improves the documentation for the `-l` flag passed to `cue import`.
This addresses a frequent point of confusion in questions to GitHub Discussions
and on Slack.

b44e16d7 fixes `cue get go` to respect the `--exclude` flag for constants. This
makes it possible to (for example) exclude all unexported identifiers from a
`cue get go` run.
