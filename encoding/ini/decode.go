...
//   - Section name dot handling is configured via [Config.SectionNameNesting].
...
// CUE struct fields when [Config.SectionNameNesting] is [SectionNamesNested].
//
...
const (
	...
	SectionNamesNested                                   // dots are treated as nested section separators
)
...
type Config struct {
	...
	// Set to [SectionNamesNested] to treat dots as nested section separators.
...
func (d *Decoder) Decode() (ast.Expr, error) {
	...
	for line := range bytes.SplitSeq(data, []byte("\n")) {
		...
		if trimmed[0] == '[' {
			...
			if d.cfg.SectionNameNesting == SectionNamesNested {
