...
//   - Quotes within values are treated as literal characters.
//   - All values are represented as CUE strings by default.
//     Set [ConfigOptions].TypedValues to parse booleans and numbers
//     into their corresponding CUE types.
//   - Blank lines are ignored.
//   - Keys and section names preserve their original case by default.
//     Set [ConfigOptions].CaseInsensitive to lowercase them.
...
//   - Escape sequences (e.g. \n, \\); backslashes are treated as literals.
...
type Config struct {
	// When true, lowercases keys and section names.
	// By default the original case is preserved.
	CaseInsensitive bool

	// When true, parses booleans and numbers into their corresponding CUE types.
...
func (d *Decoder) Decode() (ast.Expr, error) {
	...
	for line := range bytes.SplitSeq(data, []byte("\n")) {
		...
		pos := posForLine(tokenFile, lineOffsets, lineNum)
		field := makeField(key, value, pos, d.cfg.TypedValues)
		...
		if closeIdx >= 0 {
			...
			for _, marker := range []string{" ;", " #", "\t;", "\t#"} {
				if idx := strings.Index(rest, marker); idx >= 0 {
					value = strings.TrimRight(value[:afterClose+idx], " \t")
				}
			}
		}
	} else {
...
func makeField(key, value string, pos token.Pos, typedValues bool) *ast.Field {
	var v ast.Expr
	if typedValues {
		v = makeValueLit(value, pos)
	} else {
