...
func TestDecoder(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			dec := ini.NewDecoder("test.ini", strings.NewReader(input), nil)
...
func TestDecoderCaseInsensitive(t *testing.T) {
	...
	tests := []struct {
		...
		name: "LowercasesKeysAndSections",
		input: `
			AppName = MyApp
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			input := unindent(test.input)
			dec := ini.NewDecoder("test.ini", strings.NewReader(input), &ini.Config{
...
func TestDecoderTypedValues(t *testing.T) {
	...
	tests := []struct {
		...
		name: "StringsThatLookLikeNumbers",
		input: `
			zip = 01onal
			phone = 555-1234
			`,
