...
func constraintAllOf(key string, n cue.Value, s *state) {
	...
	for _, v := range items {
		x, sub := s.schemaState(v, s.allowedTypes, false)
...
func constraintIfThenElse(s *state) {
	...
	ifExpr, ifSub := s.schemaState(s.ifConstraint, s.allowedTypes, false)
	if hasThen {
		// The allowed types of the "then" constraint are constrained both
		// by the current constraints and the "if" constraint.
		thenExpr, _ = s.schemaState(s.thenConstraint, s.allowedTypes&ifSub.allowedTypes, false)
	}
	if hasElse {
		elseExpr, _ = s.schemaState(s.elseConstraint, s.allowedTypes, false)
	}
	if thenExpr == nil {
		thenExpr = top()
	}
