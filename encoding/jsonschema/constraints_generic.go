...
func constraintAddDefinitions(key string, n cue.Value, s *state) {
	...
	s.processMap(n, func(key string, n cue.Value) {
		...
		// for this node.
		if _, ok := s.defForValue.lookup(n); !ok {
			s.defForValue.set(n, nil)
			s.danglingRefs++
		}
		s.schema(n)
	})
...
func cueLocationForRef(s *state, n cue.Value, u *url.URL, schemaRoot *state) (importPath string, path cue.Path, err error) {
	...
	if !isAnchor {
		...
		if base.Exists() {
			...
			ds, ok := s.defForValue.lookup(target)
			if ds != nil {
			...
			if !ok {
				// Mark the target schema as needing an explicit definition by
				// setting its entry to nil. If it gets updated later, we'll go
				// through another pass and find the updated value above.
				s.defForValue.set(target, nil)
				s.danglingRefs++
			}
