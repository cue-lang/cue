...
// CRD holds an extracted Kubernetes CRD and the data it was derived from.
type CRD struct {
	// Versions holds the CUE schemas extracted from the CRD: one per
	// version.
	Versions map[string]*ast.File

	// Spec holds chosen fields extracted from the raw CRD document.
	Spec *CRDSpec

	// Raw holds the raw CRD document from which Spec is derived.
	Raw cue.Value
}
...
func ExtractCRDs(data cue.Value, cfg *CRDConfig) ([]*CRD, error) {
	...
	for crdIndex, crd := range crdInfos {
		...
		for i, version := range crd.Spec.Versions {
			...
			f, err := Extract(data, extractConfig)
			if err != nil {
				return nil, err
			}
		...
		crds[crdIndex] = &CRD{
			Versions: versions,
			Spec:     crdInfos[crdIndex],
			Raw:      crdValues[crdIndex],
		}
	}
	return crds, nil
}
