...
func kindToAST(k cue.Kind, nonExplicitOpen bool) ast.Expr {
	switch k {
	case cue.NullKind:
...
func (c *constraintInfo) setTypeUsed(n cue.Value, t coreType, nonExplicitOpen bool) {
	c.typ = kindToAST(coreToCUE[t], nonExplicitOpen)
	setPos(c.typ, n)
...
func (s *state) setTypeUsed(n cue.Value, t coreType) {
	...
	s.types[t].setTypeUsed(n, t, s.cfg.NonExplicitOpen)
}
...
const (
	nonExplicitlyOpen openness = iota
	explicitlyOpen
	explicitlyClosed
)
...
func (s *state) finalizeObject() {
	...
	if s.cfg.NonExplicitOpen && s.openness == nonExplicitlyOpen {
		// Nothing to do: the struct is non-explicitly open but
	...
	if needsTypeDisjunction {
		...
		for i, t := range s.types {
			...
			case allowed:
				npossible++
				if s.knownTypes&k != 0 {
					disjuncts = append(disjuncts, kindToAST(k, s.cfg.NonExplicitOpen))
				}
			}
		}
