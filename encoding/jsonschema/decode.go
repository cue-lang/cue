...
func (d *decoder) checkRegexp(n cue.Value, s string) bool {
	...
	if errors.As(err, &regErr) {
		...
		case syntax.ErrInvalidCharRange:
			...
			// class ranges than Go supports currently
			// (see https://github.com/golang/go/issues/14509)
			// so treat an unknown character class range as a
			// feature error rather than a bad regexp.
			// TODO translate names to Go-supported class
			// names when possible.
			if d.cfg.StrictFeatures {
				d.errf(n, "%v", err)
			}
