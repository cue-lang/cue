...
func (s *state) maybeDefine(expr ast.Expr) ast.Expr {
	...
	if def.importPath == "" {
		if !s.builder.put(def.path, expr, s.comment()) {
			s.errf(n, "redefinition of schema CUE path %v", def.path)
			return expr
		}
	}
	return s.refExpr(n, def.importPath, def.path)
}

func (s *state) defForNode(n cue.Value) *definedSchema {
	...
	// This node has been referred to but not
	// actually defined. We'll need another pass
	// to sort out the reference.
