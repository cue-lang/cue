...
func runExternalSchemaTests(t *testing.T, m *cuetdtest.M, filename string, s *externaltest.Schema) {
	...
	// Run tests against the Extract-only schema
	t.Run("Extract", func(t *testing.T) {
		for _, test := range s.Tests {
			t.Run(testName(test.Description), func(t *testing.T) {
				runExternalSchemaTest(t, m, ctx, s, test, schemaValue, &test.Skip)
			})
		}
	})

	// Run Generate round-trip tests for draft2020-12 only
	if vers != jsonschema.VersionDraft2020_12 {
		// Generation only supports 2020-12 currentle
		return
	}
	t.Run("RoundTrip", func(t *testing.T) {
		generatedSchemaValue, err := roundTripViaGenerate(t, schemaValue)
		if err != nil {
			t.Logf("location: %v", testdataPos(s))
			for _, test := range s.Tests {
				t.Run(testName(test.Description), func(t *testing.T) {
					testFailed(t, m, &test.SkipRoundTrip, test, err.Error())
				})
			}
			testFailed(t, m, &s.SkipRoundTrip, s, err.Error())
			return
		}
		testSucceeded(t, m, &s.SkipRoundTrip, s)

		// Run tests against the round-trip schema
		for _, test := range s.Tests {
			t.Run(testName(test.Description), func(t *testing.T) {
				runExternalSchemaTest(t, m, ctx, s, test, generatedSchemaValue, &test.SkipRoundTrip)
			})
		}
	})
}
...
func runExternalSchemaTest(t *testing.T, m *cuetdtest.M, ctx *cue.Context, s *externaltest.Schema, test *externaltest.Test, schemaValue cue.Value, skipField *externaltest.Skip) {
	defer func() {
	...
	if test.Valid {
		if err != nil {
			testFailed(t, m, skipField, test, errors.Details(err, nil))
		} else {
			testSucceeded(t, m, skipField, test)
		}
	} else {
		if err == nil {
			testFailed(t, m, skipField, test, "unexpected success")
		} else {
			testSucceeded(t, m, skipField, test)
		}
	}
...
func writeExternalTestStats(testDir string, tests map[string][]*externaltest.Schema) error {
	...
	fmt.Fprintf(outf, "Extract tests:\n\n")
	fmt.Fprintf(outf, "v3:\n")
	showStats(outf, "v3", false, false, tests)
	fmt.Fprintf(outf, "\nOptional tests\n\n")
	fmt.Fprintf(outf, "v3:\n")
	showStats(outf, "v3", true, false, tests)
	fmt.Fprintf(outf, "\nGenerate round-trip tests (draft2020-12 only):\n\n")
	fmt.Fprintf(outf, "v3:\n")
	showStats(outf, "v3", false, true, tests)
	fmt.Fprintf(outf, "\nOptional tests\n\n")
	fmt.Fprintf(outf, "v3:\n")
	showStats(outf, "v3", true, true, tests)
	return nil
}

func showStats(outw io.Writer, version string, showOptional, isRoundTrip bool, tests map[string][]*externaltest.Schema) {
	schemaOK := 0
	schemaTot := 0
	testOK := 0
	testTot := 0
	...
	for filename, schemas := range tests {
		...
		for _, schema := range schemas {
			schemaTot++
			schemaSkipped := getSchemaSkip(schema, isRoundTrip)[version] != ""
			if !schemaSkipped {
				schemaOK++
			}
			for _, test := range schema.Tests {
				testSkipped := getTestSkip(test, isRoundTrip)[version] != ""
				testTot++
				if !testSkipped {
					testOK++
				}
...
func getSchemaSkip(s *externaltest.Schema, isRoundTrip bool) externaltest.Skip {
	if isRoundTrip {
		return s.SkipRoundTrip
	}
	return s.Skip
}

func getTestSkip(s *externaltest.Test, isRoundTrip bool) externaltest.Skip {
	if isRoundTrip {
		return s.SkipRoundTrip
	}
	return s.Skip
