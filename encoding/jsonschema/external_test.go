...
func TestExternal(t *testing.T) {
	...
	if t.Failed() {
		if cuetest.DiffGoldenFiles() {
			return // an update would refuse to write anything
		}
	...
	files, err := externaltest.FormatTestDir(testDir, tests)
	qt.Assert(t, qt.IsNil(err))
	for _, filename := range slices.Sorted(maps.Keys(files)) {
		cuetest.WriteGoldenFile(t, filename, files[filename])
	}
	cuetest.WriteGoldenFile(t, "external_teststats.txt", externalTestStats(tests))
}
...
func externalTestStats(tests map[string][]*externaltest.Schema) []byte {
