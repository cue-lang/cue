...
func Generate(v cue.Value, cfg *GenerateConfig) (ast.Expr, error) {
	...
	fields := []ast.Decl{&ast.Field{
		Label: ast.NewStringLabel("$schema"),
		Value: ast.NewString(VersionDraft2020_12.String()),
	}}
	if len(g.defs) != 0 {
		...
		fields = append(fields, &ast.Field{
			Label: ast.NewStringLabel("$defs"),
			Value: &ast.StructLit{Elts: defFields},
		})
	}
	...
	return &ast.StructLit{Elts: fields}, nil
}
