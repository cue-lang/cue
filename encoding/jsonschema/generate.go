...
// isExplicitBottom reports whether v is an explicit bottom literal (_|_).
// This is distinct from values that evaluate to bottom due to errors or conflicts.
func isExplicitBottom(v cue.Value) bool {
	syntax := v.Syntax()
	if syntax == nil {
		return false
	}
	_, ok := syntax.(*ast.BottomLit)
	return ok
}
