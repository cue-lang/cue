...
type GenerateConfig struct {
	...
	// NameFunc is used to determine how references map to JSON Schema
...
func Generate(v cue.Value, cfg *GenerateConfig) (ast.Expr, error) {
	...
	g := &generator{
		cfg:    cfg,
		defs:   anyhash.NewMap[*CUERef, defEntry](cueRefHasher{}),
	...
	if n := g.defs.Len(); n != 0 {
		...
		slices.SortFunc(defKeys, func(k1, k2 *CUERef) int {
			return cmp.Compare(k1.Path.String(), k2.Path.String())
		})
		g.cfg.NameFunc(defKeys)
		hasEmpty := false
		slices.SortFunc(defKeys, func(k1, k2 *CUERef) int {
			hasEmpty = hasEmpty || k1.Name == "" || k2.Name == ""
			return cmp.Compare(k1.Name, k2.Name)
		})
		if hasEmpty {
			return nil, fmt.Errorf("NameFunc did not set Name field in all *CUERef values")
		}
		prev := ""
		for _, k := range defKeys {
			if k.Name == prev {
				return nil, fmt.Errorf("NameFunc returned non-unique name %q", k.Name)
			}
			prev = k.Name
		}
	}
	...
	if len(defKeys) > 0 {
		...
		for _, k := range defKeys {
			entry, _ := g.defs.Get(k)
			def := optimize(entry.item, g.unique)
...
type generator struct {
	...
	defs *anyhash.Map[*CUERef, defEntry]
...
// defEntry holds information about a single definition entry.
type defEntry struct {
	// ref is the canonical *CUERef pointer for this definition,
	// the same pointer used as the map key.
	ref *CUERef
	// item is the generated schema item for this definition.
	item internItem
}
...
func (g *generator) makeItem0(v cue.Value, mode closedMode) item {
	...
	case cue.NoOp, cue.SelectorOp:
		...
		if entry, ok := g.defs.Get(lookupRef); ok {
			return entry.ref
		}
		g.defs.Set(lookupRef, defEntry{ref: lookupRef}) // Prevent infinite loops on cycles.
		defMode := open
		if isDefinition(path) {
			defMode = closedRecursively
		}
		g.defs.Set(lookupRef, defEntry{ref: lookupRef, item: g.makeItem(v, defMode)})
		return lookupRef
	case cue.AndOp:
...
// See [GenerateConfig.NameFunc] for more information.
//
...
func DefaultNameFunc(refs []*CUERef) {
	if len(refs) == 0 {
		return
	}
...
// cueRefHasher implements [anyunique.Hasher]
// for the [CUERef] type.
//
// The [CUERef.Name] field is not included in the
// hash or equality because it's set after the map
// is populated.
type cueRefHasher struct{}

func (cueRefHasher) Hash(h *maphash.Hash, r *CUERef) {
	maphash.WriteComparable(h, r.Inst)
	for _, sel := range r.Path.Selectors() {
		h.WriteString(sel.String())
	}
}

func (cueRefHasher) Equal(x, y *CUERef) bool {
	return x.Inst == y.Inst && x.Path.String() == y.Path.String()
}
