...
// removeConjunction makes a best-effort attempt to remove the given conjunct
// from v.
func removeConjunction(v, conjunct cue.Value) cue.Value {
	var v1 cue.Value
	pos := conjunct.Pos()
	strVal := fmt.Sprintf("%#v", conjunct)
	for c := range conjuncts(v) {
		// Cheap check: if the positions are different, then they're
		// definitely different conjuncts.
		if c.Pos() == pos {
			if fmt.Sprintf("%#v", c) == strVal {
				continue
			}
		}
		log.Printf("different positions for %#v: %v; want %v", c, c.Pos(), pos)
		v1 = v1.Unify(c)
	}
	return v1
}
