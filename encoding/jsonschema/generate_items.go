...
func isolateRef(g *generator, expr ast.Expr) ast.Expr {
	...
	var refField ast.Decl
	rest := make([]ast.Decl, 0, len(st.Elts))
	for _, d := range st.Elts {
		if fieldLabel(d) == "$ref" {
			refField = d
		} else {
	...
	if refField == nil || len(rest) == 0 {
		return expr
	}
	rest = append(rest, makeField("allOf", ast.NewList(makeSchemaStructLit(refField))))
	return makeSchemaStructLit(rest...)
}
