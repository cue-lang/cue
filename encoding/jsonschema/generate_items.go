...
type item interface {
	...
	// It can use u to create new unique items.
	apply(f func(internItem, *uniqueItems) internItem, u *uniqueItems) item

	// hash writes the hash of the item to h; it should use u to find the hash
	// value to write for any items it contains.
...
func (i *itemNot) generate(g *generator) ast.Expr {
	return singleKeyword("not", i.elem.Get().generate(g))
}
...
func (i *itemItems) generate(g *generator) ast.Expr {
	...
	if i.rest.Get() != nil {
		fields = append(fields, makeField("items", i.rest.Get().generate(g)))
	}
...
func (i *itemContains) generate(g *generator) ast.Expr {
	fields := []ast.Decl{makeField("contains", i.elem.Get().generate(g))}
	if i.min != nil {
...
func (it *itemProperties) hash(h *maphash.Hash, u *uniqueItems) {
	// Hash properties in sorted order for consistency
	for _, name := range slices.Sorted(maps.Keys(it.properties)) {
		h.WriteString(name)
		u.writeHash(h, it.properties[name])
	}
	// Hash required fields in sorted order
	for _, name := range slices.Sorted(slices.Values(it.required)) {
		h.WriteString(name)
	}
	// Hash additionalProperties
	u.writeHash(h, it.additionalProperties)
	// Hash patternProperties in sorted order
	for _, name := range slices.Sorted(maps.Keys(it.patternProperties)) {
		h.WriteString(name)
		u.writeHash(h, it.patternProperties[name])
	}
}
...
func (i *itemProperties) generate(g *generator) ast.Expr {
	...
	for name, it := range i.properties {
		propFields = append(propFields, makeField(name, it.Get().generate(g)))
	}
	...
	if i.additionalProperties.Get() != nil {
		fields = append(fields, makeField("additionalProperties", i.additionalProperties.Get().generate(g)))
	}
	if len(i.patternProperties) > 0 {
		pp := &ast.StructLit{}
		for _, p := range slices.Sorted(maps.Keys(i.patternProperties)) {
			pp.Elts = append(pp.Elts, makeField(p, i.patternProperties[p].Get().generate(g)))
		}
...
func (i *itemProperties) apply(f func(internItem, *uniqueItems) internItem, u *uniqueItems) item {
	...
	if additionalProperties.Get() != nil {
...
func (i *itemIfThenElse) generate(g *generator) ast.Expr {
	fields := []ast.Decl{makeField("if", i.ifElem.Get().generate(g))}
	if i.thenElem.Get() != nil {
		fields = append(fields, makeField("then", i.thenElem.Get().generate(g)))
	}
	if i.elseElem.Get() != nil {
		fields = append(fields, makeField("else", i.elseElem.Get().generate(g)))
	}
	return makeSchemaStructLit(fields...)
}
...
// hashExpr hashes an AST expression using its formatted representation.
// This is a simple approach that ensures structurally equivalent expressions
// hash to the same value.
func hashExpr(h *maphash.Hash, expr ast.Expr) {
...
type internItem = anyunique.U[item]
