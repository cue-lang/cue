		...
		case *ast.Ellipsis:
			...
			if parent != nil {
				...
				case *ast.StructLit, *ast.File:
					// Closing a struct with ellipses results in a closed struct, so it's safe to remove `...` from `close({...})`
					if sc.current.inCloseCall || (sc.current.inParams && !sc.current.inDefinition) {
