...
import (
	...
	"cuelang.org/go/encoding/jsonschema/internal"
...
var closednessTests = []closednessTest{{
	...
	name: "UnifictionWithReferenceIntoClosedFieldOfStructLiteralInDefinition",
	src: `#x: {
		{a: close({b: 5, ...})}.a & {a:1}
    }`,
...
`,
	...
	// Although functions in CUE can be renamed, making it hard to tell if omitting ellipses is safe,
	// eliding the ellipsis from `matchN`'s parameters for a JSONSchema encoder's output is considered safe.
	// Keep this testcase for record.
