...
type structBuilderNode struct {
	...
	// value holds the value associated with the node, if any, not including the
	...
	// entries holds the entries present under this node.
...
func (b *structBuilder) entryForPath(p cue.Path, putting bool) *structBuilderNode {
	if err := p.Err(); err != nil {
	...
	n := &b.root
	n.isPresent = n.isPresent || putting
...
func (b *structBuilder) syntax() (*ast.File, error) {
	...
	for _, decl := range db.decls {
		if f, ok := decl.(*ast.Field); ok {
			sel, err := selectorForLabel(f.Label)
			if err != nil {
				panic(err) // should never happen
			}
			for _, ident := range b.refIdents[sel] {
				ident.Node = f.Value
			}
		}
	}
...
func (n *structBuilderNode) hasPresentEntry() bool {
