...
// Package koala converts XML to and from CUE as described here: aka.ms/koala.
//
// Details of the mapping are provided at aka.ms/koala, although a high-level summary is provided below:
//
// Elements are modeled as CUE structs,
// Attributes are modeled as struct fields prefixed with $,
// Inner text content of an element is modeled as a field named $$.
//
// Although the mapping differs to the Badgerfish convention (sklar.com/badgerfish), it is inspired by it.
//
...
type Decoder struct {
	...
	// a description of this model can be found at aka.ms/koala.
...
func NewDecoder(fileName string, r io.Reader) *Decoder {
	dec := &Decoder{reader: r, fileName: fileName}
	return dec
}
