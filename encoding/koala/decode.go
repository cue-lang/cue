...
func (dec *Decoder) decodeEndElement(xmlToken xml.EndElement) error {
	...
	if dec.currXmlElement != nil && dec.currInnerText != nil {
		...
		if len(dec.currXmlElement.children) == 0 {
			...
			err := dec.checkCurrXmlNodeForMixedContentError()
			if err != nil {
				return err
			}
		}
...
// return an error if the current XML element has non-whitespace text content and sub-elements
func (dec *Decoder) checkCurrXmlNodeForMixedContentError() error {
	xmlNode := dec.currXmlElement
	if len(xmlNode.children) > 0 {
		if !xmlNode.textContentIsWhiteSpace {
