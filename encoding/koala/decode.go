...
func (dec *Decoder) decodeEndElement() error {
	...
	if dec.currXmlElement != nil && dec.currInnerText != nil {
		...
		if len(dec.currXmlElement.children) == 0 {
			dec.currField.addFieldToCurrElement(dec.currInnerText)
...
func (cf *currFieldInfo) addFieldToCurrElement(field *ast.Field) {
	cf.field.Value.(*ast.StructLit).Elts = append(cf.field.Value.(*ast.StructLit).Elts, field)
}
