...
`,
  ...
  <test v="v2.1">
   ...
   <text>content</text>
		...
		expectedError: `myXmlFile.xml:2:3
schema.cue:2:7
  ...
  <test v="v2.1">
   ...
   <text>content</text>
		...
		text: {
			...
			expectedError: `myXmlFile.xml:3:4
schema.cue:4:8
test.edge.$n: conflicting values int and "2.65" (mismatched types int and string)
`,
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			dec, err := NewDecoder(fileName, strings.NewReader(test.inputXML))
			if err != nil {
			...
			if err != nil {
				t.Fatalf("failed to convert to file: %v", err)
			}
			c := cuecontext.New()
...
func TestElementDecoding(t *testing.T) {
	...
	note: {
		...
		{
			name: "1.1 Simple self-closing element",
			inputXML: `<note>
	<to/>
	<from>Jani</from>
	<heading>Reminder</heading>
  ...
  <h:tr>
	...
	node: {
		...
		{
			name: "13. Spacing either side of xml",
			inputXML: `
...
func TestErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "1. Text after root node followed by subelements",
			inputXML: `<note>
		mixed
		<from>Jani</from>
		<heading>Reminder</heading>
		...
		{
			name: "2. Text in middle of subelements",
			inputXML: `<note>
		<to/>
		mixed
		<from>Jani</from>
		...
		{
			name: "4. Text before end of root element",
			inputXML: `<note>
		<to/>
		<from>Jani <subElement/></from>
		<heading>Reminder</heading>
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			dec, e := NewDecoder(fileName, strings.NewReader(test.inputXML))
			if e != nil {
				t.Fatalf("failed to create decoder: %v", e)
			}
			_, err := dec.Decode()

			qt.Assert(t, qt.Equals(err.Error(), test.expectedError))
		})
	}
}
