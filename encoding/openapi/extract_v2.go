...
//
// Note: The form of the extracted representation may, and probably
...
func buildBase(doc cue.Value) (*ast.File, error) {
	expr, skip, err := buildBaseExpr(doc)
	if err != nil {
		return nil, err
	}
	if skip {
		panic("unexpected skip resulting from buildBaseExpr")
	}
...
func buildBaseExpr(v cue.Value) (expr ast.Expr, skip bool, err error) {
	...
	case cue.ListKind:
		...
		var elts []ast.Expr
		for it.Next() {
			ev, skip, err := buildBaseExpr(it.Value())
			if err != nil {
				return nil, false, err
			}
			if skip {
