...
func ExtractV2(data cue.Value, cfg *ExtractConfig) (*ast.File, error) {
	...
	version, err := detectVersion(data, cfg)
	if err != nil {
		return nil, err
	}
...
// detectVersion returns the OpenAPI version of the document, using the
// configured version if set and otherwise the document's "openapi" field.
func detectVersion(v cue.Value, cfg *ExtractConfig) (Version, error) {
