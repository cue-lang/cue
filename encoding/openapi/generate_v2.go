...
type GenerateConfig struct {
	...
	// schemas in the document; it must set a distinct [jsonschema.CUERef.Name]
	// for each. If nil, a default naming scheme is used.
...
//
// Note: this functionality is currently experimental. The form of the generated
// document may, and probably will, change from release to release.
func GenerateV2(v cue.Value, cfg *GenerateConfig) (ast.Expr, error) {
	...
	var allNames []string
	if cfg.AllSchemas {
		names, vals := topLevelSchemas(doc)
		allNames = names
	...
	exprs, shared, err := jsonschema.GenerateMulti(g.schemaValues, sharedSchemaRoot, &jsonschema.GenerateConfig{
		Version:         jsVersion,
	...
	// Fold the AllSchemas top-level schemas into the shared set.
	for i, name := range allNames {
		if _, ok := shared[name]; ok {
			continue // already present because it is referenced
		}
...
// to be generated, along with the component name to use for each.
func topLevelSchemas(doc cue.Value) (names []string, values []cue.Value) {
	...
	for it.Next() {
		...
		names = append(names, strings.TrimPrefix(sel.String(), "#"))
