...
type Config struct {
	...
	ReferenceFunc func(val cue.Value, path []string) string
...
func (c *Config) compose(inst cue.InstanceOrValue, schemas *ast.StructLit) (x *ast.StructLit, err error) {
	...
	case nil:
		if title == "" {
			...
			// TODO can we find out import path from title?
			//if p := val.ImportPath; title == "" && p != "" {
...
func (g *Generator) Schemas(inst cue.InstanceOrValue) (*OrderedMap, error) {
	comps, err := schemas(g, inst.Value())
	if err != nil {
		return nil, err
	}
