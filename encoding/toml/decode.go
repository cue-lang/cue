...
func NewDecoder(r io.Reader) *Decoder {
	...
	return &Decoder{r: r, seenNames: make(map[string]bool)}
}
...
type Decoder struct {
	...
	// seenNames tracks which dot-separated rooted paths we have already decoded,
...
func (d *Decoder) nextRootNode(tnode *toml.Node) error {
	...
	case toml.KeyValue:
		...
		topField := curField
		dotName := quoteLabelIfNeeded(curName)
		...
		for keys.Next() {
			...
			curField = nextField
			dotName += "." + quoteLabelIfNeeded(nextName)
		}
		if d.seenNames[dotName] {
			return fmt.Errorf("duplicate key: %s", dotName)
		}
		d.seenNames[dotName] = true
