...
func (d *debugPrinter) value0(v reflect.Value, impliedType reflect.Type) {
	...
	for {
		...
		if k == reflect.Interface {
			// For example, *ast.Ident can be the actual type behind an ast.Expr.
			actualType = v.Type()
		}
	}
	...
	case reflect.Slice, reflect.Struct:
		valueStart := d.pos()
		// We print the actual type when it differs from the implied type.
		if actualType != impliedType {
			d.printf("%s", actualType)
		}
		d.printf("{")
		d.level++
		var anyElems bool
