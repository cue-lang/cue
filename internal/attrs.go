...
func ParseAttrBody(pos token.Pos, s string) (a Attr) {
	...
	scan.InitAtOffset(pos.File(), []byte(s), fileOffset, nil, scanner.DontInsertCommas)
	for {
		start := scan.Offset() - fileOffset
		tok, err := scanAttributeTokens(&scan, 1<<token.COMMA|1<<token.BIND|1<<token.EOF)
		if err != nil {
		...
		case token.EOF:
			...
			return a
		case token.COMMA:
			val := s[start : scan.Offset()-1-fileOffset]
			a.appendField("", val, val) // All but final comma.
			continue
		}
		valStart := scan.Offset() - fileOffset
		key := s[start : valStart-1] // All but =.
		tok, err = scanAttributeTokens(&scan, 1<<token.COMMA|1<<token.EOF)
		if err != nil {
			// Shouldn't happen because bracket nesting should have been checked previously by
			// the regular CUE parser.
			a.Err = err
		...
		valEnd := len(s)
		if tok != token.EOF {
			valEnd = scan.Offset() - 1 - fileOffset // All but final comma
		}
...
func scanAttributeTokens(s *scanner.Scanner, close uint64) (token.Token, errors.Error) {
	for {
		pos, tok, _ := s.Scan()
		if s.ErrorCount > 0 {
			// Shouldn't happen because the text should have been scanned previously by
			// the regular CUE parser.
			return 0, errors.Newf(pos, "error scanning attribute text")
		}
		...
		case token.LPAREN:
			_, err = scanAttributeTokens(s, 1<<token.RPAREN)
		case token.LBRACE:
			_, err = scanAttributeTokens(s, 1<<token.RBRACE)
		case token.LBRACK:
			_, err = scanAttributeTokens(s, 1<<token.RBRACK)
		case token.RPAREN, token.RBRACK, token.RBRACE:
			err = fmt.Errorf("unexpected '%s'", tok)
		}
		if err != nil {
			return 0, errors.Newf(pos, "%v", err)
		}
	}
}
