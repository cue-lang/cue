	...
	jobs: {
		(trybot.key): {
			...
			steps: [
				...
				// Assert according to the containsDispatchTrailer expression that
				// we do not contain any dispatch trailer. Note this appears to
				// duplicate the check below but verifying against the GitHub
				// expression logic here is critical to ensuring we catch any
				// discrepancies in the matching logic
				json.#step & {
					if:  containsDispatchTrailer
					run: """
