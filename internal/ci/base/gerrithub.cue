		...
		test: {
			...
			steps: [
				...
				json.#step & {
					...
					// rerunLatestWorkflow runs the trybot workflow for the commit
					// at the tip of the branch under inspection.
					let rerunLatestWorkflow = {
						#repo:   string
						#branch: string
						#sha:    string
						"""
						id=$(\(curlGitHubAPI) "https://api.github.com/repos/\(#repo)/actions/workflows/\(trybot.key).yml/runs?branch=\(#branch)&event=push&head_sha=\(#sha)&per_page=1" | jq -r '.workflow_runs[] | .id')
						if [[ "$id" != "" ]]
						then
					...
					run: """
						...
						do
							...
							do
								...
								\(rerunLatestWorkflow & {#repo: githubRepositoryPath, #branch: "$j", #sha: "$sha", _})
								...
								git push -f trybot FETCH_HEAD:$j 2>&1 | tee push_result

								success=false
								for try in {1..20}; do
									echo "Push to trybot try $try"
									if git push -f trybot FETCH_HEAD:$j 2>&1 | tee push_result; then
										success=true
										break
									fi
									sleep 1
								...
								if grep up-to-date push_result
								then
									echo "Trigger workflow on \(trybotRepositoryPath)"
								   \(rerunLatestWorkflow & {#repo: trybotRepositoryPath, #branch: "$j", #sha: "$sha", _})
								else
