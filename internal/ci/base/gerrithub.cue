			...
			steps: [
				...
				for v in [{condition: "!=", expr: "fromJSON(steps.payload.outputs.value)"}, {condition: "==", expr: "github.event.client_payload"}] {
					json.#step & {
						...
						# be the commit from a CL. Note the order is significant here
						# because the ref must be the second argument in order for
						# us to use FETCH_HEAD below.
						git fetch origin refs/heads/${{ \(v.expr).targetBranch }} "${{ \(v.expr).ref }}"
