			...
			steps: [
				...
				for v in cases {
					...
					json.#step & {
						...
						# For now none of our trybot workflows rely on having full
						# history.  Setting --depth 1 speeds things up considerably.
						# If the trybot workflow does start to depend on more history
						# we can either increase this number or make it configurable.
						git fetch --depth 1 origin ${{ \(v.expr).ref }}
		...
		steps: [
			...
			json.#step & {
				...
				run:  """
						...
						# We fetch full depth here to ensure that from a consistency
						# perspective, the conditions in the trybot repo on the
						# default branch match the main repo for all workflows this
						# might trigger.
