...
// step is an abstraction over githubactions.#Step. Specifically, it allows a
// #run field to be specified that is an abstraction on top of the run field.
// If the #run field is specified, the run field is templated from it.
step: {
	githubactions.#Step

	// Extend the API of a step such that we define run-steps in terms of #run.
	// The run field used by GitHub Actions in the exported configuration is a
