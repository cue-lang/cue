...
#deleteBuildBranchJob: json.#job & {
	...
	steps: [
		json.#step & {
			run: """
				\(_#tempBotGitDir)
				git push \(#trybotRepositoryURL) :${GITHUB_REF#refs/heads/}
				"""
		},
	]
}
			...
			steps: [
				json.#step & {
					...
					run:  """
						...
						git push \(#trybotRepositoryURL) ci/${{ github.event.client_payload.payload.changeID }}/${{ github.event.client_payload.payload.commit }}
						"""
				},
			]
		}
