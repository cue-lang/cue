...
workflows: trybot: _base.#bashWorkflow & {
	...
	on: {
		push: {
			branches: list.Concat([["trybot/*/*", _base.params.testDefaultBranch], core.protectedBranchPatterns]) // do not run PR branches
	...
	jobs: {
		test: {
			...
			steps: [
				...
				json.#step & {
					if:  "github.repository == '\(core.githubRepositoryPath)' && (\(_base.#isProtectedBranch) || github.ref == 'refs/heads/\(_base.params.testDefaultBranch)')"
