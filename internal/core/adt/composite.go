...
type Vertex struct {
	...
	notRooted bool // indicates that there is no path from the root of the tree.
...
func (v *Vertex) Rooted() bool {
	return !v.notRooted && !v.Label.IsLet() && !v.IsDynamic
}
...
func (v *Vertex) GetArc(c *OpContext, f Feature, t ArcType) (arc *Vertex, isNew bool) {
	...
	arc = &Vertex{
		...
		Label:     f,
		ArcType:   t,
		notRooted: v.IsDynamic || v.Label.IsLet() || v.notRooted,
	}
