...
func (v *Vertex) VisitLeafConjuncts(f func(Conjunct) bool) {
	v.visitConjuncts(v.Conjuncts, f)
}

func (v *Vertex) visitConjuncts(a []Conjunct, f func(Conjunct) bool) bool {
	for _, c := range a {
		switch x := c.x.(type) {
		case *ConjunctGroup:
			if !v.visitConjuncts(*x, f) {
				return false
			}
		default:
			if !f(c) {
...
func (v *Vertex) VisitAllConjuncts(f func(c Conjunct, isLeaf bool)) {
	v.visitAllConjuncts(v.Conjuncts, f)
}

func (v *Vertex) visitAllConjuncts(a []Conjunct, f func(c Conjunct, isLeaf bool)) {
	for _, c := range a {
		switch x := c.x.(type) {
		case *ConjunctGroup:
			f(c, false)
			v.visitAllConjuncts(*x, f)
		default:
			f(c, true)
		}
	}
