...
func (e *Environment) evalCached(c *OpContext, x Expr) Value {
	...
	if !ok {
		...
		err := c.errs
		v = c.evalState(x, partial, allKnown, yield) // TODO: should this be finalized?
		c.e, c.src = env, src
		c.errs = err
