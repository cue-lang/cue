	...
	case *StructLit:
		...
		for _, d := range v.Decls {
			...
			case *Field:
				...
				arcState := arc.getNodeContext(n.ctx, 0)
				...
				arcState.node.addConjunctUnchecked(MakeConjunct(env, c, ci))
		...
		if len(fields) > 0 {
			...
			// Is this necessary (given closedness rules), is this pose
			// a performance problem?
			//
			// TODO: is it okay to insert the original StructLit? (Probably)
			// st := &StructLit{Decls: fields}
			// v.Init()
			n.node.AddStruct(v, env, ci)
			switch {
			case !ec.done:
				ec.structs = append(ec.structs, v)
			case len(ec.envs) > 0:
				v.Init()
			}
		}
