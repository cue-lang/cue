...
type Constraints struct {
	...
	// To check if a field is allowed, it's correpsonding CUE value can be
	// unified with this value.
	Allowed Value
}

// A PatternConstraint represents a single [pattern]: T.
//
// The Vertex holds a list of conjuncts to represet the constraints. We use
// a Vertex so that these can be evaluated and compared on equality.
// Unlike for regular Vertex values, these constraints have the
// CloseInfo.closeContext set. The closeContext is needed when matching
// subfields to ensure that conjuncts get inserted into the proper groups.
type PatternConstraint struct {
	Pattern    Value
	Constraint *Vertex
}

// insertListElipsis wraps insertConstraint by converting a list ellipsis
// to a pattern constraint representation.
func (n *nodeContext) insertListElipsis(offset int, ellipsis Conjunct) {
	ctx := n.ctx
...
func (n *nodeContext) insertConstraint(pattern Value, c Conjunct) bool {
	if c.CloseInfo.closeContext == nil {
...
func matchPatternValue(ctx *OpContext, pattern Value, f Feature, label Value) (result bool) {
	...
	// fast track, it is probably more efficient to handle everything in fast
	// track mode. One way to signal this would be to have a "value thunk" at
