...
func (c *OpContext) ResolveExists(x Expr, name string) (bool, *Bottom) {
	...
	// Follow defaults: a selection on a disjunction with a default
	// picks the default disjunct elsewhere in the language
	// (e.g. `(*{a: 1} | {b: 2}).a == 1`), and the old `!= _|_`
	// validator did the same via [OpContext.getDefault]. exists()
	// should agree: `exists(d.a)` is true when the default has
	// `a`, false when it doesn't. Only descend when the value is
	// actually a disjunction; otherwise Default() can produce a
	// stale copy (e.g. closing an open list).
...
func (c *OpContext) Evaluate(env *Environment, x Expr) (result Value, complete bool) {
	...
	// 	case nil:
	// 	case *scheduler:
	// 		result = c.NewErrf("unresolved value %s", c.Str(x))
	// 	default:
	// 		panic(r)
	// 	}
	// }()
...
// a permanent error. It uses full finalization mode (same as != _|_ comparison)
// to detect sub-field errors like conflicting values in {x: 1, y: 2} & {x: 3}.
// Any errors produced during evaluation are discarded after checking.
//
// For inline struct/binary expressions (BinaryExpr, StructLit), ChildErrors
// including IncompleteErrors from for-comprehensions over undefined fields
// are propagated via c.errs and cause this to return (false, nil).
//
// For let-bound expressions (LetReference), a fresh re-evaluation is forced
// to detect pending comprehension failures. If the fresh evaluation yields
// only incomplete errors, returns (false, nil). If it yields permanent errors,
// returns (false, permanentError) so the error propagates to the call site.
//
// For other pre-evaluated vertices (field references), only the top-level
// BaseValue is checked; sub-field incompleteness is not considered an error.
