...
//  - the key and value of the pattern constraints
...
//
// It should be clear that we cannot reliably compare the results of a Vertex to
// determine equality. We need a different strategy.
//
...
//
//  1. the disjunction holes that were already processed are equal
...
// may not filter some disjuncts that are identically logically. There are
// probably strategies to further optimize this. For instance, if all remaining
// holes do not contribute to closedness, which can be determined by walking up
// the closedness parent chain, we may be able to safely filter disjuncts with
// equal results.
//
// # Invariants
//
...
// with the closeContext at which each was defined. A single tasks is scheduled
// to process them all at once upon the first encounter of a disjunction.
//
// The algorithm is as follows:
//  - initialize the current Vertex n with the result evaluated so far as
...
func (n *nodeContext) processDisjunctions() *Bottom {
	...
	case 1:
		d := cross[0].node
		// XXX
		// TODO: is finalize here correct?
		// d.unify(n.ctx, allKnown, finalize)
		// d.freeNodeState()
		n.node.BaseValue = d

		// mergeVertex(n.node, cross[0].node)
	...
	for _, p := range cross {
		for j, d := range dn.disjuncts {
			...
			if err != nil {
				...
				continue
			} // XXX: use BaseValue as state may already be gone?
...
func (n *nodeContext) doDisjunct(c Conjunct, m defaultMode, mode runMode) (*nodeContext, *Bottom) {
	...
	d.disjunct = c
	// XXX: undo subfields frozen, node finalized
	...
	// openDebugGraph(n.ctx, v, "disjunct")
...
// appendDisjunct adds a disjunct d to a, if it is not a duplicate.
func appendDisjunct(ctx *OpContext, a []*nodeContext, d *nodeContext) []*nodeContext {
	if d == nil {
		return a
	}

	nx := d.node.Indirect().BaseValue
	if nx == nil || isCyclePlaceholder(nx) {
		nx = d.getValidators(finalized)
	}
...
outer:
	for _, xn := range a {
		if xn.node.status != finalized && d.node.status != finalized {
			// Partial node
			...
			for i, h := range xn.disjunctCCs { // TODO(perf): only iterate over completed
				x, y := findIntersections(h.cc, d.disjunctCCs[i].cc)
				if !equalPartialNode(xn.ctx, x, y) {
					continue outer
				}
			}
			if len(xn.tasks) != len(d.tasks) {
				continue
			}
			...
			vx, okx := nx.(Value)
			ny := xn.node.Indirect().BaseValue
			if ny == nil || isCyclePlaceholder(ny) {
				ny = d.getValidators(finalized)
			}
			vy, oky := ny.(Value)
			...
			// Complete nodes.
			if !Equal(ctx, xn.node.Indirect(), d.node.Indirect(), CheckStructural) {
				continue outer
			}
		}
