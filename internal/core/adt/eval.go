...
func (n *nodeContext) addVertexConjuncts(c Conjunct, arc *Vertex, inline bool) {
	...
	for _, ac := range arc.Conjuncts {
		var a []*Vertex
		if env != nil {
			a = env.Deref
		}
		if inline {
			ac = updateCyclic(ac, cyclic, nil, nil)
		} else {
			ac = updateCyclic(ac, cyclic, arc, a)
		}
		...
		ac.CloseInfo = closeInfo
		ac.CloseInfo.IsOptional = c.CloseInfo.IsOptional
		n.addExprConjunct(ac)
	}
}
