...
func (n *nodeContext) completeArcs(state VertexStatus) {
	...
	if !assertStructuralCycle(n) {
		...
		for _, a := range n.node.Arcs {
			...
			// TODO: remove hasError check. This has some impact here and there,
			// seemingly mostly for the better, but isolate in separate CL to
			// allow for better bug bisection.
			if !hasError && !a.isDefined() {
				continue
			}
