...
// currently being evaluated until it reaches a body registered in c.tryFrames.
//
...
func (c *OpContext) markSkipTry() {
	for v := c.vertex; v != nil; v = v.Parent {
		for i := range c.tryFrames {
			if c.tryFrames[i].body == v {
				c.tryFrames[i].skip = true
				return
			}
		}
	}
...
func (x *TryClause) yield(s *compState) {
	...
	// Register this body so that a ?-marked reference that fails while
	// finalizing it (or one of its descendants) is attributed to this try
	// rather than to an enclosing or interleaving one. See markSkipTry.
	c.tryFrames = append(c.tryFrames, tryFrame{body: v})
	v.Finalize(c)
	skip := c.tryFrames[len(c.tryFrames)-1].skip
