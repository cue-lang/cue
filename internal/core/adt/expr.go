...
func (x *TaggedInterpolation) evaluate(c *OpContext, state combinedFlags) Value {
	fun := c.value(x.Tag, combinedFlags{
		status:    partial,
		condition: scalarKnown,
		mode:      yield,
	})
	...
	for i := 0; i < len(parts); i += 2 {
		strings.Elems = append(strings.Elems, c.value(parts[i], combinedFlags{
			status:    finalized,
			condition: scalarKnown,
			mode:      yield,
		}))
	...
	args := []Value{
		c.value(strings, combinedFlags{
			status:    partial,
			condition: concreteKnown,
			mode:      yield,
		}),
		c.value(values, combinedFlags{
			status:    partial,
			condition: concreteKnown,
			mode:      yield,
		}),
	...
	return c.evalState(result, combinedFlags{
		status:    partial,
