...
func (x *TryClause) yield(s *compState) {
	...
	// Capture any final (non-incomplete) errors to report as optimization.
	// Report final errors immediately since they'd be hit during re-evaluation.
	if err := c.PopState(frame); err != nil && !err.IsIncomplete() {
		c.AddBottom(err)
	}
