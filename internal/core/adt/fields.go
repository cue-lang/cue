...
//
// insertArc1 is exclusively used by insertArc insert conjuncts for regular
...
// insertArc is exclusively used by nodeContext.insertArc1 and is used to insert
// a conjunct in closeContext, so grouped by other conjuncts from the same
...
//
// It the ensures that the fields contained in dst are allowed by the fields
...
outer:
	...
	// TODO: is this correct or do we need to merge the patterns?
	dst.Patterns = closed.Patterns

	dst.isClosed = true
}

// notAllowedError reports an "field not allowed" error in n and sets the value
...
// finalizePattern update c.Expr to a CUE Value representing all fields allowed
