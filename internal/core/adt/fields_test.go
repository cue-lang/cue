...
func writeArcs(x *adt.FieldTester, v *adt.Vertex) string {
	b := &strings.Builder{}
...
func writePatterns(x *adt.FieldTester, pairs []adt.PatternConstraint) string {
	b := &strings.Builder{}
...
func vertexString(x *adt.FieldTester, b *strings.Builder, v *adt.Vertex, indent int) {
	hasVertex := false
...
func pExpr(x *adt.FieldTester, e adt.Expr) string {
	a, _ := export.All.Expr(x.Runtime, "test", e)
	b, _ := format.Node(a)
	return string(b)
}
