...
// values as possible. It currently mostly for nodeContext values, but we plan
// to collect other values as well.
//
// ### nodeContext
//
...
//
// TODO(mem):
// Finally, we need to have a mechanism to collect temporary Vertex values that
// are not part of the output tree. This is done with the [nodeContext.toFree]
// field, which tracks Vertex values that can be cleaned up as soon as all
// processing for a nodeContext has been completed.
...
type reclaimer struct {
	...
	guard   bool
	force   bool // allow a node to be freed if it is not finalized.
	recurse bool
}
...
// reclaimRecursive reclaims all buffers of.v ana its children. It forces
