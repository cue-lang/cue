...
type overlayContext struct {
	...
	// TODO: similarly store copied tasks.
	tasks []*task
}
...
func (ctx *overlayContext) cloneRoot(root *nodeContext) *nodeContext {
	...
	// v.state.overlays = root

	// Allocate all closeContexts from the insertion points of the disjunctions
	// up to the root for all remaining disjunctions from the given position.
	// for ; i < len(root.disjunctions); i++ {
	// 	x := root.disjunctions[i]
	...
	// for _, c := range ctx.vertices {
	// 	// clone disjunctions if necessary
	// 	if c.state == nil {
	// 		continue
	// 	}
	// 	v := c.state.underlying
	// 	for _, t := range v.state.tasks {
	// 		if t.run == handleDisjunctions && t.state == taskREADY {
	// 			for _, d := range v.state.disjunctions {
	// 				cc := d.cloneID.cc
	// 				d.cloneID.cc = ctx.allocCC(cc)
	// 				for ; cc != nil; cc = cc.parent {
	// 					ctx.initCloneCC(cc)
	// 				}
	// 				c.state.scheduleDisjunction(d)
	// 			}
	// 		}
	// 	}
	// }
...
func (ctx *overlayContext) cloneVertex(x *Vertex) *Vertex {
	...
	*v = *x
	...
	if v.state != nil {
		// XXX: use new cloneNodeContext specifically for disjunctions.
...
func (ctx *overlayContext) initCloneCC(x *closeContext) {
	...
	if cc, ok := x.parentConjuncts.(*closeContext); ok {
		if cc.overlay == nil {
			panic("expected overlay")
		}
		o.parentConjuncts = cc.overlay
	}
