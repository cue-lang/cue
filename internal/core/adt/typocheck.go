...
func (c *conjunctInfoChecker) check(v *Vertex) {
	...
	if state := v.state; state != nil {
		if len(state.conjunctInfo) > 0 {
			b := c.ctx.NewErrf("vertex %v has %d conjunctInfos", v.Label, len(state.conjunctInfo))
			c.errs = errors.Append(c.errs, b.Err)
		}
		if v.state.generation != c.ctx.generation {
			b := c.ctx.NewErrf("vertex %v has generation %d, expected %d", v.Label, v.state.generation, c.ctx.generation)
			c.errs = errors.Append(c.errs, b.Err)
		}
	}
