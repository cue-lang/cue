...
// isInProgress reports whether v is in the mids of being evaluated. This means
...
func (v *Vertex) unify(c *OpContext, needs condition, mode runMode) bool {
	...
	//
	// TODO: is using mode == finalize here correct?
	...
	case mode != finalize: // XXX Or disjunction mode recursive?
		return false
	}
	...
	// XXX: what does this do? Is this necessary?
...
func (n *nodeContext) completeAllArcs(needs condition, mode runMode) bool {
	...
	if n.underlying != nil {
		...
		//
		// TODO: for now, this seems not necessary. Mover this will cause
		// benchmarks/cycle to display a spurious structural cycle. But it
		// shortens some of the structural cycle depths.
		//
	...
	for n.arcPos < len(n.node.Arcs) {
		...
		if a.ArcType == ArcPending {
			// TODO: cancel tasks?
			a.ArcType = ArcNotPresent // TODO: is this ever run
			continue
		}
...
func (v *Vertex) lookup(c *OpContext, pos token.Pos, f Feature, flags combinedFlags) *Vertex {
	...
	default:
		...
		arcState = arc.getState(c) // XXX: consider bare and initialize on
	}
