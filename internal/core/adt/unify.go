...
func (n *nodeContext) initNode(v *Vertex) {
...
func (v *Vertex) unify(c *OpContext, needs condition, mode runMode) bool {
	...
	defer c.PopArc(c.PushArc(v)) // XXX: remove?
	...
	// At this moment we have processed all conjuncts that may determine
	// a scalar value. We signal we have enough evidence to determine
	// the scalar even if we don't have it yet.
	// XXX: check mask necessary to set this.
	...
	if cc := n.node.cc; n.meets(needTasksDone) && !cc.done {
		cc := n.node.cc
		cc.notifyDone = true
		for _, a := range cc.arcs {
			if a.kind == NOTIFY {
				a.cc.decDependent(n.ctx, a.kind, cc) // REF(arcs)
			}
		}
	}
