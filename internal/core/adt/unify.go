...
// time. Any other pending task, or a task registered on the requested arc
// itself, may still contribute to f and defeats the fast path.
//
...
func (n *nodeContext) arcSettledForLookup(f Feature) bool {
	...
	for _, t := range n.tasks {
		if t.state == taskREADY && !t.isPushedDownComp() {
			return false
		}
	}
...
func (v *Vertex) lookup(c *OpContext, pos token.Pos, f Feature, flags Flags) *Vertex {
	...
	if state != nil {
		...
		// nodes that are already mid-execution, and the settled-arc guard
		// keeps us from forcing tasks that cannot affect the requested
		// arc (see [nodeContext.arcSettledForLookup]).
