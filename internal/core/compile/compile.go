...
func (c *compiler) labeledExprAt(k int, f ast.Decl, lab labeler, expr ast.Expr) adt.Expr {
	...
	if k < len(c.stack)-1 {
		// Cap the capacity, so that if there is growth, we don't overwrite any
		// values we need to restore later. This shouldn't happen too often, as
		// this will result in a non-reclaimable allocation.
