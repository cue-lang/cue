	...
	if f, ok := n.Scope.(*ast.Field); ok {
		...
		case *ast.Field:
			...
			if alias, _ := f.Label.(*ast.Alias); alias != nil {
				if f.Alias != nil {
					// TODO(ai): report error and pick f.Alias
				}
				ident = alias.Ident
			} else if f.Alias != nil {
				ident = f.Alias.Field
			} else {
				// TODO(ai): report error
			}
...
func (c *compiler) markAlias(d ast.Decl) {
	...
	case *ast.Field:
		...
		if x.Alias != nil {
			...
			if x.Alias.Field != nil && x.Alias.Field.Name != "_" {
				c.insertAlias(x.Alias.Field, aliasEntry{
					// label:  (*fieldLabel)(x),
					source: x.Alias,
				})
			}
			// if x.Alias.Label != nil && x.Alias.Label.Name != "_" {
			// 	c.insertAlias(x.Alias.Label, aliasEntry{
			// 		label:  (*fieldLabel)(x),
			// 		source: x.Alias,
			// 	})
			// }
...
func (c *compiler) decl(d ast.Decl) adt.Decl {
	...
	case *ast.Field:
		...
		case *ast.ListLit:
			...
			if a := x.Alias; a != nil && a.Label != nil && a.Label.Name != "_" {
...
func (c *compiler) addLetDecl(d ast.Decl) {
	...
	case *ast.Field:
		...
		} else if a := x.Alias; a != nil && a.Field != nil && a.Field.Name != "_" {
			ident = x.Alias.Field
		} else {
			break
		}
