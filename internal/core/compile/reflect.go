...
var allows = &adt.Builtin{
	...
	Func: func(call adt.BuiltinCallContext) adt.Expr {
		...
		// Use Arg, not Value: the first argument is used as a schema, and
		// the pre-evaluated argument is unwrapped, which discards the
		// closedness that close() records on a wrapper vertex.
		v, ok := call.Arg(0).(*adt.Vertex)
		if !ok {
