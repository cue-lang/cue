...
// Dependencies
//
// A dependency a reference relation from one Vertex to another. A Vertex
...
//             |-    \- Target Node     Pointed to be Reference
//             |-         \- UsedPath   The sole path used within Node
//
// Also verify that these concepts are correctly reflected in the API:
// Source:
...
// Recurse visits the dependencies of this dependency, using the same
// visit function as original.
func (d *Dependency) Recurse() {
	d.v.visit(d.Node, false)
}
...
func (c *visitor) reportDependency(env *adt.Environment, ref adt.Resolver, v *adt.Vertex) {
	...
	if !v.Rooted() {
		...
		// explicitly mark whether to visit non-rooted nodes. Visiting these
		// may be necessary when substituting values.
		switch _, ok := ref.(*adt.FieldReference); {
		case !ok:
			// Do not report non-rooted for selectors.
			return
...
// markConjuncts transitively mark all reference of the current node.
...
// markInternalResolvers marks dependencies for non-rooted nodes. As such
