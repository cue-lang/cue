...
func (e *exporter) decl(env *adt.Environment, d adt.Decl) ast.Decl {
	...
	case *adt.DynamicField:
		...
		// // Replace with a concrete label if the dynamic label was made concrete
		// // unconditionally.
		// // TODO: use the evaluator or some other faster mechanism. It appears
		// // that the evaluator that is being passed down isn't always accurate.
		// if env.Vertex != nil {
		// 	for _, arc := range env.Vertex.Arcs {
		// 		for _, c := range arc.Conjuncts {
		// 			if arc.Label.IsRegular() && c.Field() == x {
		// 				f.Label = e.value(arc.Label.ToValue(e.ctx)).(ast.Label)
		// 			}
		// 		}
		// 	}
		// }
