...
func (e *exporter) sortBinaryTree(env *adt.Environment, x *adt.BinaryExpr) (b ast.Expr) {
	...
	slices.SortStableFunc(exprs, lessLeafNodes)
...
func lessLeafNodes[T adt.Node](a, b T) int {
	if diff := cmpInt(typeOrder(a), typeOrder(b)); diff != 0 {
		return diff
	}
	...
	if cmp := strings.Compare(posA.Filename(), posB.Filename()); cmp != 0 {
		return cmp
	}

	if diff := cmpInt(posA.Offset(), posB.Offset()); diff != 0 {
		return diff
	}
