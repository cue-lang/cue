	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			testAllPermutations(t, indexer, testCase.inputs, func(t *testing.T, permutation [][]adt.Feature, graph *topological.Graph) {
				...
				if !slices.EqualFunc(cyclesNames, testCase.expected, slices.Equal) {
					t.Fatalf("\nFor permutation: %v\n       Expected: %v\n            Got: %v", permutationNames(indexer, permutation), testCase.expected, cyclesNames)
				}
			})
		})
	}
