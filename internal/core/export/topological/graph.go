...
func VertexFeatures(indexer adt.StringIndexer, v *adt.Vertex) []adt.Feature {
	...
	structs := slices.Clone(v.Structs)
	slices.SortFunc(structs, func(a, b *adt.StructInfo) int {
	...
	isImplicit := func(s *adt.StructInfo) bool {
	...
	for _, s := range structs {
		...
		if src := s.Src; src == nil && currentFilename != "" {
			currentFilename = ""
			previous = previous[:0]
			next = next[:0]
		} else if src != nil {
			if filename := src.Pos().Filename(); filename != currentFilename {
		...
		implicit := isImplicit(s)
		if implicit && len(next) != 0 {
			previous = append(previous, next...)
			next = next[:0]
		}
		debug("starting root. Implicit unification? %v\n", implicit)
		next = append(next, addEdges(builder, dynamicFields, outgoing, previous, implicit, s)...)
	}
