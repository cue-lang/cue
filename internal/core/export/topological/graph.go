...
func structLitPositions(v *adt.Vertex) map[*adt.StructLit]int {
	var parentStructLitsPositions map[*adt.StructLit]int
	if parent := v.Parent; parent != nil && v.Label != adt.InvalidLabel {
		var parentStructLits []*adt.StructLit
		for _, arc := range parent.Arcs {
			arc.VisitAllConjuncts(func(c adt.Conjunct, isLeaf bool) {
				...
				if isStructInfo && isField && field.Label == v.Label {
					parentStructLits = append(parentStructLits, expr)
				}
			})
		}
		slices.SortFunc(parentStructLits, func(a, b *adt.StructLit) int {
			aPos, bPos := a.Src.Pos(), b.Src.Pos()
			if c := cmp.Compare(aPos.Filename(), bPos.Filename()); c != 0 {
				return c
			}
			return cmp.Compare(aPos.Offset(), bPos.Offset())
		})
		if len(parentStructLits) != 0 {
			parentStructLitsPositions = make(map[*adt.StructLit]int)
			for idx, structLit := range parentStructLits {
				parentStructLitsPositions[structLit] = idx
			}
		}
	}
	return parentStructLitsPositions
}
...
func VertexFeatures(indexer adt.StringIndexer, v *adt.Vertex) []adt.Feature {
	...
	// vertex's own label. We sort these by filename and offset, and
	// then later add extra edges to the graph of features to ensure
	// this ordering is honoured.
	parentStructLitsPositions := structLitPositions(v)
	...
	slices.SortFunc(roots, func(a, b *adt.StructInfo) int {
		aPos, aFound := parentStructLitsPositions[a.StructLit]
		bPos, bFound := parentStructLitsPositions[b.StructLit]
		if aFound == bFound {
	...
	for _, s := range roots {
		...
		_, foundInParent := parentStructLitsPositions[s.StructLit]
		if foundInParent {
