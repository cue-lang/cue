...
func (graph *Graph) StronglyConnectedComponents() []*StronglyConnectedComponent {
	...
	for idx, node := range graph.nodes {
		node.sccNodeState = &nodeStates[idx]
	}
...
func (scc *sccFinderState) findSCC(cur *Node) {
	...
	for _, next := range cur.Outgoing {
		...
		if !nextScc.visited {
			...
			// the current node and the next node completes a
			// cycle.
			curScc.lowLink = min(curScc.lowLink, nextScc.index)
		}
	}

	// If the lowlink value of the node is equal to its DFS value, this is the
	// head node of a strongly connected component that's shaped by the node
	// and all nodes on the stack.
	if curScc.lowLink == curScc.index {
		component := &StronglyConnectedComponent{visited: true}

		var componentNodes Nodes

		for idx := len(scc.stack) - 1; idx >= 0; idx-- {
			nodeN := scc.stack[idx]
