...
//	x: a & b & c
//
// for x, the StructInfos corresponding to a, b and c will all be
...
type structMeta struct {
	...
	// Should this struct be considered to be part of an explicit unification (e.g. x & y)?
	isExplicit bool
...
func (sMeta *structMeta) String() string {
	...
	return fmt.Sprintf("{%p sl:%p %v (explicit? %v; root? %v)}", sMeta, sl, sMeta.pos, sMeta.isExplicit, sMeta.isRoot)
}

func (sMeta *structMeta) hasDynamic(dynamicFields map[*adt.DynamicField][]adt.Feature) bool {
	for _, decl := range sMeta.structInfo.Decls {
		if dynField, ok := decl.(*adt.DynamicField); ok {
			if _, found := dynamicFields[dynField]; found {
				return true
			}
		}
	}
...
// expression &, then we look up the structMeta for the
// arguments to the binary expression, and mark them as explicit
// unification.
...
func (batchesPtr *structMetaBatches) appendBatch(batch structMetaBatch) {
	...
	} else if prevBatch := batches[l-1]; batch.isExplicit() && prevBatch.isExplicit() && batch[0].pos.Filename() == prevBatch[0].pos.Filename() {
...
type vertexFeatures struct {
	...
	dynamicFields map[*adt.DynamicField][]adt.Feature
	outgoing      map[adt.Decl][]*structMeta
}

func (self *vertexFeatures) compareStructMeta(a, b *structMeta) int {
	if c := comparePos(a.pos, b.pos); c != 0 {
		return c
	}
	aHasDyn, bHasDyn := a.hasDynamic(self.dynamicFields), b.hasDynamic(self.dynamicFields)
	switch {
...
func VertexFeatures(indexer adt.StringIndexer, v *adt.Vertex) []adt.Feature {
	...
	dynamicFields := dynamicFieldsFeatures(v)
	...
	for _, root := range roots {
		if len(batch) == 0 || (batch[0].pos == root.pos && !root.hasDynamic(dynamicFields)) {
	...
	for _, batch := range batches {
		...
		if len(previousBatch) != 0 && previousBatch[0].pos.Filename() != batch[0].pos.Filename() {
			previous = nil
		}
...
func (vf *vertexFeatures) addEdges(previous []adt.Feature, sMeta *structMeta) []adt.Feature {
	debug("--- S %p (%p :: %T) (sl: %p) (explicit? %v) ---\n", sMeta, sMeta.structInfo.Decl, sMeta.structInfo.Decl, sMeta.structInfo.StructLit, sMeta.isExplicit)
	debug(" previous: %v\n", previous)
	...
	for idx, decl := range sMeta.structInfo.Decls {
		debug(" %p / %d: d (%p :: %T)\n", sMeta, idx, decl, decl)
		...
		case *adt.DynamicField:
			...
			if labels := vf.dynamicFields[decl]; len(labels) > 0 {
				currentLabel = labels[0]
				vf.dynamicFields[decl] = labels[1:]
			}
		}
		if currentLabel != adt.InvalidLabel {
			...
			if exists && node.structMeta == sMeta {
				debug("    skipping 1\n")
			} else if exists && !sMeta.isExplicit && filename != "" && node.structMeta != nil && node.structMeta.pos.Filename() == filename {
				debug("    skipping 2\n")
			} else {
				debug("    %v %v\n", node, exists)
