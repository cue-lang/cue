...
var SharedRuntime = sync.OnceValue(func() *Runtime {
	...
	//
	// TODO(mvdan): is it a problem that the evaluator version in SharedRuntime
	// follows the global setting, which may be different than what was supplied via
	// Go code for each context like cuecontext.EvaluatorVersion(cuecontext.EvalV3)?
	cueexperiment.Init()
