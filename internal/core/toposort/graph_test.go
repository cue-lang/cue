...
func makeFeatures(indexer adt.StringIndexer, inputs [][]string) [][]adt.Feature {
	...
	for i, names := range inputs {
		...
		for j, name := range names {
			features[j] = adt.MakeStringLabel(indexer, name)
		}
		result[i] = features
	}
	return result
...
func permutationNames(indexer adt.StringIndexer, permutation [][]adt.Feature) [][]string {
	...
	for i, features := range permutation {
		permNames[i] = featuresNames(indexer, features)
	}
	return permNames
}
...
func featuresNames(indexer adt.StringIndexer, features []adt.Feature) []string {
	...
	for i, feature := range features {
		names[i] = feature.StringValue(indexer)
	}
	return names
}
