...
func main() {
	...
	for f, s := range try(try(rep.Worktree()).Status()) {
		...
		revertFileWithStatsOnlyChanges(base, rep, f)
	}
}

type hasher func(string) plumbing.Hash

// head creates a hasher for the file contents of the current index (staged
// files).
...
// head creates a hasher for files in the HEAD commit.
func head(rep *git.Repository) hasher {
...
func revertFileWithStatsOnlyChanges(base hasher, rep *git.Repository, filepath string) {
	println("===", filepath)
	...
	baseContents := try(ioutil.ReadAll(r))

	aa := txtar.Parse(baseContents)
	...
	os.WriteFile(filepath, baseContents, 0644)
