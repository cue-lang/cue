...
var cmds = map[string]Cmd{
	...
	"oauthregistry": {
		Usage: "oauthregistry <mode>",
		Run:   cmdOauthRegistry,
	},
}
...
// cmdOauthRegistry starts an HTTP server with enough endpoints to test
// `cue login`, and points CUE_REGISTRY at it. Its single argument describes the
// oauth server's behavior; see [NewMockRegistryOauth].
func cmdOauthRegistry(e CmdEnv, args []string) error {
	if len(args) != 1 {
		return ErrUsage
	}
	srv, err := NewMockRegistryOauth(args[0])
	if err != nil {
		return err
	}
	e.Defer(srv.Close)
	u, _ := url.Parse(srv.URL)
	e.Setenv("CUE_REGISTRY", u.Host+"+insecure")
	return nil
}
