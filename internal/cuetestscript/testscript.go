...
// SetupEnv returns an [Env] that operates on e, for use by a
// [testscript.Params.Setup] function running the cue command in process.
func SetupEnv(e *testscript.Env) Env {
	return setupEnv{e}
}

// TestscriptCmds returns the commands from [Cmds] in the form expected by
...
func tsCmd(cmd Cmd) func(ts *testscript.TestScript, neg bool, args []string) {
...
// setupEnv adapts [testscript.Env] to [Env].
...
// cmdEnv adapts [testscript.TestScript] to [CmdEnv].
type cmdEnv struct {
