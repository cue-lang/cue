...
// isHiddenOrDef reports whether name represents a hidden field (_foo, _#foo)
// or a definition (#foo). Used to distinguish these from list-element index
// selectors (e.g. "0", "1") that appear when iterating a list-embedded struct.
func isHiddenOrDef(name string) bool {
...
func (c *cmpCtx) cmpStruct(path cue.Path, s *ast.StructLit, val cue.Value) error {
	...
	for iter.Next() {
		...
		if hasEmbed && !sel.IsString() && !isHiddenOrDef(name) {
			continue
		}
