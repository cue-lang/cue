...
type parsedTestAttr struct {
	...
	// "closed", "leq", "skip", "permute", "file", "desc", "ignore".
	directive string
...
type errArgs struct {
	...
	// the line containing the @test attribute.  posSet is true when pos= was
...
func parseTestAttr(a *ast.Attribute) (parsedTestAttr, error) {
	...
	if len(parsed.Fields) == 0 || (len(parsed.Fields) == 1 && parsed.Fields[0].Key() == "" && parsed.Fields[0].Value() == "") {
...
func extractTestAttrs(f *ast.File, fileName string) []attrRecord {
	...
	walkStruct = func(sl *ast.StructLit, path cue.Path) {
		...
		for _, elt := range sl.Elts {
			...
			case *ast.Field:
				subName := identStr(e.Label)
				subPath := appendPath(path, subName)
				walkField(e, subPath)
				newElts = append(newElts, elt)
	...
	for _, decl := range f.Decls {
		...
		fieldName := identStr(field.Label)
		fieldPath := cue.MakePath(cue.Str(fieldName))
...
// appendPath appends a string selector to an existing path.
func appendPath(base cue.Path, name string) cue.Path {
	sels := base.Selectors()
	return cue.MakePath(append(sels, cue.Str(name))...)
}
...
// fileHasTestAttrs reports whether any @test(...) attribute appears anywhere
// in f — as a field attribute on any field or as a decl attribute inside any
// struct literal at any nesting depth.
func fileHasTestAttrs(f *ast.File) bool {
	return declsHaveTestAttrs(f.Decls)
}
...
func (r *inlineRunner) runArchive() {
	...
	// ignoredNames: fields marked @test(ignore) — exempt from the sub-test
	// list but NOT added as sub-tests (no t.Run call).
	...
	for _, rec := range allRecords {
		...
		name := sels[0].String()
		if len(sels) == 1 && rec.parsed.directive == "ignore" {
			ignoredNames[name] = true
		} else {
			rootNames[name] = true
		}
	}
	// If a field is both ignored and a root (shouldn't happen in well-formed
	// files but can arise from multiple attributes), root takes precedence.
	for name := range rootNames {
		delete(ignoredNames, name)
	}
	...
	for _, f := range cueFiles {
		for _, decl := range f.strippedAST.Decls {
			...
			if !rootNames[name] || ignoredNames[name] {
				continue
			}
...
func selectActiveDirectives(records []attrRecord, path cue.Path, version string) []parsedTestAttr {
	...
	result := make([]parsedTestAttr, 0, len(byDirective))
	for _, pa := range byDirective {
...
func (r *inlineRunner) runDirective(t *testing.T, path cue.Path, val cue.Value, pa parsedTestAttr) {
	...
	case "ignore":
		// @test(ignore) marks a field as a fixture or reference — no assertions.
		// The coverage check excludes it; the runner never dispatches a sub-test for it.
	case "desc":
...
func (r *inlineRunner) runEqInline(t *testing.T, path cue.Path, val cue.Value, pa parsedTestAttr) {
	...
	cmpErr := (&cmpCtx{baseLine: 0}).astCmp(nil, expr, val)
	if cmpErr == nil {
...
func (r *inlineRunner) applyPosWritebacks() {
	...
	for i, f := range r.archive.Files {
		...
		sort.Slice(writes, func(a, b int) bool {
			return writes[a].attrOffset > writes[b].attrOffset
		})
