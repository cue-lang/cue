...
func eqWriteValue(opCtx *adt.OpContext, b *strings.Builder, v cue.Value, nestedIndent string) {
	...
	if k == cue.ListKind {
		if nestedIndent != "" {
			var compact strings.Builder
			eqWriteList(opCtx, &compact, vx, "")
			if compact.Len() <= eqCompactThreshold {
				b.WriteString(compact.String())
				return
			}
		}
		eqWriteList(opCtx, b, vx, nestedIndent)
		return
	}
...
func eqWriteList(opCtx *adt.OpContext, b *strings.Builder, vx *adt.Vertex, nestedIndent string) {
	b.WriteByte('[')
	first := true
