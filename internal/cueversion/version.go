...
func UserAgent(clientType string) string {
	...
	return fmt.Sprintf("Cue/%s (%s; %s) Go/%s (%s/%s)", LanguageVersion(), clientType, BuildVersion(), goVersion, runtime.GOOS, runtime.GOARCH)
}
