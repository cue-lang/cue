...
func (d *differ) diffValue(x, y cue.Value) (kind Kind, script *EditScript) {
	...
	// be evaluated twice. We might solve this by marking destination vertices
	// as shared.
	...
	if xOrig != nil && yOrig != nil && (xOrig.IsShared || yOrig.IsShared) {
		...
		if cached, ok := d.seen[k]; ok {
			// Returning nil for the EditScript is safe: Modified with a nil
			// script is the existing signal for "differs but no field-level
			// detail", and Profile.Diff synthesises a bare EditScript{X,Y} for
			// that case. For Identity a nil script is always correct.
			return cached, nil
		}
		defer func() {
			if d.seen == nil {
