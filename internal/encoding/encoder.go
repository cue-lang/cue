	...
	retryOpen:
		...
		if err != nil {
			if errors.Is(err, fs.ErrExist) {
				...
				// This is done as a retry to avoid an Lstat before every OpenFile.
				stat, err := os.Lstat(path)
				if err == nil && stat.Mode()&modePipe != 0 {
					mode = os.O_WRONLY
					goto retryOpen
				}
...
// modePipe includes all the file mode types where --outfile can write to
// without overwriting the entire file with a new regular file.
var modePipe = fs.ModeNamedPipe | fs.ModeDevice
