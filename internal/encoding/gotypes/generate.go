...
func Generate(ctx *cue.Context, insts ...*build.Instance) error {
	...
	for len(insts) > 0 { // we append imports to this list
		...
		appendf("// Code generated by \"cue exp gengotypes\"; DO NOT EDIT.\n\n")
...
func (g *generatedDef) appendf(format string, args ...any) {
...
func (g *generator) emitType(val cue.Value, optional bool, optionalStg optionalStrategy) error {
	...
	case cue.StructKind:
		...
		for iter.Next() {
			...
			if strings.ContainsAny(cueName, "\\\"`,\n") {
				g.def.appendf("// CUE field %q: encoding/json does not support this field name\n\n", cueName)
				continue
			}
			g.def.appendf("%s ", goName)
			...
			g.def.appendf(" `json:\"%s%s\"`", cueName, omitEmpty)
			g.def.appendf("\n\n")
		}
		g.def.appendf("}")
	case cue.ListKind:
		...
		if !elem.Exists() {
			...
			g.def.appendf("any /* CUE closed list */")
		} else if err := g.emitType(elem, false, optionalStg); err != nil {
			return err
		}

	case cue.NullKind:
		g.def.appendf("*struct{} /* CUE null */")
	case cue.BoolKind:
		g.def.appendf("bool")
	case cue.IntKind:
		g.def.appendf("int64")
	case cue.FloatKind:
		g.def.appendf("float64")
	case cue.StringKind:
		g.def.appendf("string")
	case cue.BytesKind:
		g.def.appendf("[]byte")

	case cue.NumberKind:
		// Can we do better for numbers?
		g.def.appendf("any /* CUE number; int64 or float64 */")

	case cue.TopKind:
		g.def.appendf("any /* CUE top */")
	...
	default:
		...
		if op, _ := val.Expr(); op == cue.OrOp {
			g.def.appendf("any /* CUE disjunction: %s */", k)
			break
		}
		g.def.appendf("any /* TODO: IncompleteKind: %s */", k)
	}
	return nil
}
...
func (g *generator) emitTypeReference(val cue.Value) bool {
	...
	case "time.Duration":
		...
		g.def.appendf("string /* CUE time.Duration */")
		return true
	case "time.Time":
		defsOnly = false
	}
