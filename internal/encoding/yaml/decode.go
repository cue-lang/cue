...
// block-style nodes, it skips to the first non-whitespace character
// after the anchor.  When the node has no anchor, it returns the
// node's position as-is.
...
func (d *decoder) sequence(yn *yaml.Node) (ast.Expr, error) {
	scopeEnd := d.scopeEnd // save before the loop modifies it
	list := &ast.ListLit{
	...
	if yn.Style&yaml.FlowStyle != 0 {
		...
		list.Rbrack = d.tokFile.Pos(scopeEnd-1, rel)
...
func (d *decoder) mapping(yn *yaml.Node) (ast.Expr, error) {
	scopeEnd := d.scopeEnd // save before insertMap modifies it
	strct := &ast.StructLit{
	...
	if yn.Style&yaml.FlowStyle != 0 {
		...
		strct.Rbrace = d.tokFile.Pos(scopeEnd-1, rel)
