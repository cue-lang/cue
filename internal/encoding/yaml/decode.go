...
type Decoder struct {
	...
	pendingMergeComments *yaml.Node
...
func (d *Decoder) addComments(n ast.Node, yn *yaml.Node, linePos int8) {
	...
	headComment, footComment := yn.HeadComment, yn.FootComment
	if _, ok := n.(*ast.StructLit); !ok {
		headComment += "\n" + footComment
		footComment = ""
	}
	// println(d.lastPos.Line, len(yn.HeadComment), yn.Line)
	if comments := d.comments(headComment); len(comments) > 0 {
	...
	if comments := d.comments(footComment); len(comments) > 0 {
		ast.AddComment(n, &ast.CommentGroup{
...
func (d *Decoder) sequence(yn *yaml.Node) (ast.Expr, error) {
	...
	if multiline && !noNewline {
		...
		// } else {
		// 	list.Rbrack = list.Rbrack.WithRel(token.Blank)
	}
	return list, nil
}
...
outer:
	for i := 0; i < l; i += 2 {
		...
		if yn := d.pendingMergeComments; yn != nil {
			d.pendingMergeComments = nil
			d.addComments(label, yn, 1)
		}
		field.Label = label
