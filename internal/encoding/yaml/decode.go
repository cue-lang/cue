...
outer:
	for i, mv := range values {
		...
		keyLineComments := d.nodeComments(mv.Key)
...
func (d *decoder) merge(yn gast.Node, m *ast.StructLit, multiline bool) error {
	...
	case *gast.AliasNode:
		// Guard against a merge key referencing its own mapping, like
		// alias() does for value aliases.
		name := n.Value.GetToken().Value
		if d.extractingAliases[name] {
			return d.posErrorf(n, "anchor %q value contains itself", name)
		}
		...
		err = d.merge(target, m, multiline)
