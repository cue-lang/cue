	...
	case *gast.MappingNode:
		...
		for i, mv := range n.Values {
			...
			if e.quotedKey != "" {
				...
				mv.Key = gast.String(gtoken.New(e.quotedKey, e.quotedKey, pos))
			}
			if len(e.head) > 0 {
...
// one token.
func singleToken(s string) (*gtoken.Token, bool) {
...
func isNumberTokenType(t gtoken.Type) bool {
	switch t {
...
func decodesAsNonString(s string) bool {
	...
	switch tk.Type {
	case gtoken.BoolType, gtoken.NullType, gtoken.ImplicitNullType,
		gtoken.InfinityType, gtoken.NanType, gtoken.MergeKeyType:
		return true
