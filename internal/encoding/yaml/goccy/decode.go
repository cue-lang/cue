...
// see https://github.com/goccy/go-yaml/issues/905. Tags are resolved
// through any %TAG directive handles, tracked here at the token level
// with the same per-document scope as the decoder, so that a shorthand
// for the merge tag is rewritten and a redefined "!!" handle is not.
//
// TODO: this rewrite could become unnecessary once the goccy issue
// above is fixed.
...
// end marker terminates the current segment. Splitting also preserves
// the empty documents between consecutive "---" headers, which goccy
// drops when parsing a whole stream at once.
...
func splitCommentRuns(cg *yast.CommentGroupNode) []commentRun {
	if cg == nil {
		return nil
	}
	var runs []commentRun
