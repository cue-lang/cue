...
// implementation based on CUE_EXPERIMENT=yamlv4.
package goccy
...
//
// TestUnmarshal now compiles and validates every decoded result, which catches
...
func NewDecoder(filename string, b []byte) *decoder {
	// goccy only handles UTF-8 input without a byte order mark, but YAML
	// also allows a UTF-8 BOM as well as UTF-16 with a BOM, so strip or
	// transcode such input first. All further positions refer to the
	// UTF-8 form with the mark removed.
	b = decodeBOM(b)
	...
	for i := 0; i < len(tokens); i++ {
		...
		case gtoken.TagType:
			...
			if isMergeTag && i+2 < len(tokens) &&
				tokens[i+2].Type == gtoken.MappingValueType {
				next := tokens[i+1]
...
// when the parse error involves such a comment we retry with comment
// tokens inside flow collections removed, and then with all comment
// tokens removed, dropping those comments much like yaml.v3 did.
//
// TODO: the retries could be removed if goccy learns to parse flow
// collections holding only comments.
...
func errorInvolvesFlowComment(toks gtoken.Tokens, err error) bool {
...
// decodeBOM strips a leading byte order mark from b, transcoding UTF-16
// content to UTF-8. Input without a BOM is returned as-is.
func decodeBOM(b []byte) []byte {
	...
	case bytes.HasPrefix(b, []byte{0xFF, 0xFE}), bytes.HasPrefix(b, []byte{0xFE, 0xFF}):
		...
		if err != nil {
			return b // let the lexer report an error on the raw bytes
		}
		return out
	}
	return b
...
func (d *decoder) registerAnchor(n *gast.AnchorNode, value gast.Node) {
	if d.anchors == nil {
...
func (d *decoder) insertMap(values []*gast.MappingValueNode, m *ast.StructLit, multiline, mergeValues bool, parentScopeEnd int, ownKeys map[string]bool) error {
...
func (d *decoder) merge(yn gast.Node, m *ast.StructLit, multiline bool, ownKeys map[string]bool) error {
	...
	case *gast.MappingValueNode:
		return d.insertMap([]*gast.MappingValueNode{n}, m, multiline, true, d.scopeEnd, ownKeys)
	case *gast.AliasNode:
...
func (d *decoder) label(yn gast.MapKeyNode) (ast.Label, error) {
	...
	// String keys may of course begin with a dash.
...
func (d *decoder) keyLabel(inner gast.Node, pos token.Pos) (ast.Label, error) {
	key, ok := inner.(gast.MapKeyNode)
	if !ok {
...
func (d *decoder) taggedLabel(n *gast.TagNode, pos token.Pos) (ast.Label, error) {
	expr, err := d.tagged(n)
	if err != nil {
		return nil, err
	}
...
// yamlStyleFloat matches the plain scalars that yaml.v3 resolved as floats,
// per resolve.go in go.yaml.in/yaml/v3.
...
// yamlV3SpecialFloats maps the plain scalar spellings that yaml.v3
// resolved as infinities and NaN to their decoded forms.
var yamlV3SpecialFloats = map[string]string{
...
// yamlV3NumberKind reports whether yaml.v3 resolved the plain scalar s as
// a number, returning token.INT, token.FLOAT, or token.ILLEGAL when s is
// not a number. It mirrors the resolver in go.yaml.in/yaml/v3's
// resolve.go, aside from the infinities and NaN of yamlV3SpecialFloats.
// In particular, we cannot use CUE's own number syntax to decide, as it
// is a superset of YAML's: for example, `1Gi` is a valid CUE literal but
// a string in YAML.
func yamlV3NumberKind(s string) token.Token {
	if s == "" {
	...
	case c == '+' || c == '-' || (c >= '0' && c <= '9'):
		// yaml.v3 stripped underscores before resolving numbers.
...
// that goccy lexes as strings but that yaml.v3 resolved as numbers, such
...
func (d *decoder) scalarString(n *gast.StringNode) (ast.Expr, error) {
	...
	if n.GetToken().Type == gtoken.StringType { // a plain, unquoted scalar
		if num, ok := yamlV3SpecialFloats[value]; ok {
			return d.makeNum(pos, num, token.FLOAT), nil
		}
		switch yamlV3NumberKind(value) {
		case token.INT:
		...
		case token.FLOAT:
			...
			// strings instead; that is what most YAML decoders do, even
			// though yaml.v3 resolved them as floats.
...
func (d *decoder) tagged(n *gast.TagNode) (ast.Expr, error) {
	...
	case "!!float":
		switch inner.(type) {
		case *gast.InfinityNode, *gast.NanNode:
