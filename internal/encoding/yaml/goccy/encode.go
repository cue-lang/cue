...
import (
	...
	yyaml "github.com/goccy/go-yaml"
	yast "github.com/goccy/go-yaml/ast"
	ylexer "github.com/goccy/go-yaml/lexer"
	yprinter "github.com/goccy/go-yaml/printer"
...
func Encode(n ast.Node, opts ...EncodeOption) (b []byte, err error) {
	...
	enc := yyaml.NewEncoder(nil,
		// Use idiomatic indentation.
		yyaml.Indent(2),
		yyaml.IndentSequence(cfg.indentSequence),
		// Prefer single quotes where goccy quotes on its own accord;
		// strings quoted deliberately by this package are pre-rendered
		// with double quotes.
		yyaml.UseSingleQuote(true),
		yyaml.UseLiteralStyleIfMultiline(true))
	node, err := enc.EncodeToNode(v)
	if err != nil {
		return nil, err
	}
	if seq, ok := node.(*yast.SequenceNode); ok && cfg.indentSequence {
	...
	var p yprinter.Printer
	out := p.PrintNode(node)
	// goccy pads the blank lines within literal block scalars to the
	// block's indentation. Render them as truly blank lines
	// instead. Lines of only spaces cannot
...
// MarshalYAML implements [yyaml.BytesMarshaler].
...
// tree: entries correspond one-to-one, in order, to mapping entries or
...
func applyInfo(node yast.Node, info *encInfo) yast.Node {
	if info == nil {
		return node
	}
	switch n := node.(type) {
	case *yast.MappingNode:
		...
		for i, mv := range n.Values {
			...
			if e.quotedKey != "" {
				...
				var pos *ytoken.Position
				if tk := mv.Key.GetToken(); tk != nil && tk.Position != nil {
					p := *tk.Position
					pos = &p
				}
				mv.Key = yast.String(ytoken.New(e.quotedKey, e.quotedKey, pos))
			}
			if len(e.head) > 0 {
				...
				if e.headGap {
					...
					if tk := mv.Key.GetToken(); tk != nil && tk.Position != nil {
						tk.Position.Line = 3
						tk.Prev = &ytoken.Token{Origin: "x", Position: &ytoken.Position{Line: 1}}
					}
				}
			}
			if len(e.foot) > 0 {
		...
		for i := range n.Values {
			...
			if len(e.head) > 0 {
				if len(n.ValueHeadComments) == 0 {
					n.ValueHeadComments = make([]*yast.CommentGroupNode, len(n.Values))
				}
	...
	if info.tag != "" {
		...
		var pos *ytoken.Position
		if tk := inner.GetToken(); tk != nil && tk.Position != nil {
			p := *tk.Position
			pos = &p
		}
		tn := yast.Tag(ytoken.New(tag, tag, pos))
		tn.Value = inner
		return tn
	}
	return node
...
func literalBlock(node yast.Node, value string, indent int) yast.Node {
	sn, ok := node.(*yast.StringNode)
	if !ok {
		return node
	}
	pos := func() *ytoken.Position {
...
// Note that this depends on goccy's printer deriving vertical gaps from
// the line delta between a token and its Prev token; the synthetic
// positions carry no other meaning.
func commentGroup(lines []commentLine, leadingBlank bool) *yast.CommentGroupNode {
	var toks []*ytoken.Token
	line := 2
	if leadingBlank {
		line++
	}
	prev := &ytoken.Token{Origin: "x", Position: &ytoken.Position{Line: 1}}
	for _, l := range lines {
		if l.gap {
			line++
		}
		tk := ytoken.New(l.text, l.text, &ytoken.Position{Line: line})
		tk.Prev = prev
		prev = tk
		line++
		toks = append(toks, tk)
	}
	return yast.CommentGroup(toks)
}
...
func quoteFlowUnsafe(v any, info *encInfo) any {
	...
	case yyaml.MapSlice:
		for i := range x {
			e := info.entry(i)
			if e != nil && e.quotedKey == "" {
				if k, ok := x[i].Key.(string); ok && strings.ContainsAny(k, flowUnsafe) {
					e.quotedKey = singleQuoted(k)
				}
			}
	...
	case []any:
		for i := range x {
			x[i] = quoteFlowUnsafe(x[i], info.entry(i))
		}
	}
	return v
}

// singleQuoted returns s as a YAML single-quoted scalar.
func singleQuoted(s string) string {
	return "'" + strings.ReplaceAll(s, "'", "''") + "'"
}

// needsSingleQuoting reports plain scalars that goccy leaves unquoted
// but that decoders read as something else entirely: the "?" explicit
// key indicator alone or followed by a space, and the "<<" merge key.
...
func encodeScalar(b *ast.BasicLit) (any, error) {
	...
	case token.STRING:
		...
		case needsSingleQuoting(str):
			return rawScalar(singleQuoted(str)), nil
...
func yamlIsNumber(s string) bool {
	if isDecimalNumber(s) {
		// The common case, avoiding the lexer.
		return true
	}
	tk, ok := singleToken(s)
	return ok && isNumberTokenType(tk.Type)
}

// isDecimalNumber matches simple decimal integers and floats such as
// 123 and 12.5, which YAML always reads back as numbers. Anything else,
// like exponents or other bases, is left to the lexer.
func isDecimalNumber(s string) bool {
	if len(s) > 1 && s[0] == '0' && s[1] != '.' {
		// A YAML 1.1 octal like 01; not produced by CUE, but do not
		// pass judgment on it here.
		return false
	}
	digits, dots := 0, 0
	for i := 0; i < len(s); i++ {
		...
		case c >= '0' && c <= '9':
			digits++
		case c == '.' && digits > 0 && dots == 0 && i < len(s)-1:
			dots++
		default:
			return false
		}
...
// one token.
func singleToken(s string) (*ytoken.Token, bool) {
	toks := ylexer.Tokenize(s)
	if len(toks) != 1 {
		return nil, false
	}
	return toks[0], true
...
// isNumberTokenType reports whether a goccy token type is one of the
// numeric scalar types.
func isNumberTokenType(t ytoken.Type) bool {
	switch t {
	case ytoken.IntegerType, ytoken.BinaryIntegerType, ytoken.OctetIntegerType,
		ytoken.HexIntegerType, ytoken.FloatType:
		return true
	}
	return false
}
...
// nonStringStarts holds the bytes that can start a plain scalar which
// resolves as a non-string value: numbers, infinities and NaN, nulls,
// the true/false booleans (YAML 1.1 words such as "yes" are handled by
// legacyStrings before this function is consulted), and merge keys.
const nonStringStarts = "0123456789+-.~<tTfFnN"
...
func decodesAsNonString(s string) bool {
	if s == "" || strings.IndexByte(nonStringStarts, s[0]) < 0 {
		// The common case, avoiding the lexer.
		return false
	}
	tk, ok := singleToken(s)
	...
	switch tk.Type {
	case ytoken.BoolType, ytoken.NullType, ytoken.ImplicitNullType,
		ytoken.InfinityType, ytoken.NanType, ytoken.MergeKeyType:
		return true
	case ytoken.StringType:
		if tk.Value != s {
		...
		// [decoder.scalarString]. Values shaped like broken YAML 1.1
		// octals, such as `0778`, need quoting as well: our decoder
		// reads them back as strings, but some YAML 1.1 decoders
		// resolve them as floats.
		if _, ok := specialFloats[s]; ok {
			return true
		}
		return numberKind(s) != token.ILLEGAL || rxAnyOctalYaml11().MatchString(s)
	default:
		return isNumberTokenType(tk.Type)
	}
}
...
func encodeDecls(decls []ast.Decl) (any, *encInfo, error) {
	...
	for _, d := range decls {
		...
		case *ast.CommentGroup:
			docForNext = commentLines(docForNext, x)
			continue
		...
		case *ast.Field:
			...
			if needsSingleQuoting(name) {
				valueInfo.quotedKey = singleQuoted(name)
			} else if shouldQuote(name) || strings.ContainsRune(name, '\n') {
			...
			m = append(m, yyaml.MapItem{Key: name, Value: value})
	...
	// comments, so that a blank line separates them from the entry
	// they follow.
