...
func Encode(n ast.Node, opts EncodeOptions) ([]byte, error) {
	v, info, err := encode(n)
	if err != nil {
		return nil, err
	}
	enc := yaml.NewEncoder(nil,
	...
	if seq, ok := node.(*yast.SequenceNode); ok && !opts.CompactSequences {
		// A document-level sequence has no mapping key to be indented
		// relative to.
		seq.AddColumn(-indentWidth)
	}
	node = applyInfo(node, info, 1, false)
	if len(info.line) > 0 {
...
// truly empty lines; see the caller for why this is safe.
func stripBlankLinePadding(b []byte) []byte {
	// By the same invariant, any line ending in a space is a padded
	// blank line; most documents have none.
...
type EncodeOptions struct {
	...
	// indentation as their enclosing mapping key rather than indented
	// relative to it.
...
// prependHead prepends lines to e's head comments, marking a blank
// line between the two runs when both are present.
func (e *encInfo) prependHead(lines []commentLine) {
	if len(lines) == 0 {
		return
	}
	if len(e.head) > 0 {
...
func applyInfo(node yast.Node, info *encInfo, col int, inFlow bool) yast.Node {
	...
	case *yast.SequenceNode:
		...
		for i := range n.Values {
			...
			// Printing a block sequence re-indents an element's block
			// content to sit past the sequence start; the origin must
			// match for that to round-trip.
			startCol := 0
...
func literalBlock(node yast.Node, value string, indent int) yast.Node {
	...
	lit := yast.Literal(ytoken.New("|-", "|-", clonePos(sn)))
	// The literal node renders as its start token ("|-") followed by
	// the value token's origin, which must carry the indentation.
	vtok := ytoken.New(value, strings.Repeat(" ", indent)+value, clonePos(sn))
	lit.Value = yast.String(vtok)
	return lit
}

// gapAnchor returns a synthetic predecessor token: goccy's printer
// renders a blank line above a token whose line is more than one past
// its Prev token's. The anchor carries no other meaning.
func gapAnchor() *ytoken.Token {
	return &ytoken.Token{Origin: "x", Position: &ytoken.Position{Line: 1}}
}
...
// synthetic line deltas; see [gapAnchor].
...
func quoteFlowUnsafe(v any, info *encInfo) any {
	...
	case yaml.MapSlice:
		for i := range x {
			...
			if e != nil && e.flow {
				// A nested single-line collection was already quoted
				// by its own pass.
				continue
			}
...
// in YAML 1.1.
//
// These strings are always double quoted upon export to allow for
// backward compatibility with YAML 1.1 parsers.
...
func encodeExprs(exprs []ast.Expr) ([]any, *encInfo, error) {
	info := &encInfo{entries: make([]*encInfo, 0, len(exprs))}
...
func encodeDecls(decls []ast.Decl) (any, *encInfo, error) {
	...
	for _, d := range decls {
		...
		case *ast.Field:
			...
			addDocs(x, valueInfo)
			if len(docForNext) > 0 && len(valueInfo.head) == 0 {
				// Standalone comment groups are separated from the
				// field below them by a blank line.
				valueInfo.headGap = true
			}
			valueInfo.prependHead(docForNext)
			docForNext = nil

			m = append(m, yaml.MapItem{Key: name, Value: value})
