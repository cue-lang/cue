...
func ComposeFile(filename string, docs []ast.Expr) *ast.File {
	f := &ast.File{Filename: filename}
	switch len(docs) {
	case 0:
	case 1:
		switch x := docs[0].(type) {
		case *ast.StructLit:
			f.Decls = x.Elts
		default:
	...
	default:
		f.Decls = []ast.Decl{&ast.EmbedDecl{Expr: &ast.ListLit{Elts: docs}}}
	}
	return f
}
...
func ExtractLenient(filename string, b []byte) (*ast.File, error) {
	...
	for {
		expr, err := d.Decode()
		if err != nil {
			// Decode never returns an expression alongside an error,
			// and in lenient mode the only error is io.EOF; guard
