...
// pathComponents splits the name path into a slice of directory names (which
// may be empty), and the final basename.
...
//
// Note if an element in as has the same name as an element in bs,
// then the element in as is selected in preference, and the entry in
// bs is ignored.
...
// Close implements [iofs.File]
func (handle *combinedDirHandle) Close() error {
	var errDelegate, errOverlay error
	if delegateHandle := handle.delegateHandle; delegateHandle != nil {
		handle.delegateHandle = nil
		errDelegate = delegateHandle.Close()
	}
	if overlayHandle := handle.overlayHandle; overlayHandle != nil {
		handle.overlayHandle = nil
...
func (handle *combinedDirHandle) ReadDir(n int) ([]iofs.DirEntry, error) {
	...
	delegateHandle := handle.delegateHandle
