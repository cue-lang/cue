...
// is dirty. Returns true iff the module ends up in a splendid status.
...
// FindPackagesOrModulesForFile searches both the module itself, and
// packages within the module, returning a (possibly empty) list of
// [packageOrModule]s to which the file belongs. The file must be
// enclosed by the module's rootURI.
//
// The file will belong to the module if the file the module's modFileURI.
//
...
// here. For example, if file is a/b.cue, and it contains "package x",
// and a package a/c:x within the same module is already loaded, then
// the results will contain both package a:x and a/c:x, but only if
// a/c:x is already loaded.
...
func (m *Module) ReloadPackages() error {
	...
	for _, loadedPkg := range loadedPkgs.All() {
		...
		ip := m.NormaliseImportPath(loadedPkg.ImportPath())
	...
	for pkg, oldPkg := range pkgsImportsWorklist {
		...
		if oldPkg != nil {
			for _, i := range oldPkg.Imports() {
				imports[m.NormaliseImportPath(i.ImportPath())] = struct{}{}
			}
		}
		for _, i := range pkg.pkg.Imports() {
			ip := m.NormaliseImportPath(i.ImportPath())
...
// NormaliseImportPath is used to normalise and canonicalise import
...
// major versions.
func (m *Module) NormaliseImportPath(importPath string) ast.ImportPath {
	if valid := m.ReloadModule(); !valid {
