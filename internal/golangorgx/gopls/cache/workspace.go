...
// Workspace corresponds to an LSP Workspace.
...
func NewWorkspace(cache *Cache, debugLog func(string)) *Workspace {
	index := atomic.AddInt64(&workspaceIDCounter, 1)
	return &Workspace{
...
func (w *Workspace) String() string { return w.id }

func (w *Workspace) EnsureFolder(fetchFolderOptions func(dir protocol.DocumentURI) (*settings.Options, error), dir protocol.DocumentURI, name string) error {
	...
	folder, err := NewWorkspaceFolder(fetchFolderOptions, dir, name)
	if err != nil {
		return err
	}
...
func (w *Workspace) RemoveFolder(dir protocol.DocumentURI) {
	for i, wf := range w.folders {
		if wf.dir == dir {
			w.folders = slices.Delete(w.folders, i, i+1)
...
func (w *Workspace) UpdateFolderOptions(fetchFolderOptions func(folder protocol.DocumentURI) (*settings.Options, error)) error {
	for _, wf := range w.folders {
		if err := wf.UpdateOptions(fetchFolderOptions); err != nil {
			return err
		}
	}
	return nil
...
func (w *Workspace) FileWatchingGlobPatterns(ctx context.Context) map[protocol.RelativePattern]struct{} {
	patterns := map[protocol.RelativePattern]struct{}{
		// Note that we use **/module.cue and not
		// **/cue.mod/module.cue. This is because it is possible that a
		// valid module.cue is within the lsp workspace, but the cue.mod
		// directory is not.
		{Pattern: "**/module.cue"}: {},
	}
...
// files loaded by each package that we've loaded. Files that the
// editor/client have opened will almost certainly be active files
// (provided we've found their package and module), but the inverse
// doesn't hold: it's not the case that all active files are open in
// the editor/client.
//
...
func (w *Workspace) DidModifyFiles(ctx context.Context, modifications []file.Modification) error {
	...
	for uri, fh := range updatedFiles {
		...
		// nil) and we fail to find an existing package who's "leaf"
...
func (w *Workspace) FindModuleForFile(file protocol.DocumentURI) (*Module, error) {
	...
	for ; fileDir != "file:///"; fileDir = fileDir.Dir() {
		...
		return w.NewModule(fh)
	}

	return nil, nil
}

func (w *Workspace) NewModule(fh fscache.FileHandle) (*Module, error) {
	m, err := NewModule(fh, w.registry, w.overlayFS, w.debugLog)
	if err != nil {
		return nil, err
	}
