...
func autoNetworkAddressPosix(cuelspPath, id string) (network string, address string) {
	...
	// rebuilt.
	h := sha256.New()
	cmd := exec.Command("go", "tool", "buildid", cuelspPath)
	cmd.Stdout = h
	var pathHash []byte
	if err := cmd.Run(); err == nil {
		...
		sum := sha256.Sum256([]byte(cuelspPath))
		pathHash = sum[:]
	}
	...
	basename := filepath.Base(cuelspPath)
	idComponent := ""
	if id != "" {
		idComponent = "-" + id
	}
