...
func (s *server) EventuallyShowMessage(ctx context.Context, msg *protocol.ShowMessageParams) error {
	...
	if pending == nil {
		...
		s.pendingMessages = pending
	}
	pending.Add(msg)
	return nil
}

func (s *server) MaybeShowPendingMessages(ctx context.Context) {
	if s.state != serverInitialized {
		return
	}
	pending := s.pendingMessages
...
func (s *server) EventuallyUseWorkspaceFolders(folders []protocol.WorkspaceFolder) {
	if s.state != serverInitializing {
		panic("Must not be called except when server is initializing")
	}

	s.pendingFolders = &pendingFolders{folders: folders}
}
...
func (s *server) MaybeUseWorkspaceFolders(ctx context.Context) error {
	...
	if pending := s.pendingFolders; pending != nil {
...
func (pm *pendingMessages) Add(msg *protocol.ShowMessageParams) {
