...
const maxLoad = 0.75 // 75% load factor before resizing
...
func New[T Int](capacity int) *Set[T] {
	...
	return &Set[T]{
		...
		gen:         1,
		maxLoad:     int(float64(capacity) * maxLoad),
	}
}
...
func (s *Set[T]) rehash(newCap int) {
	...
	s.gen = 1
	s.size = 0
	s.maxLoad = int(float64(newCap) * maxLoad)
...
func nextPow2(v int) int {
	...
	return 1 << (32 - bits.LeadingZeros(uint(v)))
}
