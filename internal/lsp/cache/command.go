...
func (w *Workspace) CommandCueHubEval(ctx context.Context, uri protocol.DocumentURI, withServerLocked func(func())) error {
	f := w.GetFile(uri)
	if f == nil || f.cueHub == nil {
		return nil
	}
	...
	go withServerLocked(func() {
		w.debugLog("cuehub: requesting evaluation")
		if _, err := f.cueHub.RequestEvaluation(eval); err != nil {
