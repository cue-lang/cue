...
func (w *Workspace) CommandExternalValidate(ctx context.Context, uri protocol.DocumentURI) error {
	...
	w.enqueue(func() {
		...
		if err := f.extValidator.StartValidation(validation); err != nil {
...
// Result implements [extvalidator.ResponseHandler]
func (v *externalValidation) Result(resultMsg *extproto.EvalResultMsg) {
	w := v.workspace
	w.enqueue(func() {
...
// Finished implements [extvalidator.ResponseHandler]
func (v *externalValidation) Finished(*extproto.EvalFinishedMsg) {
}

// Clear implements [extvalidator.ResponseHandler]
...
func (w *Workspace) extValidatorOnDirtyChanged(*extvalidator.Validator) {
	w.enqueue(func() {
