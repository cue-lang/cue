...
func (f *standaloneFile) reload() error {
	...
	if syntax == nil {
		// The file's content cannot be parsed at all. Keep the
		// standalone file, with its error, so that the error is
		// published as a diagnostic and any previously published
		// diagnostics are updated; but there is no evaluator for it.
		f.definitions = nil
		w.debugLogf("%v Error when reloading: %v", f, err)
		return ErrBadFile
	}
	if extm := w.extValidatorMgr; extm != nil {
		extv := extm.EnsureValidator(f.uri, w.extValidatorOnDirtyChanged)
