...
type Workspace struct {
	...
	// non-blocking.
	enqueue         func(func())
	extValidatorMgr *extvalidator.ExtValidatorManager
}
...
func NewWorkspace(cache *Cache, client protocol.Client, debugLog func(string), enqueue func(func())) *Workspace {
	...
	if extProfile := cache.extProfile; extProfile != nil {
		w.extValidatorMgr = extvalidator.NewExtValidatorManager(extProfile, overlayFS, debugLog)
	}
	return w
}

func (w *Workspace) debugLogf(format string, args ...any) {
	w.debugLog(fmt.Sprintf(format, args...))
}
...
func (w *Workspace) EnsureFolder(dir protocol.DocumentURI, name string) (*WorkspaceFolder, error) {
	...
	w.debugLogf("Workspace folder added: %v", dir)
	return folder, nil
}
...
func (w *Workspace) RemoveFolder(dir protocol.DocumentURI) {
	w.folders = slices.DeleteFunc(w.folders, func(wf *WorkspaceFolder) bool {
		if wf.dir == dir {
			w.debugLogf("Workspace folder removed: %v", dir)
		}
		return wf.dir == dir
	})
}
...
func (w *Workspace) reloadPackages() {
	...
	// processedPkgs contains a key for every package we have attempted
	// to load, and we use this to prevent repeated loads of the same
	// package. Packages that failed to load due to some error still
	// have keys in this map (but a nil value).
	...
	for {
		...
		// Process the results of loading all the dirty packages from
		// all the modules. We need to do this in two passes to ensure
		// we create all necessary packages before trying to
		...
		// Once a package is marked dirty, it eventually gets
		// reloaded. modpkgload will reload its import graph, which
		// covers everything upstream. Nothing downstream needs to be
			...
			if pkg.isCue {
				...
				for _, embedding := range pkg.embeddings {
					for _, embedded := range embedding.results {
						...
						key.importPath = *ip
						if _, loaded := processedPkgs[key]; loaded {
							// If we've just loaded the embedded pkg,
							// assuming the load succeeded, the embedded pkg
							// won't be dirty, so nothing to do here.
							continue
						}
				...
				if len(pkg.dirURIs) != 1 {
					// old module system: ancestor imports not supported.
					continue
				}
				...
				for ascendantPkg := range m.ascendantCuePackages(pkg) {
					...
					if _, loaded := processedPkgs[key]; loaded {
						// If we've just loaded it, it'll find pkg in the
						// call to linkWithEmbeddedFiles that happens in a
						// moment.
						continue
					}
