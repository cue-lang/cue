...
func addFS(w *zip.Writer, fs *fscache.OverlayFS, rootURI protocol.DocumentURI) (map[protocol.DocumentURI]int32, error) {
	...
	err = iofs.WalkDir(fsys, ".", func(name string, d iofs.DirEntry, err error) error {
		...
		// Exclude '.' (hidden) files and directories, *apart* from the
		// .cuehub directory. TODO: it might be better to just ignore
		// untracked files/directories.
		if leaf := path.Base(name); strings.HasPrefix(leaf, ".") && leaf != ".cuehub" {
			if d.IsDir() {
				return iofs.SkipDir
			} else {
				return nil
