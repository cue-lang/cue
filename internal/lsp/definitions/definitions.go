...
func (n *astNode) eval() {
	...
	for len(unprocessed) > 0 {
		...
		case *ast.Package:
			...
			// imports in some other package.
			n.dfns.pkgNode.newAstNode(node, nil, n.dfns.pkgDecls)
		...
		case *ast.ImportSpec:
			...
			// avoid the possibility that evaluating a file astNode would
			// lookup every imported package and evaluate its file nodes.
			if n.isFileNode() {
				// 1) At the filenode level, where we create appropriate
				// file-scope bindings, but also pass the spec as the
				// unprocessed value to the child node;
				if node.Name == nil {
				...
				// Eval the pkgNode and its immediate children only. This
				// is enough to ensure the package declarations have been
				// found and added to the pkgDecls.
				dfns.pkgNode.eval()
