...
// this node represents. The unprocessed value is the [ast.Node] which
// is to be processed by the new astNode. The navigableBindings are
...
type astNode struct {
	...
	parent *astNode
	// unprocessed holds the nodes that make up this astNode. Once a
	// call to [node.eval] has returned, unprocessed must never be
	// modified.
...
// start file-offset to its end file-offset.
...
func (n *astNode) resolvePathRoot(name string) *navigableBindings {
	...
	for ; n != nil; n = n.parent {
		if bindings, found := n.bindings[name]; found {
			...
			// an ident and not a basiclit. But there's that ident can
			// come from any of the (potentially many) matching bindings!
			identFound := false
