...
//     navigable binding;
//  2. we record in that navigable binding the field usage node.
//
// For example:
//
//	x: y
...
// this packgae, or import this package) have been modified but this
...
func (dfns *Definitions) Reset() {
	...
	for _, fdfns := range dfns.byFilename {
		...
		clear(fdfns.unknownIdentUsageOffsets)
...
type FileDefinitions struct {
	...
	File *ast.File
	// unknownIdentUsageOffsets contains file offsets for idents that
	// are likely to be references (as opposed to declarations) and
	// likely to reference imported packages. It is lazily populated by
	// [FileDefinitions.findIdentUsageOffsets].
	unknownIdentUsageOffsets map[string][]int
}
...
func (fdfns *FileDefinitions) findIdentUsageOffsets(name string) []int {
	unknownIdents := fdfns.unknownIdentUsageOffsets
	if unknownIdents == nil {
		unknownIdents = make(map[string][]int)
		fdfns.unknownIdentUsageOffsets = unknownIdents
	}
