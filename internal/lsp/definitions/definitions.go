...
// gathering their names, and stopping when either the root is
// reached, or the navigable binding has no name. From this oldest
// ancestor, the calculated path is then resovled using the normal
...
// example, both 6 and 7 are replaced with the path x.y.
package definitions
...
type navigableBindings struct {
	...
	// because navigableBinding are used for resolving
...
// fieldDeclExpr is created so that it field declarations keys can be
// resolved. Their position is always the position of the key, and
// their expression is always nil.
