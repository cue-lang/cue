...
func Analyse(ip ast.ImportPath, forPackage DefinitionsForPackageFunc, files ...*ast.File) *Definitions {
	if forPackage == nil {
...
func (fdfns *FileDefinitions) UsagesForOffset(offset int, importedBy func(ast.ImportPath) []*Definitions) []ast.Node {
	fdfns.dfns.bootFiles()
	definitions := fdfns.definitions
	navs, found := definitions[offset]
	if !found {
		...
		if nav == nav.dfns.pkgDecls && nav.dfns != fdfns.dfns {
			...
			for n, node := range nav.usedBy {
				if _, ok := n.(*ast.ImportSpec); ok && node.fdfns == fdfns {
					...
					fmt.Printf("switched initial nav to %p\n", navs[0])
					break
				}
			}
		}
...
func usages(navsWorklist []*navigableBindings, importedBy func(ast.ImportPath) []*Definitions) {
	...
	for len(navsWorklist) > 0 {
		...
		var evalWorklist []*astNode
		{
			...
			for parent := nav; parent != nil; child, parent = parent, parent.parent {
				fmt.Printf(" building path %p %p\n", parent, parent.dfns.pkgDecls)
		...
		if isExported {
			ip := nav.dfns.ip
			for _, dfns := range importedBy(ip) {
				navsWorklist = append(navsWorklist, dfns.initialNavsForImportSpec(ip)...)
			}
		}
	}
}

func (dfns *Definitions) initialNavsForImportSpec(ip ast.ImportPath) []*navigableBindings {
	dfns.bootFiles()
	nav, found := dfns.importSpecNodes[ip]
	fmt.Println("  ", ip, dfns.importSpecNodes, found)
	if !found {
		return nil
	}
	...
	for len(unprocessed) > 0 {
		...
		case *ast.ImportSpec:
			...
			if n.isFileNode() {
				...
				if err == nil {
					remotePkgDfns = n.fdfns.dfns.forPackage(str)
				...
				if remotePkgDfns == nil {
					...
					remotePkgDfns = Analyse(ast.ImportPath{}, nil)
				}
