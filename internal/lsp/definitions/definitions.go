...
// purpose of this evaluator is to be able to calculate what each
// element of each path resolves to; there is no calculation of
// fixed-points, no subsumption, no unification. And the little that
// this evaluator does do is imprecise. For example, I do not test
// field names (even when known) against patterns. I do not compute
// the names of dynamic fields, even when it is trivial to do
// statically.
//
...
// [ast.StructLit] for example contains a number of [ast.Decl]
// nodes. When an [ast.Field] is encountered, ensure that within the
// current scope, a binding exists for the field's name, and add the
// field's value to binding's scope's unprocessed values. Note that
// evaluation of a scope is not recursive: I do not, as a matter of
// course, evaluate a scope's bindings.
//
...
// resolve paths, and record that this scope itself resolves to these
// other scopes.
//
...
// bindings, then it is guaranteed that binding contains its complete
// set of nodes before it is accessed, and so it is safe to evaluate.
//
...
// [ast.StructLit]).  If scope for y is evaluated, it will create its
// own bindings for a (containing the [ast.BasicLit] 3), and for b
...
// x.a.
//
// In conclusion: this algorithm uses the fact that I can traverse the
...
// caret is on, nor anything about the current scope. The LSP client
// knows only the caret's line and column number.
//
...
// all over several different files in the same direction. I evaluate
// and traverse into a scope if *any* of its node's ranges contain the
// file-coordinate in question. Thus I am not solely evaluating nodes
// that are lexically ancestors of the file-coordinate. Instead, I am
// evaluating all nodes that contribute to every scope that contains
// any node that is a lexical ancestor of the file-coordinate.
//
// # Algorithm 2: real CUE
//
// If I stuck to algorithm 1, it would mean that:
//
//	a₁: b: c: a₂
//	a₃: b: a₄: 5
//
...
//	x₂: y₂: 7
//
// Here, the outermost scope has two lexical bindings for x, and each
// binding has a distinct scope. But both of those scopes share a
// "navigable scope" struct and so any children that each of the two
// bindings for x have, can be merged together correctly, as
// before. Thus in this example, the outermost scope has two lexical
// bindings for x; their distinct scopes each have one binding for
// their y fields; the scopes for the two x bindings share a
// "navigable scope", and their individual bindings for y both appear
// as navigable bindings within that shared "navigable scope".
//
...
// switch to the navigable bindings for the rest.
//
// Importantly, when querying via a file-coordinate, the ranges remain
// grouped together: the rangeset is part of the "navigable scope" and
// so I still ensure that I evaluate everything that contributes to
// the navigable scopes when traversing the AST.
//
...
// same package, and it can still be resolved.
package definitions
