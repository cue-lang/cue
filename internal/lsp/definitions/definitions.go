...
func Analyse(ip ast.ImportPath, importCanonicalisation map[string]ast.ImportPath, forPackage DefinitionsForPackageFunc, pkgImporters func() []*Definitions, files ...*ast.File) *Definitions {
	if forPackage == nil {
...
func usages(navsWorklist []*navigableBindings) {
	...
	for len(navsWorklist) > 0 {
		...
		if isExported {
			fmt.Println("Exported", nav)
			for _, dfns := range nav.dfns.pkgImporters() {
				navsWorklist = append(navsWorklist, dfns.initialNavsForImportSpec(nav.dfns.ip)...)
			}
		}
	}
}
...
func (dfns *Definitions) parseImportSpec(spec *ast.ImportSpec) *ast.ImportPath {
	...
	fmt.Println("REALLY Failed to find", str, "in", dfns.importCanonicalisation, "; we are", dfns.ip)
	return &ip
}
