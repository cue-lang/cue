...
`[1:]
	...
	// str = `a: n=(2 * (div(n, 2))) | error("\(n) is not even")`
	// pos = ln(1, 1, "n")
	...
	dfns := definitions.Analyse(ast.ImportPath{}, nil, fileAst)
	...
	if false {
		...
		t.Log()
		t.Log("-- 1 --")
		t.Log(fdfns.UsagesForOffset(pos.offset, nil))
	...
	for i, n := range fdfns.UsagesForOffset(pos.offset, nil) {
...
`,
			...
			importedBy: func(ip ast.ImportPath, dfns map[string]*definitions.Definitions) []*definitions.Definitions {
				if ip.Qualifier == "a" {
					fmt.Println("HELLO!", ip.Qualifier, dfns)
					return []*definitions.Definitions{dfns["b"]} // Read: "Pkg a is imported by pkg b"
				}
				return nil
			},
		},
...
type testCase struct {
	...
	expectDefinitions map[position][]position
	expectCompletions map[position]fieldEmbedCompletions
	importedBy        func(ast.ImportPath, map[string]*definitions.Definitions) []*definitions.Definitions
}
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			analyse := func() testCaseAnalysis {
				...
				for pkgName, files := range filesByPkg {
					...
					dfns := definitions.Analyse(ip, forPackage, files...)
...
func (tc *testCase) testUsages(t *testing.T, files []*ast.File, analysis testCaseAnalysis) {
	t.Run("usages", func(t *testing.T) {
		...
		for dfn, uses := range tc.expectDefinitions {
			if slices.Contains(uses, self) || dfn.str[0] == '[' { // skip dynamic indexing for usages, and parens
				continue
			}
		...
		importedBy := func(ip ast.ImportPath) []*definitions.Definitions {
			fun := tc.importedBy
			if fun == nil {
				return nil
			}
			return fun(ip, analysis.dfnsByPkgName)
		}
		...
		for posUse, positionsWant := range expectUsages {
			...
			for i := range len(posUse.str) {
				...
				offset := offset + i
				nodesGot := fdfns.UsagesForOffset(offset, importedBy)
				fileOffsetsGot := make([]fileOffset, len(nodesGot))
				for j, node := range nodesGot {
					fileOffsetsGot[j] = fileOffsetForTokenPos(node.Pos().Position())
				}
