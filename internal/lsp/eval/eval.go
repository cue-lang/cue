		...
		for _, embed := range embeds {
			...
			if embed.IsGlob() {
				...
				for filename := range e.byFilename {
					...
					fmt.Println("embedders fakefieldName", fieldName, remoteDir, remoteFilename)
				...
				for _, remotePkgEvaluator := range remoteEvals {
					...
					if isGlob {
						...
						for remoteFilename := range remotePkgEvaluator.byFilename {
							fieldName := strings.TrimPrefix(filepath.ToSlash(remoteFilename), dir)
							fmt.Println("fakeFieldName", fieldName, dir, f.fileEvaluator.File.Filename)
