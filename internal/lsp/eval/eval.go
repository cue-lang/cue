...
// later on).
//
// If, during evaluation, a frame encounters a path, the path will
// correspond either to the value of a field (i.e. the frame is for
// something like x: y), or an embedding into a struct. The frame
// keeps track of these embedded paths and once processing of the
// frame's values is complete, it then resolves the embedded paths to
// further frames, and records that this frame itself resolves to
// these other frames (the resolvesTo field).
//
// The consequence is that the evaluation of a frame creates and fully
// populates (with their unprocessed values) all of its bindings
...
// a binding for a. It doesn't, but we also inspect all the frame that
// this frame resolves to. There is one resolved-to frame, and it does
// contain a binding for a, so we grab that. This completes the
// resolution of x.a.
//
...
//	y: 3
//
// The y on line 1 will resolve to the navigable that represent the y
...
// resolve to themselves. Having found the correct navigable, we now
// need to force the evaluation of every frame that could possibly
// make use of this navigable; evaluating a frame is enough to add
// entries to the relevant navigable's usedBy field.
//
...
// every frame that that contributes to the definition's navigable,
// all of the descendants of those frames, all of the ancestors of
// those frames, and also those ancestors' descendants. In other
// words, everything. This could get expensive, so there are a couple
// of ways we can trim evaluation.
//
//	x: {h: 3, y: 4}.y
//
// If the user is asking for the usages of h, we evaluate everything
// within the in-line struct lit, but we can detect that h is not used
// by x and so we do not need to evaluate anything further up the
// tree.
//
//	x: {i: h: 3, y: 4}.i
//
...
func New(ip ast.ImportPath, importCanonicalisation map[string]ast.ImportPath, forPackage EvaluatorForPackageFunc, pkgImporters ImportersFunc, files ...*ast.File) *Evaluator {
	...
	for _, file := range files {
		...
		fileFrame := fe.newFrame(pkgFrame, file, fileFramesNavigable)
		pkgFrame.childFrames = append(pkgFrame.childFrames, fileFrame)
		...
		fileFrame.end = tokFile.Pos(tokFile.Size()+1, token.NoRelPos)
	}
...
func (e *Evaluator) Reset() {
	...
	for _, fe := range e.byFilename {
		...
		fileFrame := fe.newFrame(pkgFrame, fe.File, fileFramesNavigable)
		pkgFrame.childFrames = append(pkgFrame.childFrames, fileFrame)
...
func (e *Evaluator) bootFiles() {
	...
	pkgFrame := e.pkgFrame
	pkgFrame.eval()
	for _, child := range pkgFrame.childFrames {
		child.eval()
	}
}
...
// as little work as possible. The provided navs are be modified
...
func usages(navsWorklist []*navigable) {
	...
	targetsSeen := make(map[*navigable]struct{})
	evaluatedNavs := make(map[*navigable]struct{})
	evaluatedFrames := make(map[*frame]struct{})
	...
	for len(navsWorklist) > 0 {
		...
		// too.
		isExported := false
		var pathElements []*navigable
		var evalWorklist []*frame
		pkgNav := nav.evaluator.pkgFrame.navigable
		{
			...
			for parent := nav; parent != nil; child, parent = parent, parent.parent {
				...
				// This might look like it's in the wrong place: we've
				// committed to evaluating all of the parent.frames, and
				// parent is appended to pathElements, but we might exit
				// at the break above without recording parent in
				// evaluatedNavs. This is correct: if we didn't have the
				// evaluatedNavs set at all, then if we found one of
				// parent's descendents in navsWorklist then we would
				// again walk up its parents and stop at the same
				// parent. But, if we find parent itself in the
				// navsWorklist then we would construct different
				// pathElements, and a different evalWorklist - we would
				// not exit this inner loop at the same point (because
				// child would still be nil). So for this reason, we do
				// not record the "final" parent in evaluatedNavs.
		...
		for len(evalWorklist) > 0 {
			...
			if _, seen := evaluatedFrames[fr]; seen {
				continue
			}
			evaluatedFrames[fr] = struct{}{}

			fr.eval()
			evalWorklist = append(evalWorklist, fr.childFrames...)
		}

		// We must evaluate every frame (and consider their ancestors
		// for evaluation) that uses any navigable binding in
		// pathElements.
		//
		//	x: {i: h: j: 3, k: i}.k.h
		//
	...
	for len(worklist) > 0 {
		...
		for i := 0; i < len(nav.frames); i++ {
			...
			if spec, ok := fr.node.(*ast.ImportSpec); ok {
				...
				pkgFrame.eval()
				for _, fr := range pkgFrame.childFrames {
...
func (f *frame) eval() {
	...
	for len(unprocessed) > 0 {
		...
		case *ast.Field:
			...
			if keyName != "" {
				childNav = f.ensureNavigable(keyName)
			}
...
// navigables.
func (f *frame) ensureNavigable(name string) *navigable {
	nav := f.navigable
...
func (f *frame) createPath(expr ast.Expr, receiver *navigable) {
	...
	for component := expr; component != nil; {
		...
		case *ast.SelectorExpr:
			...
			sel := curExpr.Sel
			start := sel.Pos()
			if curExpr.Dot.IsValid() {
				nextEnd = curExpr.Dot
				start = curExpr.Dot.Add(1)
			}
