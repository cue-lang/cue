...
func NewExtValidatorManager(profile *validatorconfig.Profile, fs *fscache.OverlayFS, debugLog func(msg string)) *ExtValidatorManager {
...
type EvalResponseHandler interface {
	// EvalResult is called for each evaluation result received from
	// the server. NB this may never be called if a subsequent new
	// evaluation request is made before any results are received.
	EvalResult(*extproto.EvalResultMsg)
	// EvalFinished is called once all evaluation results have been
	// received by the server. NB this may never be called if a new
	// evaluation request is made before the server indicates it has
	// sent all the results.
	EvalFinished(*extproto.EvalFinishedMsg)
	// Clear is called when this evaluation request is replaced with a
	// new evaluation request. This will always be called, regardless
