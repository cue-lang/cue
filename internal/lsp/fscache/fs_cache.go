...
// content contains syntax errors, the parser usually still produces
// a partial AST, which is returned along with the error: a partial
// AST with comments is far more useful to the LSP than no AST at
// all, so there is deliberately no fallback to a more restrictive
// parse mode.
//
...
func (p *cueFileParser) ReadCUE(config parser.Config) (syntax *ast.File, cfg parser.Config, err error) {
	...
	if syntax != nil {
		...
		// NB this must come after any phantom package injection: for
		// a file with no declarations, syntax.Pos() is only stable
		// once the phantom package declaration anchors it.
...
func readFile(uri protocol.DocumentURI, mtime time.Time) (*diskFileEntry, error) {
	...
	// Otherwise, throw the error away: the nil buildFile records
	// that this uri can never be converted to a CUE AST. The content
	// is still available via [FileHandle.Content], e.g. for
	// inclusion in external validator snapshots.
