...
// nodes with the original; they carry no position information, no
// comments other than doc comments, and no resolution back-references
// ([ast.Ident.Scope] and [ast.Ident.Node]). This ensures the pretty
// printer uses only its width-driven algorithm for layout.
type copier struct {
	r *renderer
...
func (c copier) value(owner reflect.Type, v reflect.Value) reflect.Value {
	if c.r.overBudget() {
		// The result will be discarded (see [ValueForOffset]), so the
		// original, un-copied value unwinds the recursion cheaply
		// without (ultimately) leaking anywhere.
		return v
	}
	switch v.Kind() {
	...
	case reflect.Struct:
		...
		for i := range t.NumField() {
			...
			if t == identType && (f.Name == "Scope" || f.Name == "Node") {
				// An ident's resolution. These can point back to the
				// input AST; our copy must not retain them.
				continue
			}
...
// value as `z: 3` then keeps the comment with the field it documents,
// while a comment documenting a label that the rendering omits is
...
func (c copier) docComments(n ast.Node) []*ast.CommentGroup {
	...
	for _, cg := range ast.DocComments(n) {
		...
		// The comment may have been inherited from a field-chain head,
		// where it was not necessarily a Doc group of the leaf itself;
		// on the copy it must be one for the printer to place it above
		// the node.
		cg.Doc = true
		docs = append(docs, cg)
	}
