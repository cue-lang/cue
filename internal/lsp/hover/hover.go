...
// declarations, across all of the given nodes; except that the
// declaration containing the cursor comes last: the user can already
// see it, making it the least informative part of the result.
...
// inlineExpr returns a copy of d's value with every reference
// throughout it resolved inline, recursively.
func (r *renderer) inlineExpr(d *eval.Decl) ast.Expr {
	expr := d.Value().(ast.Expr)
