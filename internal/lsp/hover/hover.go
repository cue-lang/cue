...
func (r *renderer) renderNodes(ns eval.NodeSet) ast.Expr {
	...
	for _, d := range decls {
		...
		for _, operand := range appendConjunctOperands(nil, d.Value().(ast.Expr)) {
...
func appendConjunctOperands(acc []ast.Expr, expr ast.Expr) []ast.Expr {
	if bin, isBin := expr.(*ast.BinaryExpr); isBin && bin.Op == token.AND {
		return appendConjunctOperands(appendConjunctOperands(acc, bin.X), bin.Y)
	}
	return append(acc, expr)
}
