...
type Registry interface {
	...
	modpkgload.Registry
	ModuleVersions(context.Context, string) ([]string, error)
}
...
// registry to download requirements and returns a resolved module file.
func Load(ctx context.Context, fsys fs.FS, modRoot string, reg Registry) (*modfile.File, error) {
	...
	log.Printf("after tidying, requirements: %q", rs.RootModules())
	mf1, err := modfileFromRequirements(mf, rs)
	if err != nil {
		return nil, err
	}
	return mf1, err
...
func modfileFromRequirements(old *modfile.File, rs *modrequirements.Requirements) (*modfile.File, error) {
	mf := &modfile.File{
		Module: old.Module,
...
func (ld *loader) resolveDependencies(ctx context.Context, rootPkgPaths []string, rs *modrequirements.Requirements) (*modrequirements.Requirements, *modpkgload.Packages, error) {
	for {
		log.Printf("---- LOADING from requirements %q", rs.RootModules())
		...
		if len(modAddedBy) == 0 {
			...
			// that we can.
			log.Printf("dependencies are stable at %q", rs.RootModules())
			return rs, pkgs, nil
		}
		toAdd := make([]module.Version, 0, len(modAddedBy))
		for m, p := range modAddedBy {
			log.Printf("added: %v (by %v)", modAddedBy, p.ImportPath())
		...
		log.Printf("after loading, requirements: %v", rs.RootModules())
	}
}
...
func (ld *loader) resolveMissingImports(ctx context.Context, pkgs *modpkgload.Packages, rs *modrequirements.Requirements) (modAddedBy map[module.Version]*modpkgload.Package, err error) {
	...
	for _, pkg := range pkgs.All() {
		...
		log.Printf("querying %q", pkg.ImportPath())
	...
	for _, pm := range pkgMods {
		...
		for _, mod := range mods {
			...
			log.Printf("cue: found potential %s in %v", pkg.ImportPath(), mod)
