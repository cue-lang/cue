...
func UpdateVersions(ctx context.Context, fsys fs.FS, modRoot string, reg Registry, versions []string) (*modfile.File, error) {
	...
	rs := modrequirements.NewRequirements(mf.QualifiedModule(), wrappedReg, mf.DepVersions(), mf.DefaultMajorVersions(), mf.Replacements())
	mversions, err := resolveUpdateVersions(ctx, wrappedReg, rs, mainModuleVersion, versions)
	if err != nil {
		return nil, err
	}
	...
	rs = modrequirements.NewRequirements(mf.QualifiedModule(), wrappedReg, newVersions, mf.DefaultMajorVersions(), mf.Replacements())
	g, err = rs.Graph(ctx)
	if err != nil {
	...
	rs = modrequirements.NewRequirements(mf.QualifiedModule(), wrappedReg, finalVersions, mf.DefaultMajorVersions(), mf.Replacements())
