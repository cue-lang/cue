...
//
// The returned registry does not implement repository iteration
// or cross-repository blob mounts.
func New(resolver modresolve.Resolver, newRegistry func(host string) (ociregistry.Interface, error)) ociregistry.Interface {
	return &registry{
		resolver:    resolver,
...
type registry struct {
	*ociregistry.Funcs
	...
	newRegistry func(host string) (ociregistry.Interface, error)
...
func (r *registry) Referrers(ctx context.Context, repo string, digest ociregistry.Digest, artifactType string) ociregistry.Iter[ociregistry.Descriptor] {
	cr, repo, err := r.resolve(repo)
	if err != nil {
		return ociregistry.ErrorIter[ociregistry.Descriptor](ociregistry.ErrUnsupported)
	}
...
func (r *registry) resolve(repo string) (reg ociregistry.Interface, repo1 string, err error) {
	...
	if reg == nil {
		reg1, err := r.newRegistry(loc.Host)
		if err != nil {
