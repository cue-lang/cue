...
func (pkgs *Packages) importFromModules(ctx context.Context, pkgPath string) (m module.Version, loc SourceLoc, altMods []module.Version, err error) {
	...
	for {
		...
		if len(mods) > 1 {
			...
			return fail(&AmbiguousImportError{ImportPath: pkgPath, Locs: locs, Modules: mods})
		}
...
// locInModule locates the location that would hold the package named by the given path,
