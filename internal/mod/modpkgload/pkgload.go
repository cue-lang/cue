...
func (pkgs *Packages) load(ctx context.Context, pkg *Package) {
	...
	if pkg.fromExternal {
		depDefault := func(prefix string) (string, modrequirements.MajorVersionDefaultStatus) {
			v, status, err := pkgs.requirements.DependencyDefaultMajorVersion(ctx, pkg.mod, prefix)
			if err != nil {
				return "", modrequirements.NoDefault
			}
			return v, status
		}
