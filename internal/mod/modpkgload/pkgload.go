...
type Packages struct {
	...
	registry             Registry
	moduleReqsCache      par.ErrCache[string, *modrequirements.Requirements]
}
...
func (pkgs *Packages) load(ctx context.Context, pkg *Package) {
	...
	pkg.mod, pkg.modRoot, pkg.locs, pkg.err = pkgs.importFromModules(ctx, pkg.path, pkgs.requirements.DefaultMajorVersion)
	if pkg.err != nil {
		return
	}
	...
	if pkg.fromExternal {
		depReqs, err := pkgs.getModuleRequirements(pkg.mod, pkg.modRoot)
		if err == nil {
			for i, imp := range imports {
				...
				m, _, _, err := pkgs.importFromModules(ctx, imp, depReqs.DefaultMajorVersion)
				if err != nil || !m.IsValid() {
					continue
				}
				mainM, _, _, mainErr := pkgs.importFromModules(ctx, imp, pkgs.requirements.DefaultMajorVersion)
...
// getModuleRequirements returns the Requirements for the given dependency
// module, reading its cue.mod/module.cue from the already-fetched module
// content. Results are cached by module version.
func (pkgs *Packages) getModuleRequirements(mod module.Version, modRoot module.SourceLoc) (*modrequirements.Requirements, error) {
	return pkgs.moduleReqsCache.Do(mod.String(), func() (*modrequirements.Requirements, error) {
		data, err := fs.ReadFile(modRoot.FS, path.Join(modRoot.Dir, "cue.mod/module.cue"))
		if err != nil {
			return nil, err
		}
		mf, err := modfile.Parse(data, mod.String())
		if err != nil {
			return nil, err
		}
		return modrequirements.NewRequirements(
			mf.QualifiedModule(),
			nil,
			mf.DepVersions(),
