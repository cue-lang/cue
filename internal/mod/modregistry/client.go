...
func (c *Client) PutCheckedModule(ctx context.Context, m *CheckedModule) error {
	...
	selfDigest, err := digest.FromReader(io.NewSectionReader(m.r, 0, m.size))
	if err != nil {
	...
	manifest.Layers[1].Digest = digest.FromBytes(m.modFileContent)
	manifest.Layers[1].MediaType = moduleFileMediaType
	manifest.Layers[1].Size = int64(len(m.modFileContent))

	if _, err := c.registry.PushBlob(ctx, repoName, manifest.Layers[0], io.NewSectionReader(m.r, 0, m.size)); err != nil {
...
func CheckModule(m module.Version, r io.ReaderAt, size int64) (*CheckedModule, error) {
	zipr, modf, _, err := modzip.CheckZip(m, r, size)
	if err != nil {
