...
`
	...
	_, err := rs.Graph(ctx)
	qt.Assert(t, qt.ErrorMatches(err, `bar.com@v0.0.2: module not found`))
...
func (r *registryImpl) ModFile(ctx context.Context, mv module.Version) (*modfile.File, error) {
...
func newRegistry(t *testing.T, registryContents string) Registry {
	...
	qt.Assert(t, qt.IsNil(err))
	regSrv, err := modregistrytest.New(regFS, "")
	qt.Assert(t, qt.IsNil(err))
