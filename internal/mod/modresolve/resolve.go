...
type Resolver interface {
	...
	// AllHosts returns all the registries that the resolver
...
type Location struct {
	...
	Repository string
	// Tag holds the tag used to tag a module version.
...
func (r *resolver) AllHosts() []string {
	// TODO if this turns out to be called frequently, amortise
	// the cost by creating the slice when the resolver
	// is created.
	hosts := make(map[string]bool)
	for _, reg := range r.cfg.ModuleRegistries {
		hosts[reg.Host] = true
	}
	if r.cfg.DefaultRegistry != nil {
		hosts[r.cfg.DefaultRegistry.Host] = true
	}
