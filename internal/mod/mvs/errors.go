...
func (e *BuildListError[V]) Error() string {
	...
	if len(stack) == 0 {
		...
		//			actualPath := mErr.Path
		//			actualVersion := mErr.Version
		//			if v, ok := mErr.Err.(*module.InvalidVersionError); ok {
		//				actualVersion = v.Version
		//			}
		//			if actualPath == e.vs.Path(m) && actualVersion == e.vs.Version(m) {
		//				fmt.Fprintf(b, "%v", e.Err)
		//			} else {
		//				fmt.Fprintf(b, "%s@%s (replaced by %s): %v", m, actualPath, actualVersion, mErr.Err)
		//			}
		//		} else {
		//			fmt.Fprintf(b, "%s: %v", m, e.Err)
		//		}
	}
	return b.String()
}
