...
func (c *CallCtxt) getList(i int) *adt.Vertex {
	...
	case ok && v.IsList():
		...
		if err := v.Bottom(); err != nil {
			// This is the list value's own error, which locates the failure
			// within the list (e.g. a bad element); keep its path rather than
			// dropping it as setCallError would.
			c.Err = &callError{err}
			return nil
		}
		return v
