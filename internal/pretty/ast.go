...
func unchainField(f *ast.Field) (chain []*ast.Field, collapsible bool) {
	...
	for _, cf := range chain[1:] {
		if len(ast.Comments(cf)) > 0 || cf.Pos().IsNewline() {
			return chain, false
		}
	}
	return chain, true
...
func (c *converter) armDoc(a chainArm) Doc {
	...
	prefix = append(prefix, c.expr(a.expr))
	return Cats(prefix...)
}
...
func (c *converter) injectInteriorComments(x ast.Expr, extra []*ast.CommentGroup) Doc {
	...
	case *ast.ListLit:
		...
		if rows := c.elementRows(n.Elts, false); rows != nil {
			body = Table(rows)
		}
