...
func (c *converter) listLit(x *ast.ListLit) Doc {
	...
	case len(unelided) == 1 && len(interior) == 0 &&
		!unelided[0].Pos().IsNewline() && !c.exprHasDocComment(unelided[0]):
		...
		// breaks, its own Nest, or a trailing comma. A doc comment
		// on the element would force a break, and that break must
		// be reflected in the list layout (so the standard
		// list-break path is used instead).
