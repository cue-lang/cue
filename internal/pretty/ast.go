...
// the converter calls [converter.maybeGroup]. See [isWrapEligible]
...
//
// The isAuthored algorithm only sets the flag on these node type. If
...
func (c *converter) simpleFieldChain(f *ast.Field) []*ast.Field {
	...
	if hasDocComment(leaf.Value) {
		return nil
	}
	if leaf.Value.Pos().IsNewline() {
		return nil
	}
	if hasCommentAt(leaf, posSuffix) {
		return nil
	}
	return chain
}
...
//     StringLit(", ") and the `//` would absorb the next
...
// docCommentBlock renders a sequence of doc comment groups followed
// by a trailing separator chosen from trailingRel: HardLine by
// default, BlankLine when trailingRel is NewSection. Pass the host
// node's RelPos to keep a blank line between the last comment and the
// body when the source had one. Returns nil for an empty input.
...
//
// cg.Line is interpreted as implying cg's RelPos is Blank. For
// parser-produced ASTs this is already true. But programmatically
// built ASTs can set cg.Line=true without supplying a positioned
// Slash (e.g. ast.AddComment with CommentGroup{Line: true}), leaving
// RelPos as NoRelPos. We upgrade such a RelPos to Blank so that these
// comments flow through the same "same-line trailing" path.
//
// Mapping:
...
// subtree (only hard line breaks - [docLineBreakHard], [docLineBreakBare],
// or BlankLine - survive), and nested
...
//
// isAuthored is set per-subtree by [converter.analyse]: see the
...
func (c *converter) maybeGroup(n ast.Node, body Doc) Doc {
	...
	// type listed in isWrapEligible. The isAuthored analyse pass
...
func (c *converter) chainTableExpr(x *ast.BinaryExpr, arms []chainArm, fieldTrailing Doc) Doc {
	...
	for i, info := range infos {
		...
		if i == 0 {
			// First row as Raw so its op suffix stays glued to the arm
...
func (c *converter) fieldRow(chain []*ast.Field) (Row, []*ast.CommentGroup) {
	...
	if len(chain) > 1 {
		_, valBroken, valCanWrap := measureCell(valDoc)
...
// val inside a Nest, so val is rendered on its own line indented
// relative to the key. Callers build preVal to start with a HardLine
// and to include any own-line Position=2 suffix comments at val's
// indent, each followed by a HardLine - the final HardLine bridges
// the last comment to val, and the placement round-trips cleanly:
// re-parsing such output attaches the comment as a doc comment of
// the value, and re-rendering produces the same indented layout.
// preVal == nil means no leading break: val is returned as-is and
// rendered on the key's line.
...
func (c *converter) importDecl(x *ast.ImportDecl) Doc {
	...
	for i, s := range x.Specs {
		...
		if i > 0 {
			// Use leadingRel so a NewSection on the spec's doc comment
			// is honoured - otherwise a blank line written before a
			// doc-commented spec is silently dropped, since the spec's
			// own RelPos is just Newline.
