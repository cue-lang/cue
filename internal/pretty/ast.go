...
func (c *converter) structLit(x *ast.StructLit) Doc {
	...
	default:
		...
		// Contiguous-opener rule: see listLit for the full explanation.
		...
		hugFirst := cleanStruct &&
			len(x.Elts) > 0 &&
			declIsContiguousOpener(x.Elts[0]) &&
			!x.Elts[0].Pos().IsNewline()
		hugLast := cleanStruct &&
			len(x.Elts) > 0 &&
			declIsContiguousOpener(x.Elts[len(x.Elts)-1]) &&
...
func (c *converter) listLit(x *ast.ListLit) Doc {
	...
	default:
		...
		// Contiguous-opener rule: when the first element is itself a
		// brace-opener (`{`, `[`, `(`) that starts on the same line as
		// `[`, the two openers share one level of indent — keep `[{`
		// adjacent even in broken mode and let the inner element's own
		// Group decide if/where to break. The same applies symmetrically
		// at the close: when the last element is a brace-opener whose
		// closer sits on the same line as `]`, keep `}]` adjacent and
		// suppress the trailing comma (there is no line between them).
		// Only "clean" lists can hug their contiguous openers/closers.
		// Any doc comment on an element, or any interior comment
		// attached to the list, forces a line break at the list level
		// (independent of whether the first/last element is a
		// brace-opener), so the list must render as a true multi-line
		// bracket pair with `]` on its own line and a trailing comma.
		...
		if c.nodeFlags[x]&flagNewlineInChildren != 0 {
		...
		if len(unelided) > 0 {
			...
			for i, e := range unelided {
				row := c.listElemRow(e, i == lastElemIdx, wantTrailingComma)
				if i > 0 {
...
// declIsContiguousOpener reports whether a decl's rendering begins
// with an opening `{`, `[`, or `(`. Only embed decls qualify: fields
// always begin with a label. Used by structLit to apply the
// contiguous-opener rule to `{{...}}`, `{[...]}`, and `{(...)}`.
func declIsContiguousOpener(d ast.Decl) bool {
	emb, ok := d.(*ast.EmbedDecl)
	return ok && isContiguousOpener(emb.Expr)
}

// anyHasDocComment reports whether any node in the slice carries a
// Position=0 (doc) comment. Used by the contiguous-opener rule to
...
func (c *converter) commentSep(cg *ast.CommentGroup, cd Doc) Doc {
	...
	// Line=false means the comment is not on the same line as the
	// preceding token. A `//` line comment placed inline would absorb
	// everything up to the next newline, silently merging it with
	// whatever follows; even a `/* block */` written on its own line
	// shouldn't be squashed onto a shared line. Fall back to a blank
...
//   - Position 1 !Line → prefix (inside, just after the opener)
//   - Position 2 !Line → suffix (inside, just before the closer)
//   - Position ≥ 3 !Line, or Line=true → trailing (after the node)
//
// Non-bracketed nodes (Field, etc.) generally only use Position 0
// (doc) and Line=true (trailing). Any other Position on such a node
// is still classified into prefix/suffix/trailing consistently, but
...
type commentSlots struct {
	doc      []*ast.CommentGroup // Position==0
	prefix   []*ast.CommentGroup // Position==1, !Line
	suffix   []*ast.CommentGroup // Position==2, !Line
	trailing []*ast.CommentGroup // Line==true, or Position≥3 and !Line
}
...
func classifyComments(n ast.Node) commentSlots {
	...
	for _, cg := range ast.Comments(n) {
		...
		default:
			// Position ≥ 3 non-line: after the node's last token.
			s.trailing = append(s.trailing, cg)
		}
	}
	return s
