...
// re-parse at the same position. Otherwise, comments are placed before the
// expression.
...
func (c *converter) chainedBinaryExpr(x *ast.BinaryExpr) *Doc {
	...
	// Build table rows. Each arm has two cells:
	// - Cell 0: expression + operator suffix (e.g. "A |")
	// - Cell 1: trailing comment (if any)
	// Comments from infos[i] belong to the PREVIOUS arm (they trail
	// the previous arm's operator).
	var rows []Row
	for i, info := range infos {
		...
		if info.comment != nil && i > 0 {
			rows[i-1].Cells[1] = info.comment
			rows[i-1].HasComment = true
		}
	}
	...
	if c.chainFieldTrailing != nil {
		...
		if last.Cells[1] == nil {
			last.Cells[1] = c.chainFieldTrailing
		} else {
			last.Cells[1] = Cats(last.Cells[1], spaceText, c.chainFieldTrailing)
		}
		last.HasComment = true
	}
