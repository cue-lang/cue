...
// forced cases the body is pre-flattened: soft Line nodes are
// replaced by their Alt text (so Wadler-Lindig will not inject
// additional width-driven newlines at this level) while HardLine /
// LitLine / BlankLine nodes are preserved.
//
// The RelPos branch additionally wraps the pre-flattened body in
// NoWidth, which propagates through every inner Doc constructor:
// Groups, Tables, Cats, Nests, IfBreaks all see an unlimited width
// budget, so chains, calls, and lists inside the RelPos-bearing
// subtree stay flat unless a HardLine forces a break. The user has
// expressed where lines break with their RelPos; the formatter must
// not add more. To opt back into multi-line layout for an inner
// construct the user puts a Newline RelPos somewhere inside it.
//
...
func (c *converter) maybeGroup(n ast.Node, shareIndent bool, body Doc) Doc {
	if c.nodeFlags[n]&flagNewlineInChildren != 0 {
		return NoWidth(preFlatten(body))
	}
	if shareIndent {
