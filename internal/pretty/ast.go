...
type converter struct {
	...
	// renders compact (`1+2`), matching the legacy formatter. It is
	// bumped by indexExpr/sliceExpr around their contents and reset to
	// 0 at a struct/list (block) boundary, whose fields/elements start
	// fresh at the top level. This is a layout property independent of
	// RelPos, so it lives here rather than in the style pre-pass.
	subscript int
}
...
func (c *converter) simpleFieldChain(f *ast.Field) []*ast.Field {
	...
	if len(chain) > 1 && !collapsible {
		...
		// whose value is that struct; fall back to the single-element
		// chain instead of bailing, so a single-line struct value still
		// aligns in the table (a multi-line value breaks the segment
		// via the leaf checks / table machinery below, as before).
		chain = chain[:1]
	}
...
func (c *converter) exprCore(x ast.Expr) Doc {
	...
	case *ast.Ident:
		...
		// be quoted into a string. Record an error, matching the legacy
		// formatter, and emit the name as a best-effort rendering.
	...
	case *ast.BadExpr:
		...
		// which is valid CUE and matches the legacy formatter. (A C-style
		// `/* ... */` comment would be invalid - CUE has only `//`.)
		return bottomLit
...
func (c *converter) label(l ast.Label) Doc {
	...
	case *ast.Ident:
		...
		// empty name) as a quoted string label, matching the legacy
		// formatter. This only arises in programmatic ASTs; the parser
		// never produces an invalid ident label.
		if !ast.IsValidIdent(x.Name) {
...
// level via [HardLine] inside a [Nest] - so the body lands at
...
func reindentMultilineString(value string) Doc {
	...
	for _, line := range lines[1:] {
		if rest, ok := strings.CutPrefix(line, strip); ok && strip != "" {
			body = append(body, HardLine(), StringLit(rest))
		} else {
...
func (c *converter) structLit(x *ast.StructLit) Doc {
	...
	case len(unelided) == 0 && !hasInterior:
		return StringLit("{}")
	case len(unelided) == 1 && !hasInterior && !isFieldDecl(unelided[0]) && c.shouldHug(unelided[0]):
	...
	layout := bracketedLayout{
		...
		sameLineOpen:  hasSameLineOpener(c, unelided),
...
func (c *converter) listLit(x *ast.ListLit) Doc {
	...
	omitCommas := listOmitsCommas(unelided, x.Lbrack, x.Rbrack)
	...
	var inner Doc
	if rows := c.elementRows(unelided, policy.wantTrailingComma, useBodyShape, omitCommas); rows != nil {
		inner = Table(rows)
	}
...
func (c *converter) computeBracketedPolicy(b bracketedLayout) bracketedPolicy {
	...
	// Nest, so shareIndent is suppressed too.
	forceOpen := false
...
// Nest so the inner content sits at one shared indent level.
//
// The opener must also break across lines to qualify: the rule drops
// the parent's Nest on the premise that the inner opener supplies the
// indent, which only holds if it actually opens. An opener that stays
// inline - e.g. a short embedded list `[1, 3]` - supplies no indent,
// so dropping the Nest would leave the rest of the body un-indented.
...
func closeBreakDoc(closerRel token.RelPos, forceClose bool) Doc {
	if forceClose {
		return relBreakOr(closerRel, HardLine())
	}
...
// lines). It mirrors the rule used by the legacy formatter (see
// cue/format.listOmitsCommas): the style is selected from the
// source's existing comma usage rather than imposed. It returns true
// when any inter-element comma is missing; for a single-element list
...
func (c *converter) binaryExprPrec(x *ast.BinaryExpr, co, depth int) Doc {
	...
	if prec < co || operatorsWouldMerge(x.Op, x.Y) {
		...
		// `++`), matching cue/format's mayCombine guard.
		maybeSpace = spaceLit
	}
...
func (c *converter) sliceExpr(x *ast.SliceExpr) Doc {
	...
	// bound is a binary expression - matching the legacy formatter
	// (`s[1+2 : 2+4]`).
	colon := colonLit
...
// there can merge with the preceding operator. Mirrors cue/format's
// mayCombine guard, which the legacy printer applies at the token
// level.
...
func (c *converter) parenExpr(x *ast.ParenExpr) Doc {
	...
	// expression: `((x))` renders as `(x)`, matching the legacy
	// formatter (cue/format). The outer paren's own comments are
	// handled by the enclosing [converter.expr].
	...
	if !multiLine {
		...
		for i, e := range x.Elts {
			if lit, ok := e.(*ast.BasicLit); ok && lit.Kind == token.STRING {
				...
				// binary operators (`"\(1+2)"`), matching the legacy
				// formatter's expr0(el, depth+1).
				c.subscript++
				parts[i] = c.expr(e)
				c.subscript--
			}
...
// in a [Nest] so it lands one level deeper than the opening line -
...
// [HardLine], rather than reproducing whatever indent the producer
// embedded.
		...
		if lit, ok := e.(*ast.BasicLit); ok && lit.Kind == token.STRING {
			for i, line := range strings.Split(lit.Value, "\n") {
				if i > 0 {
					...
					// surrounding Nest re-indents to the render depth; we
...
func (c *converter) decl(d ast.Decl) Doc {
	...
	case *ast.BadDecl:
		...
		// BadExpr: valid CUE (an embedded bottom) rather than an invalid
		// C-style comment.
		return bottomLit
...
func (c *converter) postfixAlias(a *ast.PostfixAlias) Doc {
	...
	// `~X`), matching the legacy formatter, which enforces parens, and
	// the parser, which accepts both but normalises to the parens
	// form.
	if a.Label == nil {
