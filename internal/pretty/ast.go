...
// supported.
//
// Out of scope: Rendering AST shapes the parser cannot produce. For
// example, a [ast.Field] with a Position=1 comment (between label and
// colon) has no source layout: `//` runs to end of line and the
// parser doesn't allow a newline between label and colon, so this
...
const (
	...
	// node and the RelPos node that is eligible for
	// wrapping. I.e. isAuthored is set on the eligible node nearest a
	// RelPos-containing subtree, and not any higher (in contrast to
	// relPosInChildren which propagates up the tree to the very root,
	// once set).
	isAuthored
	...
	// `//` comment, regardless of whether that comment sits on the
	// node's own line (cg.Line) or its own line. CUE has only `//` line
	// comments, which run to end-of-line, so such a tail swallows any
	// same-line token emitted after the node. This is broader than
	// endsWithOwnLineComment (which is restricted to own-line comments
	// for the declaration-separator promotion in declSep); it is used to
	// force a break before a comprehension body whose preceding clause
	// ends with a trailing comment. Propagated up only through the
	// rightmost-rendered child of non-bracketed nodes, mirroring
	// endsWithOwnLineComment.
	endsWithLineComment
)
...
type converter struct {
	...
	// subscripts, used by [converter.binaryExpr] to compact operator
	// spacing: a binary expression at the top of a value renders with
	// normal spacing (`1 + 2`), but the same expression inside `[...]`
	// renders compact (`1+2`). It is bumped by indexExpr/sliceExpr
	// around their contents and reset to 0 at a struct/list (block)
	// boundary, whose fields/elements start fresh at the top level.
	// This is a layout property independent of RelPos, so it lives
	// here rather than in the style pre-pass.
	subscript int
}

// errf records a formatting error at pos.
func (c *converter) errf(pos token.Pos, format string, args ...interface{}) {
			...
			if hasRelPos {
				...
				for i := len(stack) - 1; i >= 0; i-- {
					...
					if missing := ancestorsFlags &^ f.flags; missing == 0 {
						// If the frame already has the flags we want, we
						// know all its ancestors do too, so nothing further
						// for us to do.
						break
					}
...
//     preserved by field() under [docNest]+HardLine), and
...
func (c *converter) simpleFieldChain(f *ast.Field) []*ast.Field {
	...
	if len(chain) > 1 && !collapsible {
		...
		// whose value is that struct; use the single-element chain
		// instead of returning nil, so a single-line struct value still
		// aligns in the table (a multi-line value breaks the segment
		// via the leaf checks / table machinery below).
		chain = chain[:1]
	}
...
// the separator is promoted to NewSection when prev is a Definition
// (#Foo) or any non-field, non-comment decl (matching A4's shape).
// Inside an [infiniteWidth] wrap a lineBreakComma would otherwise be
// rewritten to stringLit(", ") and the `//` would absorb the next
// decl. Promoting also keeps the parse/format cycle idempotent when
// the comment migrates from prev's trailing slot into the next decl's
// doc on reparse.
//
// Second, if prev's rendered output ends with any `//` comment - own
// line or same line - the separator is forced to at least Newline. CUE
// line comments run to end-of-line, so an inline (`, `) separator and
// the following decl would be swallowed. This covers same-line
// trailing comments on a body the style pass marked compact (a Blank
// RelPos hint), which only arises programmatically: a same-line `//`
// in authored source already forces the next decl onto its own line.
//
...
//
// When the child can fit flat, the standard [docGroup]-based path handles
...
// into a doubly-broken layout). When it won't, the standard [docGroup]
...
// node's `{...}` / `[...]` / `(...)` area. Used by
// [converter.structLit] / [converter.listLit] to decide whether to
// route interior comments into the body via
// [converter.wrapInteriorComments] and whether to suppress the
// useBodyShape path (which doesn't preserve interior comments).
...
// caller supply pre-computed (and possibly filtered) slots. Used by
// [converter.fieldRow] to render a leaf value whose Line=true
// comments have been lifted to the field's trailing-cell column: the
// lifted slots are excluded so they aren't double-rendered.
...
func (c *converter) structLit(x *ast.StructLit) doc {
	...
	case len(elems) == 0 && !hasInterior:
		return stringLit("{}")
	case len(elems) == 1 && !hasInterior && !isFieldDecl(elems[0]) && c.shouldHug(elems[0]):
...
func (c *converter) computeBracketedPolicy(b bracketedLayout) bracketedPolicy {
	...
	forceOpen := false
	if isFieldDecl(b.firstElem) && c.hasNewlineInSubtree(b.firstElem) {
		forceOpen = true
	}
...
// cell in [docNextGroupNoop]. The caller sets this param when the body's
// cells will be rendered via [docBodyShape]; [docNextGroupNoop] forwards
...
func (c *converter) elementRows(elems []ast.Expr, trailingComma, wrapLinked, omitCommas bool) []row {
	...
	for i, e := range elems {
		...
		if i > 0 {
			...
			// horizontally (`}, {`). That is only safe when the previous
			// element does not end with a `//` comment (which would
			// swallow the following `{`) and the current element has no
			// doc comment (which needs its own line); otherwise fall back
			// to a normal element break.
			if prevLackRelPos && curLackRelPos && !prevHasComment && row.docComment == nil {
				row.sep = spaceLit
			} else {
				row.sep = elemBreak(e)
			}
...
func (c *converter) listElemRow(e ast.Expr, last, trailing, omitCommas bool) (row, []*ast.CommentGroup) {
	...
	case !last && omitCommas:
		...
		// shares a line with the next (flat mode); on separate lines
		// the line break alone separates the elements.
		comma = commaWhenFlat
	case !last:
		comma = commaLit
	case trailing:
...
// binary operand is parenthesised - without that wrap, `!a & b` would
// re-parse as `(!a) & b` and change meaning.
...
// binary operand is parenthesised - without that wrap, `a & b ...`
// would re-parse as `a & (b ...)` and change meaning.
...
// inside commaWhenBroken when a width-driven break occurs
// (programmatic mode).
...
func (c *converter) sliceExpr(x *ast.SliceExpr) doc {
	...
	if c.subscript == 0 && x.Low != nil && x.High != nil &&
		(isBinaryExpr(x.Low) || isBinaryExpr(x.High)) {
...
// rather than at body-indent + 1. With [docAtIndent], line breaks inside
// the body emit indentation at strip-prefix levels, so any [docNest]
...
// in a [docNest] so it lands one level deeper than the opening line -
// matching [reindentMultilineString] for plain strings. The embedded
// strip prefix is removed from each body line and the renderer
// re-supplies the indentation at the actual render depth via
// [docLineBreakHard], rather than reproducing whatever indent the producer
// embedded.
...
// clause. lead is the separator placed before the keyword, supplied by
// the caller so a preceding `//` comment can force the keyword onto a
// fresh line.
...
// continuation lives inside a [docNest] so it indents relative to the key;
// for a braceless chain value (`a: b: c`) the chain elements stay at
// the outer indent and the [docNest] is omitted - the chain isn't a
...
// renders flat - the enclosing [docGroup] then picks compact
...
var (
	...
	commaWhenFlat   = switchMode(nil, commaLit)
...
//
// The style package keeps an independent approximation of the
// [endsWithLineComment] flag this predicate feeds (see style's
// endsWithLineComment); the two need not agree exactly, and changes
// here do not require matching changes there.
...
// isFieldDecl reports whether n is a regular field (`label: value`).
// A struct's single-element hug is rejected for such a decl because
// the label would sit between the hugged braces (`{field: {`); embeds
// and other value-leading decls remain eligible.
func isFieldDecl(n ast.Node) bool {
	_, ok := n.(*ast.Field)
	return ok
}
...
// partitionLineComments splits in into (lineComments, others) -
// CommentGroups with Line=true land in the first return value, the
// rest in the second. Used by [converter.fieldRow] to separate
// same-line trailing comments (which can be lifted to the field's
// trailing-cell column) from comments that anchor in place (doc
// comments, prefix/suffix blocks).
...
// and the renderer re-emits the indentation at its *actual* nest
// level via [docLineBreakHard] inside a [docNest] - so the body lands at
// field-indent + 1 regardless of what the producer embedded. Empty
// body lines emit a bare newline so they carry no trailing
// whitespace, matching CUE's multi-line string semantics (the strip
// prefix is not applied to blank lines). Returns nil if the quotes
// can't be parsed or the literal isn't genuinely multi-line, so the
// caller falls back to verbatim rendering.
...
// inner content in a [docNest] of its own. When that's true, an enclosing
// construct (struct/list/call) can drop its own [docNest] at the boundary
// where it meets e: e's [docNest] will provide the one indent level the
...
// [docNest] they introduce around their broken body has the same effect
...
// time: the body is wrapped in a [docNest] and emitted between the
...
// leading RelPos (including a Newline carried by a doc comment): that
// element is on a new line, and so are all later ones. Used to
// implement the same-line-opener rule: when any element opener shares
// a line with the parent's `[`/`{`/`(`, the parent suppresses its own
// [docNest] so the inner content sits at one shared indent level.
//
// The opener must also break across lines to qualify: the rule drops
// the parent's [docNest] on the premise that the inner opener supplies the
// indent, which only holds if it actually opens. An opener that stays
// inline - e.g. a short embedded list `[1, 3]` - supplies no indent,
// so dropping the [docNest] would leave the rest of the body un-indented.
...
// openBreak / row separators under asInfiniteWidth - its [docNest] is
// unused, and stacking it on top of an inner construct's own [docNest]
// pushes inner breaks one indent level too deep. The parent's [docNest] is
...
// at the parent's caller-indent + its own [docNest] - not + 2.
...
// lines). The style is selected from the source's existing comma
// usage rather than imposed. It returns true when any inter-element
// comma is missing; for a single-element list the trailing comma
// (before ']') is checked instead.
//
// The Scanned bit on lbrack distinguishes scanner-produced lists from
// programmatically constructed ASTs. Programmatic ASTs (Scanned
// false) always keep commas, since they carry no meaningful comma
// hints. The parser only admits a missing inter-element comma at
// language version v0.17.0+, so a list reaches this with a missing
// comma only when the comma-free style is valid for the file.
...
// visual grouping, so the chain skips its own [docNest] and the arms
...
// structure that could anchor a comment. Used by [converter.fieldRow]
// to decide whether a Line=true comment on the value can be safely
// re-routed to the field's trailing-cell column. Composite
// expressions (BinaryExpr, UnaryExpr, CallExpr, ...) are excluded
// because the parser legitimately attaches internal-layout comments
// to them (e.g. "between operator and RHS" lands at Position=2,
// Line=true on a BinaryExpr and must stay inside the expression's
// rendering).
...
// grouping by tree shape, but programmatic builders
// ([cuelang.org/go/cue.Value.Syntax], the tools/fix passes, etc.)
// don't insert [*ast.ParenExpr] wrappers, so each rendering site
// whose context binds tighter than some binary operator must
// materialise the parens itself or the output would re-parse to a
// differently-grouped tree.
//
// Only [*ast.BinaryExpr] needs the wrap: every other expression type
// either is itself bracketed (StructLit/ListLit/ParenExpr/CallExpr/
// IndexExpr/SliceExpr), produces a single token (Ident/BasicLit/
// BottomLit/BadExpr), or binds at least as tightly as the postfix /
// unary contexts (PostfixExpr, UnaryExpr, SelectorExpr).
...
// isBinaryExpr reports whether e is a binary expression.
func isBinaryExpr(e ast.Expr) bool {
