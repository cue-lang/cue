...
// comments with Newline / NewSection get their own line(s); a
// non-Line=true comment with no RelPos hint falls back to a blank
// line — Line=false means the parser saw the comment on its own
// source line, and CUE's `//` comments run to end-of-line, so
// squashing it onto a shared line would silently absorb everything
// up to the next newline. (The blank-line fallback also matches how
// the parser re-attaches such comments as doc of the following decl,
// which triggers the declSep NewSection upgrade, preserving
// idempotency.)
func (c *converter) commentSep(cg *ast.CommentGroup, cd Doc) Doc {
	...
	return Cat(relBreakOr(cg.Pos().RelPos(), soft), cd)
}
