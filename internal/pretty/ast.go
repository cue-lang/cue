...
// File overview
//
// This file translates CUE AST nodes (cuelang.org/go/cue/ast) into
// the Doc algebra defined in doc.go. The translation entry point is
// [converter.node]; it recurses through the AST and produces a Doc
// that render.go renders to bytes.
//
...
// (built by code, or parser output with RelPos stripped) typically
// have none. pretty selects between two modes per-subtree:
//
//   - parsed mode: the formatter respects the authored layout. The
//     converted Doc is wrapped in NoWidth + preFlatten; soft Lines
//     and IfBreak resolve statically, and width-driven Group breaks
//     are suppressed. Only HardLine / LitLine / BlankLine produce
//     line breaks.
//
//   - programmatic mode: width-driven Wadler-Lindig. Groups pick
//     flat or broken to fit the width budget. Soft Lines break when
//     the line overflows. Wrapped in [Programmatic] so that, when
//     this subtree is itself nested inside a parsed wrap, the inner
//     Group's flat-vs-broken decision still uses the real width
//     budget instead of inheriting the outer NoWidth's MaxInt
//     budget. (No-op when not nested inside a parsed wrap.)
//
...
// cluster (via flagShouldWrap), so a stray RelPos hint in an
// otherwise-programmatic AST does not flip the whole file into
// parsed mode. A programmatic pocket nested inside a parsed wrap
// (a wrap-eligible subtree with no RelPos descendants) gets
// width-driven layout via the Programmatic boundary; the surrounding
// parsed-mode treatment continues to apply to its parsed siblings.
//
// # Tables
//
...
// # Terminology
//
//   - RelPos: a token.Pos's relative-position hint. Drives parsed-
//     mode layout decisions.
//   - parsed mode / programmatic mode: see above. "Programmatic"
...
//     called. Each wrap site is a candidate for the parsed-mode
//     NoWidth+preFlatten wrap; flagShouldWrap selects which
//     actually fire.
...
//     single-element synthetic StructLit (e.g. a: b: c: 1), or a
//     sequence of BinaryExprs with the same operator (e.g. a | b
//     | c).
//
// # Scope
//
...
// mode while preserving any authored layout in nested parsed
// pockets.
//
// Out of scope:
//
...
const (
	...
	// carry a Newline or NewSection RelPos. Used to predict whether
	// the subtree will render with forced breaks (relevant to single-
	// element hug decisions and to statically resolved trailing-comma
	// placement).
	flagNewlineInChildren nodeFlag = 1 << iota
	// flagRelPosInChildren marks a node whose strict descendants carry
	// any RelPos at all (NoSpace, Blank, Newline, NewSection, Elided).
	// Reported via [converter.parsed]. Used by layout decisions that
	// need to know whether a subtree carries authored layout (e.g.
	// the bracket-policy logic and the chain-Doc construction); the
	// parsed-mode wrap itself is now driven by flagShouldWrap.
	flagRelPosInChildren
	// flagCommentInSubtree marks a node whose subtree (including
	// itself) has an attached comment group.
	flagCommentInSubtree
	// flagShouldWrap marks a node where [converter.maybeGroup] should
	// wrap with NoWidth + preFlatten. Set on the smallest wrap-
	// eligible ancestor of each connected RelPos cluster (or on the
	// node itself if it carries RelPos directly and is wrap-eligible).
	// Wrap-eligible types are listed in [isWrapEligible]. The flag is
	// computed in [converter.precompute] via a bottom-up "uncovered
	// RelPos" pass: a wrap-eligible node sets the flag and stops
	// propagating its subtree's RelPos to ancestors, so an outer wrap-
	// eligible node only fires for *other* RelPos clusters.
	flagShouldWrap
)
...
//
// The flagShouldWrap algorithm only sets the flag on these types
...
// precompute populates c.nodeFlags in a single ast.Walk over root's
// subtree. For each visited node it ORs the relevant flag bits onto
// every ancestor up the current stack, short-circuiting at the first
// ancestor that already carries every bit being propagated — total
// work stays amortized O(N). The strict-descendants flags
// (flagNewlineInChildren, flagRelPosInChildren) are propagated from
// the parent up; flagCommentInSubtree is self-inclusive (the node
// with the comment also gets marked).
//
// flagShouldWrap is computed by the post-order callback using a
// parallel "uncovered RelPos" stack: each node OR-s its own RelPos
// and its children's uncovered bits up; if a wrap-eligible node sees
// any uncovered RelPos, it sets flagShouldWrap on itself and
// suppresses propagation to its parent (its subtree is now covered
// by its own wrap). This finds the smallest wrap-eligible covering
// subtree for each connected RelPos cluster.
func (c *converter) precompute(root ast.Node) {
	...
	ast.Walk(root,
		func(n ast.Node) bool {
			...
			hasUncovered = append(hasUncovered, n.Pos().HasRelPos())
			var fromParent, fromSelf nodeFlag
			if n.Pos().IsNewline() {
				fromParent |= flagNewlineInChildren
			}
			if n.Pos().HasRelPos() {
				fromParent |= flagRelPosInChildren
			}
			if len(ast.Comments(n)) > 0 {
				fromSelf |= flagCommentInSubtree
			}
			// Self-inclusive bits also propagate up, so combine them
			// when ascending; only fromSelf is set on n itself.
			if fromSelf != 0 {
				c.nodeFlags[n] |= fromSelf
			}
			ascend := fromParent | fromSelf
			for i := len(stack) - 2; ascend != 0 && i >= 0; i-- {
				f := c.nodeFlags[stack[i]]
				missing := ascend &^ f
				if missing == 0 {
					break
				}
				c.nodeFlags[stack[i]] = f | missing
			}
			return true
		},
		func(n ast.Node) {
			myHas := hasUncovered[len(hasUncovered)-1]
			hasUncovered = hasUncovered[:len(hasUncovered)-1]
			stack = stack[:len(stack)-1]
			if isWrapEligible(n) && myHas {
				c.nodeFlags[n] |= flagShouldWrap
				myHas = false
			}
			if myHas && len(hasUncovered) > 0 {
				hasUncovered[len(hasUncovered)-1] = true
			}
		})
}
...
func (c *converter) file(f *ast.File) Doc {
	...
	// programmatic source (NoSpace) gets a softLineComma.
	...
	if len(f.Decls) > 0 {
		firstDeclSep = relBreakOr(leadingRel(f.Decls[0]), HardLine())
	}
...
// composed of Package / ImportDecl / CommentGroup. The CUE parser
// rejects field decls before any ImportDecl, so this prefix is the
// run of "header" decls that visually precede the file body.
// Elided decls are skipped (they're invisible) so they don't
...
// CommentGroup). Each decl is emitted as a standalone Doc with
// relpos-driven separators between them; alignment never applies
...
func (c *converter) fileHeader(decls []ast.Decl) Doc {
	...
	for _, decl := range decls {
		...
		if _, ok := decl.(*ast.CommentGroup); ok {
			...
			d = c.withComments(decl, c.decl(decl))
		}
...
func (c *converter) declSlice(decls []ast.Decl) Doc {
	...
	// same row).
	multiLine := declsAreMultiLine(decls)
	...
	for _, decl := range decls {
		...
		if prev != nil {
			sep = c.declSep(decl, prev)
			if multiLine && sep == softLineComma {
				sep = HardLine()
			}
		}
		prev = decl
...
// declsAreMultiLine reports whether any inter-decl separator in decls
// is a hard break (Newline / NewSection RelPos). When true, declSlice
// promotes every comma-style separator (NoSpace / Blank) to HardLine
// so the struct doesn't render with a mix of own-line and shared-line
// fields.
func declsAreMultiLine(decls []ast.Decl) bool {
	first := true
	for _, decl := range decls {
		if decl.Pos().RelPos() == token.Elided {
			continue
		}
		if !first && leadingRel(decl) >= token.Newline {
			return true
		}
		first = false
	}
...
func (c *converter) isSimpleField(f *ast.Field) bool {
	...
	// A value on its own line (Newline/NewSection RelPos) is rendered
	// via field() so valNeedsLeadingBreak can place it under a
	// Nest+HardLine. fieldRow's inline cells layout would otherwise
	// drop the user-written break. Routing through field() also makes
	// the row-break that fieldRow performs for over-wide rows
	// idempotent: re-parsing places Newline RelPos on the value, and
	// field() reproduces the break.
	if leaf.Value.Pos().IsNewline() {
		return false
	}
	// A Position=2 comment (between colon and value) forces val
	// onto its own line — `//` runs to end-of-line, so val can't
	// share a line with the comment. The table layout in fieldRow
	// has no place to put a between-cells, on-its-own-line comment,
	// so route to field() which handles the leadingBreak case.
...
// heuristic.
//
// And: if the previous decl ends with a `//` trailing comment, the
// separator is promoted to at least Newline (HardLine) regardless of
// d's authored RelPos. CUE's `//` runs to end-of-line, so the next
// decl must start on a fresh line. This matters specifically inside
// a parsed wrap (NoWidth + preFlatten), where a softLineComma
// separator would otherwise be preFlattened to Text(", ") and
// emitted literally on the comment's line — the `//` would then
// absorb the next decl. Outside a parsed wrap softLineComma in
// broken mode would emit a newline anyway, so the promotion is a
// no-op there.
func (c *converter) declSep(d ast.Decl, prev ast.Decl) Doc {
	rel := leadingRel(d)

	// When the current decl carries a doc comment and the previous
	// decl is either a Field that's a Definition or any non-field,
	// non-comment decl, fill in a blank line — regardless of the
	// authored RelPos (provided rel is not already NewSection or
	// higher). This matches cue/format's long-standing convention:
	// definitions form a section break, and non-field decls (let,
	// embed, comprehension, attribute, ...) are visually distinct
	// enough from a doc-commented sibling field that they warrant
	// the same treatment. Running both formatters over the same
	// input yields the same blank-line discipline.
	//
	// prevTrailing is true when prev is in a parsed wrap and its
	// subtree carries a non-doc comment that may end up at the end
	// of prev's rendered output. The "in a parsed wrap" check uses
	// flagRelPosInChildren — if prev has any RelPos in its subtree,
	// the smallest wrap-eligible ancestor will have set
	// flagShouldWrap, and the inter-decl separator after prev will
	// be subject to preFlatten. Outside a parsed wrap, soft Lines
	// emit newlines correctly in broken mode and no promotion is
	// needed.
	prevTrailing := prev != nil &&
		c.nodeFlags[prev]&flagRelPosInChildren != 0 &&
		hasNonDocCommentInSubtree(prev)

	// When the current decl carries a doc comment, or when prev's
	// rendered output may end with a `//` comment (which on reparse
	// migrates into d's doc), fill in a blank line if prev is a
	// Definition or any non-field, non-comment decl. Treating both
	// cases the same keeps the parse/format cycle idempotent: the
	// first-pass output places the blank between let and the comment-
	// commented field even when the AST attaches the comment as a
	// trailing on let (programmatic shape).
	if rel < token.NewSection && prev != nil && (hasDocComment(d) || prevTrailing) {
		if field, ok := prev.(*ast.Field); ok {
			if internal.IsDefinition(field.Label) {
				rel = token.NewSection
			}
		} else if _, ok := prev.(*ast.CommentGroup); !ok {
			rel = token.NewSection
		}
	}

	// Catch-all for the regular-Field case where the NewSection
	// upgrade above didn't fire: prev's render may end with a `//`
	// comment that would otherwise swallow a preFlattened
	// softLineComma. HardLine is the minimum needed.
	if rel < token.Newline && prevTrailing {
		rel = token.Newline
	}

	return relBreakOr(rel, softLineComma)
}

// hasNonDocCommentInSubtree reports whether n's subtree carries any
// comment at a position other than posDoc. Used by declSep to
// detect when prev's render output may end with a `//` comment that
// would otherwise swallow the inter-decl separator if left as a
// soft Line under a parsed wrap. Conservative — comments rendered
// mid-decl don't strictly require the subsequent promotion, but
// the over-promotion is harmless: in broken mode (without
// preFlatten) softLineComma also emits a newline.
func hasNonDocCommentInSubtree(n ast.Node) bool {
	if n == nil {
		return false
	}
	found := false
...
// parsed reports whether n's subtree carries any RelPos — that is,
...
// construction. The parsed-mode wrap (NoWidth + preFlatten) is
// driven by flagShouldWrap, not this function — see [maybeGroup].
func (c *converter) parsed(n ast.Node) bool {
	return c.nodeFlags[n]&flagRelPosInChildren != 0
}
...
// subtree.
//
// When flagShouldWrap is set on n, the subtree renders in parsed
// mode: body is wrapped in NoWidth(Group(preFlatten(body))).
// preFlatten replaces soft Lines with their flat alternates and
// resolves IfBreak to its broken branch, so the formatter cannot
// inject width-driven newlines into the wrapped subtree — only
// HardLine / LitLine / BlankLine survive. NoWidth additionally
// gives every inner Group / Table an unlimited width budget, so
// nested chains, calls, and lists stay flat unless a HardLine
// forces them. The inner Group flips the inherited mode: under
// NoWidth a Group picks flat unless preFlatten left a HardLine in
// the body, so a single-line authored construct stays single-line
// and Tables do not emit column-aligned padding on one line.
//
// Otherwise the subtree renders in programmatic mode: body is
// wrapped in Programmatic(Group(body)). Plain Group(body) would
// suffice when this subtree is at top level, but when it's nested
// inside an enclosing parsed wrap the outer NoWidth would propagate
// inward and freeze the inner Group at flat. Programmatic punches
// a hole in any enclosing NoWidth + preFlatten scope: at render
// time it resets noWidth=false and mode=modeBreak for body, so the
// inner Group does its own flat-vs-broken decision against the
// real width budget. Outside any enclosing parsed wrap,
// Programmatic is a no-op.
//
// flagShouldWrap is set per-subtree by [precompute]: see the
// "uncovered RelPos" pass there and [isWrapEligible] for the set
// of node types where the flag (and therefore this wrap) can fire.
func (c *converter) maybeGroup(n ast.Node, body Doc) Doc {
	// Integrity check: every maybeGroup call site must operate on a
	// type listed in isWrapEligible. The flagShouldWrap precompute
	// pass only flags eligible types, so a call on a non-eligible
	// type would silently never enter the parsed-mode wrap branch
	// even when the subtree carries RelPos. Panic loudly so the
	// drift is caught immediately.
	if !isWrapEligible(n) {
		panic(fmt.Sprintf("pretty: maybeGroup called on non-wrap-eligible type %T; add it to isWrapEligible", n))
	}
	if c.nodeFlags[n]&flagShouldWrap != 0 {
		return NoWidth(Group(preFlatten(body)))
	}
	// No flagShouldWrap: this subtree has no RelPos to preserve,
	// so its layout is the formatter's job. Wrap in Programmatic
	// so that, when this subtree is itself nested inside an
	// enclosing parsed wrap (NoWidth + preFlatten), its inner
	// Group can still do width-driven flat-vs-broken decisions
	// instead of being frozen flat by the outer NoWidth. Outside
	// any enclosing parsed wrap, Programmatic is a no-op.
	return Programmatic(Group(body))
}
...
// docGroups are preserved verbatim: a Group's contents only matter
// in flat mode (in broken mode each soft Line is a newline anyway),
// so there is nothing for preFlatten to rewrite at this level. When
// the caller wraps the result in NoWidth, the renderer additionally
// forces those nested Groups into flat mode at render time. Returns
// d unchanged when the subtree contains nothing to rewrite (checked
// in O(1) via the precomputed needsPreFlatten bit on each Doc).
func preFlatten(d Doc) Doc {
	...
	case *docNoWidth:
		return NoWidth(preFlatten(x.Child))
	case *docAtIndent:
		return AtIndent(x.Prefix, preFlatten(x.Child))
	case *docTable:
...
// relBreakOr returns the break Doc corresponding to a RelPos with a
// caller-supplied fallback for the non-newline cases. Newline always
// produces HardLine and NewSection always produces BlankLine — those
// "hard" cases are universal across every CUE separator decision.
// The soft fallback is what differs between callers:
//
//   - [relBreak] (default): noSepLine — a soft Line whose flat alt is empty.
//   - [elemBreak] / [chainArmSep]: softLineSpace — flat alt is " ".
//   - [declSep]: softLineComma — flat alt is ", ".
//   - [postCommentRows] / [docCommentBlock]: HardLine — these contexts
//     are always rendered in broken mode.
//   - [commentSep]: spaceText (or blankLine when the comment is on its own line).
//
// Centralising the Newline/NewSection mapping here makes adding a new
// separator policy a one-line change and prevents the (recurring) bug
// of one site forgetting NewSection.
...
// relBreak returns the break Doc corresponding to a RelPos: BlankLine
// for NewSection, HardLine for Newline, and a soft Line (empty when
// flat, newline when broken) otherwise. Equivalent to
// `relBreakOr(rel, noSepLine)`.
func relBreak(rel token.RelPos) Doc {
	return relBreakOr(rel, noSepLine)
}
...
// Every expression flows through withComments so doc-position
// comments and post-expression trailing comments are placed
// uniformly. Bracketed / chain expressions whose exprCore already
// handles their interior comments (see selfManagesInteriorComments)
// have those slots skipped inside withComments.
...
func (c *converter) structLit(x *ast.StructLit) Doc {
	...
	case len(x.Elts) == 1 && !hasInterior && c.shouldHug(x.Elts[0]):
		return Cats(lBrace, c.decl(x.Elts[0]), rBrace)
	}
...
func (c *converter) listLit(x *ast.ListLit) Doc {
	...
	case len(unelided) == 1 && !hasInterior && c.shouldHug(unelided[0]):
		return Cats(lBracket, c.expr(unelided[0]), rBracket)
	}
...
// openBreak / row separators under preFlatten — its Nest is unused,
...
type bracketedLayout struct {
	node       ast.Node     // for maybeGroup + flagRelPosInChildren lookup
...
func (c *converter) computeBracketedPolicy(b bracketedLayout) bracketedPolicy {
	...
	//     predecessor, so under preFlatten the parent's openBreak and
	...
	shareIndent := parsed && cleanForShare && !hugFirst && (b.sameLineOpen || b.noElemNewline)
	...
	//   - In parsed mode the body is preFlattened and IfBreak is
	//     resolved statically to its Broken branch — so we have to
	//     decide statically too: only when the closing bracket has
	//     Newline/NewSection RelPos (i.e. lands on its own line).
...
func openBreakDoc(lineHeader, hasInterior bool, firstElem ast.Node, openerRel token.RelPos) Doc {
	...
	case hasInterior:
		return HardLine()
	case firstElem != nil:
		return relBreak(leadingRel(firstElem))
	default:
...
func (c *converter) elemBreak(e ast.Expr) Doc {
	return relBreakOr(leadingRel(e), softLineSpace)
}
...
func (c *converter) chainGroupArms(x *ast.BinaryExpr, arms []chainArm) Doc {
	...
	first = Cat(first, Text(opStr))

	rest := make([]Doc, 0, 2*(len(arms)-1))
	for i := 1; i < len(arms); i++ {
		elem := c.armDoc(arms[i])
		if i < len(arms)-1 {
			elem = Cat(elem, Text(opStr))
		}
		rest = append(rest, chainArmSep(arms[i].expr), elem)
	}
	...
	//
	//   - parsed mode: detect statically. The chain breaks iff any
	//     arm separator carries Newline/NewSection RelPos. The IfBreak
	//     trick doesn't work here because preFlatten (under NoWidth)
	//     resolves IfBreak to its Broken branch, which would force
	...
	chainBreaks := false
	for i := 1; i < len(arms); i++ {
		if leadingRel(arms[i].expr) >= token.Newline {
			chainBreaks = true
			break
		}
	}
	...
	case c.parsed(x):
		wrapped = body
	default:
		wrapped = IfBreak(Nest(body), body)
	}

	return c.maybeGroup(x, Cat(first, wrapped))
}

// chainArmSep returns the separator Doc to use before a chain arm,
// honouring Newline / NewSection RelPos on the arm's expression. A
// soft Line is used otherwise so the surrounding Group decides
// flat-vs-broken on width.
func chainArmSep(e ast.Expr) Doc {
	return relBreakOr(leadingRel(e), softLineSpace)
}
...
func (c *converter) binaryExprPrec(x *ast.BinaryExpr, co, depth int) Doc {
	...
	for _, cg := range slots.suffix {
		if cg.Pos().RelPos() == token.Blank {
			opInline = append(opInline, spaceText, c.commentGroup(cg))
		} else {
	...
	brokenRHS := func(body Doc) Doc {
		...
		return Cats(left, maybeSpace, Text(op), Cats(opInline...), Nest(Cats(inner...)))
	}
	...
	if len(interior) > 0 {
		if s, ok := x.Y.(*ast.StructLit); ok && s.Lbrace.IsValid() {
			...
			return Cats(left, maybeSpace, Text(op), maybeSpace, injected)
		}
	...
	return Cats(left, maybeSpace, Text(op), maybeSpace, right)
}
...
// lands on its own line (parsed mode), and via the IfBreak inside
	...
	for _, e := range x.Elts {
		if lit, ok := e.(*ast.BasicLit); ok && lit.Kind == token.STRING {
			...
			for i, p := range parts {
				...
				if !inBody {
					opener = append(opener, Text(p))
				} else {
					body = append(body, Text(p))
				}
			}
		} else {
			ed := c.expr(e)
...
// Interpolation: the whitespace before the closing `"""` (or `”'`,
// `#"""`, …). Returns "" if no usable prefix is found.
func stripPrefixFromInterp(x *ast.Interpolation) string {
	...
	return tail[:len(tail)-len(trimmed)]
}
...
func (c *converter) fieldRow(f *ast.Field) (Row, []*ast.CommentGroup, int) {
	...
	if len(chain) == 1 {
		...
		for i := len(chain) - 2; i >= 0; i-- {
			inner = Group(Cat(labelDocs[i], Nest(Cat(softLineSpace, inner))))
		}
		key = inner
	}
	...
	// Position >= posTrailingMin: split by Line. Line=true is a
	// same-line trailing `//` comment → the trailingComment cell
	// (column-aligned across rows). Line=false with Newline/NewSection
	// RelPos is visually separate from the field's row — returned
	// to the caller to emit as sibling Raw rows after the field.
	// Line=false without a Newline RelPos is anomalous; fall back to
	// treating it as trailing rather than dropping it.
	for _, cg := range slots.trailing {
		hasComment = true
		switch {
		case cg.Pos().RelPos() == token.Blank:
			trailingComment = joinLines(trailingComment, c.commentGroup(cg))
	...
	if isChain && binHasTrailing {
		val = c.chainTableExpr(bin, binArms, trailingComment)
...
func (c *converter) fieldKey(f *ast.Field) Doc {
	...
	if f.Constraint == token.OPTION || f.Constraint == token.NOT {
		key = Cat(key, Text(f.Constraint.String()))
	}
	if hasColon {
...
func (c *converter) importDecl(x *ast.ImportDecl) Doc {
	...
	for i, s := range x.Specs {
		...
		if i > 0 {
			...
			spec = Cat(relBreakOr(leadingRel(s), HardLine()), spec)
		}
		specs[i] = spec
	}
