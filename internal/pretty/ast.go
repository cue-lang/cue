...
func (c *converter) computeBracketedPolicy(b bracketedLayout) bracketedPolicy {
	...
	shareIndent := parsed && cleanForShare && !hugFirst && b.sameLineOpen
...
func (c *converter) chainGroupArms(x *ast.BinaryExpr, arms []chainArm) Doc {
	...
	return c.maybeGroup(x, Cat(first, Nest(Cats(rest...))))
}
...
// Unlike lists, calls never emit a trailing comma before ')'
// (allowsTrailingComma defaults to false, so policy.WantTrailingComma
// is always false).
func (c *converter) callExpr(x *ast.CallExpr) Doc {
	...
	layout := bracketedLayout{
		...
		// allowsTrailingComma left at its zero value (false): CUE's
		// parser rejects `f(a,)`.
	}
	policy := c.computeBracketedPolicy(layout)
