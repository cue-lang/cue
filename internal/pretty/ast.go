...
const (
	...
	// branch on "does this subtree carry authored layout".
	flagRelPosInChildren
	// flagNonDocCommentInSubtree marks a node whose subtree (including
	// itself) has an attached non-doc comment (Position > 0). Used by
	// declSep to detect when prev's render output may end with a `//`
	// comment that would otherwise swallow the inter-decl separator if
	// left as a soft Line under a parsed wrap.
	flagNonDocCommentInSubtree
...
// the parent up; flagNonDocCommentInSubtree is self-inclusive (the
// node with the comment also gets marked).
//
		...
		func(n ast.Node) bool {
			...
			for _, cg := range ast.Comments(n) {
				if cg.Position != posDoc {
					fromSelf |= flagNonDocCommentInSubtree
					break
				}
			}
...
// RelPos values fall back to softLineComma since declarations need
...
//     decl. Matches cue/format's visual grouping convention.
//   - If prev's subtree carries a `//` comment that may render at
//     the end of its output, the separator is promoted (to NewSection
//     where the rule above also fires, otherwise to Newline). Inside
//     a parsed wrap (NoWidth + preFlatten) a softLineComma would
//     otherwise be rewritten to Text(", ") and the `//` would absorb
//     the next decl. Promoting also keeps the parse/format cycle
//     idempotent when the comment migrates from prev's trailing slot
//     into the next decl's doc on reparse.
func (c *converter) declSep(d ast.Decl, prev ast.Decl) Doc {
	rel := leadingRel(d)

	const trailingMask = flagRelPosInChildren | flagNonDocCommentInSubtree
	prevTrailing := prev != nil && c.nodeFlags[prev]&trailingMask == trailingMask
...
//   - [elemBreak]: softLineSpace — flat alt is " ".
