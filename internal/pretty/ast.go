...
const (
	...
	// Used by [converter.maybeGroup] to gate the NoWidth wrapper:
	// parsed input always has RelPos somewhere, so this flag fires for
	// any subtree that came from the scanner. Programmatically built
	// AST subtrees with no RelPos hints fall through to the width-
	// driven Group path.
...
// whether the AST came from the scanner (parser) rather than being
// built programmatically. Drives the formatter's headline rule:
// parsed source's layout is preserved (NoWidth + preFlatten),
// programmatic source flows through width-driven layout (Group).
...
// maybeGroup wraps body in the right top-level layout primitive.
//
// Parsed source (any descendant carries a RelPos) goes through
// NoWidth(Group(preFlatten(body))). preFlatten replaces soft Line
// nodes with their Alt text and IfBreak with its Broken branch, so
// the formatter cannot inject width-driven newlines at this level —
// only HardLine / LitLine / BlankLine survive. NoWidth makes every
// inner Group/Table see an unlimited budget too, so nested chains,
// calls, and lists also stay flat unless a HardLine forces them.
// The rule: when the AST came from the scanner, the formatter
// respects the user's layout — it adjusts indentation and alignment
// but never adds newlines the user didn't write. The inner Group
// flips the inherited mode: under NoWidth the Group picks flat
// unless preFlatten left a HardLine in the body, so a parsed
// single-line construct inherits modeFlat and Tables don't emit
// column-aligned padding on one line.
//
// Stripped / programmatic source (no RelPos anywhere) goes through
// Group(body) — width-driven layout. Soft Lines break when the body
// doesn't fit, lists/structs/calls/chains explode their elements
// onto one-per-line.
...
// docGroups are preserved verbatim: their flat-mode-only contents
// already carry no soft breaks at this level, so there is nothing
// to rewrite. When the caller wraps the result in NoWidth, the
// renderer additionally forces those nested Groups into flat mode
// at render time. Returns d unchanged when the subtree contains
// nothing to rewrite (checked in O(1) via the precomputed
// needsPreFlatten bit on each Doc).
...
func (c *converter) chainGroupArms(x *ast.BinaryExpr, arms []chainArm) Doc {
	...
	//   - stripped mode: chain might break on width at render time.
	//     Use IfBreak so the Group's flat-vs-broken decision drives
	//     whether the Nest applies.
	chainBreaks := false
	for i := 1; i < len(arms); i++ {
...
func (c *converter) binaryExprPrec(x *ast.BinaryExpr, co, depth int) Doc {
	...
	var opInline []Doc               // posSuffix && Line=true
	var midBlock []*ast.CommentGroup // posSuffix && Line=false
	for _, cg := range slots.suffix {
		if cg.Line {
...
// braced StructLit.
//
// hasTrailingLine reports whether any intermediate node carries a
// same-line `//` trailing comment (Line=true at Position >= posSuffix
// — the "trailing the operator" shape that chainTableExpr aligns in
// a comment column). It's a free byproduct of the comment scan,
// saving callers a separate ancestor walk to make the same decision.
func flattenBinaryChain(x *ast.BinaryExpr) (arms []chainArm, hasTrailingLine bool) {
...
// TrailingComma() when a width-driven break occurs (stripped mode).
