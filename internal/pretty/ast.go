...
func (c *converter) node(n ast.Node) Doc {
	switch n := n.(type) {
	case *ast.File:
		return c.withComments(n, c.declSlice(n.Decls))
	case ast.Expr:
		return c.expr(n)
	case ast.Decl:
		return c.decl(n)
...
func (c *converter) expr(x ast.Expr) Doc {
	...
	if bin, ok := x.(*ast.BinaryExpr); ok && (bin.Op == token.OR || bin.Op == token.AND) {
		return doc // comments already handled
	}
...
func (c *converter) listElem(e ast.Expr, last bool) Doc {
	...
	return Cats(append(append(before, doc, comma), after...)...)
}
...
func (c *converter) binaryExprPrec(x *ast.BinaryExpr, co, depth int) Doc {
	...
	if x.Y.Pos().HasRelPos() && x.Y.Pos().RelPos() >= token.Newline {
		return Group(Cats(left, maybeSpace, Text(op), HardLine(), right))
	}

	return Group(Cats(left, maybeSpace, Text(op), maybeSpace, right))
}
