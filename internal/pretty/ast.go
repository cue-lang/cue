...
type converter struct {
	// newlineInChildren[n] is true iff some strict descendant of n
	// carries a Newline or NewSection RelPos.
	newlineInChildren map[ast.Node]bool
	// commentInSubtree[n] is true iff n or some descendant of n has
	// an attached comment group.
	commentInSubtree map[ast.Node]bool
}
...
func (c *converter) precompute(root ast.Node) {
	c.newlineInChildren = make(map[ast.Node]bool)
	c.commentInSubtree = make(map[ast.Node]bool)
	if root == nil {
		return
	}
	var stack []ast.Node
		...
		func(n ast.Node) bool {
			...
			if n.Pos().IsNewline() {
				for i := len(stack) - 2; i >= 0; i-- {
					if c.newlineInChildren[stack[i]] {
						break
					}
					c.newlineInChildren[stack[i]] = true
				}
			}
			if len(ast.Comments(n)) > 0 {
				for i := len(stack) - 1; i >= 0; i-- {
					if c.commentInSubtree[stack[i]] {
						break
					}
					c.commentInSubtree[stack[i]] = true
				}
			}
			return true
		},
...
func (c *converter) hasAnyCommentInSubtree(n ast.Node) bool {
	return n != nil && c.commentInSubtree[n]
}
...
func (c *converter) hasNewlineInSubtree(n ast.Node) bool {
	...
	return n.Pos().IsNewline() || c.newlineInChildren[n]
}
