...
func (c *converter) file(f *ast.File) Doc {
	...
	if header != nil {
		...
		if body != nil {
			...
			// lineBreakComma.
			parts = append(parts, c.declSep(f.Decls[headerEnd], f.Decls[headerEnd-1]))
		}
	}
	if body != nil {
...
func (c *converter) computeBracketedPolicy(b bracketedLayout) bracketedPolicy {
	...
	//     lands on its own line). TrailingComma()'s [docSwitchMode]
...
func (c *converter) chainGroupArms(x *ast.BinaryExpr, arms []chainArm) Doc {
	...
	// broken branch and force the Nest onto a visually flat chain. So
	// this case must skip the Nest statically.
	//
	// In every other case the separators track the group's mode, so
	// [docSwitchMode] applies the Nest exactly when they break: an
	// authored chain with hard separators forces the group broken
	// (Nest applies), and a programmatic chain's soft separators break
