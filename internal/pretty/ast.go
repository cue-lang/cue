...
func (c *converter) basicLit(x *ast.BasicLit) Doc {
	...
	// quote, repeated on each body line). For an *authored* literal -
	// parsed source, whose position carries a RelPos - that
	// indentation is the user's and we reproduce it verbatim. For a
	// *programmatic* literal with no RelPos - the embedded indent is
	// whatever the producer guessed from its own nesting (the exporter
	// uses its stack depth), which need not match the literal's
	// eventual render position. Re-indent it here so the body and
	// closing quote sit one level deeper than the opening line.
...
// reindentMultilineString re-indents a programmatic multi-line string
// or bytes literal so its body and closing quote render one level
// deeper than the opening line (the opener stays inline after the
// `key: `). value is the full literal text,
// e.g. "\"\"\"\n\tbody\n\t\"\"\"".
//
