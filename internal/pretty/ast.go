...
//
// The hazard arises only when operand starts with an operator
// character, which happens exactly when operand is itself a UnaryExpr:
// it then begins with the first byte of its own operator's text -
// operand `-5` begins with `-`; operands `=~"x"` and `==x` begin with
// `=`. Every other operand begins with a digit, letter, quote, or
// bracket (a binary operand is parenthesised by [wrapForPrecedence], so
// it begins with `(`) and can never merge.
//
