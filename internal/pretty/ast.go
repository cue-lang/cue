...
func (c *converter) chainedBinaryExpr(x *ast.BinaryExpr) *Doc {
	...
	for _, a := range arms {
		var commentDoc *Doc
		// Comments at Position ≤ 1 (before or between operator and
		// operand) are associated with the arm's expression and should
		// be injected into it (e.g., placed inside a struct body).
		// Comments at Position > 1 with Line=true are trailing comments
		// on the operator line (e.g., "a | // comment").
