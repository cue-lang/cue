...
// binaryChainTable lays out a comment-bearing chain as a [docTable] with
// the trailing comments column-aligned. A | or & chain renders its arms
// standalone ([converter.chainTableExpr]); other operators render each
// arm through [converter.binaryOperand] so the precedence-aware spacing
// is preserved ([converter.arithChainTableExpr]).
...
// ...) via [converter.chainTable] so the trailing comments column-align.
// Each arm is rendered through [converter.binaryOperand] with the chain's
// precedence and depth - exactly as [converter.binaryExprPrec] renders
// its operands - so the compact-vs-spaced layout that signals precedence
// is preserved: the head as a left operand (precedence prec), the rest as
// right operands (prec+1). The operator joining the rows is spaced only
// when prec sits below the chain's cutoff, matching binaryExprPrec.
...
//
// We lift such a comment to the argument's trailing-comment column (see
// [converter.listElemRow] and [converter.withCommentsSlots]); the
// node's own renderer skips it (via [nonTrailingPrefix], or
// [flattenBinaryChain] for | / & chains) so it is not also rendered
// inside the node. This is needed only for the node kinds whose
// renderer would otherwise treat a PosPrefix comment as interior
// content - the [nodeManagesInteriorComments] set: a BinaryExpr (the
// candidate bracket is its final operand) or a StructLit / ListLit (the
// bracket is the argument itself).
//
// Whether the comment is interior or trailing is decided by
// [commentTrailsBracket]: a comment genuinely inside an empty `{}`/`[]`
// (e.g. `a & {// commented-out field\n}`) stays interior and is injected
// back into the brackets; everything else is lifted.
