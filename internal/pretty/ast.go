...
func hasAnyCommentInSubtree(n ast.Node) bool {
	found := false
...
func hasNewlineInSubtree(n ast.Node) bool {
	found := false
	ast.Walk(n, func(node ast.Node) bool {
		if found || node == nil {
			return false
		}
		if node.Pos().IsNewline() {
			found = true
			return false
		}
		return true
...
func (c *converter) expr(x ast.Expr) Doc {
	...
	//   - BinaryExpr: all binary expressions are routed through
	//     binaryExpr, which dispatches | and & chains with trailing
	//     comments to chainTableExpr and everything else to
	//     binaryExprPrec.
...
func (c *converter) structLit(x *ast.StructLit) Doc {
	...
	case len(x.Elts) == 1 && len(interior) == 0 &&
		!x.Elts[0].Pos().IsNewline() && !hasAnyCommentInSubtree(x.Elts[0]) &&
		hasNewlineInSubtree(x.Elts[0]):
		// Single-decl struct whose decl's subtree contains an
		// explicit Newline/NewSection RelPos (so the decl WILL
		// render with forced breaks) and carries no comments:
		// "{" hugs the decl directly ("{a: {...}}" rather than
		// "{\n    a: {...}\n}"). The hug exists only to defeat the
		// cascade that would otherwise double-indent the inner
		// content; when the content can fit flat, the standard
		// Group-based path handles width-based breaking properly.
...
func (c *converter) listLit(x *ast.ListLit) Doc {
	...
	case len(unelided) == 1 && len(interior) == 0 &&
		!unelided[0].Pos().IsNewline() && !hasAnyCommentInSubtree(unelided[0]) &&
		hasNewlineInSubtree(unelided[0]):
		// Single-element list whose element's subtree contains an
		// explicit Newline/NewSection RelPos (so the element WILL
		// render with forced breaks) and carries no comments: hug
		// the brackets around the element ("[...{...}]" rather than
		// "[\n    ...{...},\n]"). The hug exists only to defeat the
		// cascade that would otherwise double-indent the broken
		// element; when the element can fit flat, the standard
		// Group-based path handles width-based breaking properly.
	...
	default:
		...
		if len(unelided) > 0 {
			...
			for i, e := range unelided {
				...
				elems = append(elems, c.listElem(e, i == lastElemIdx))
			}
			inner = Cats(elems...)
		}
		inner = c.prependInteriorComments(interior, inner)
...
//
// For the last element, the comma is a TrailingComma (emitted only
// in broken mode) so an inline single-line list does not acquire a
...
func (c *converter) listElem(e ast.Expr, last bool) Doc {
	...
	// Nodes that manage their own comments (same set as expr()): the
	// doc already has before/after comments baked in, so wrapping
	// with withComments here would render them a second time.
	switch e.(type) {
	case *ast.BinaryExpr, *ast.StructLit, *ast.ListLit:
...
func (c *converter) unaryExpr(x *ast.UnaryExpr) Doc {
	...
	// Nodes that manage their own comments (same set as expr()) have
	// already baked them into the returned doc; don't re-wrap.
	switch x.X.(type) {
	case *ast.BinaryExpr, *ast.StructLit, *ast.ListLit:
		return body
	}
...
func (c *converter) callExpr(x *ast.CallExpr) Doc {
	...
	// Single-arg hug: if the sole argument will render with forced
	// breaks (a Newline/NewSection RelPos somewhere in its subtree)
	// and carries no comments, "(" hugs the argument directly
	// ("f([...])" rather than "f(\n    [...],\n)"). Mirrors the
	// single-element hug in listLit and the single-decl hug in
	// structLit. When the argument can fit flat, the standard
	// Group-based path below handles width-based breaking properly.
	if len(x.Args) == 1 &&
		!x.Args[0].Pos().IsNewline() && !hasAnyCommentInSubtree(x.Args[0]) &&
		hasNewlineInSubtree(x.Args[0]) {
	...
	for i, a := range x.Args {
		...
		elems = append(elems, c.listElem(a, i == lastArgIdx))
	}
