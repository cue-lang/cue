...
// # Scope
//
// In scope: rendering an AST to CUE source. Idempotent across the
// parse/format/parse cycle for valid CUE input.
//
// Out of scope: modifying the AST (pretty is read-only); honouring
// real terminal tab stops (a fixed visual indent width is assumed —
// see [Config.IndentWidth]); byte-for-byte parity with cue/format
// (we follow cue/format's conventions where they make sense, e.g.
// the blank-line discipline around definitions and non-field decls,
// but do not aim for exact equivalence).
...
// different type, add that type here.
...
func (c *converter) field(f *ast.Field) Doc {
	...
	for _, cg := range slots.prefix {
		// Position 1: between label and colon — appended to key.
		key = Cat(key, c.commentSep(cg, c.commentGroup(cg)))
	}
	for _, cg := range slots.suffix {
		// Position 2: between colon and value — placed before val on
		// its own line.
...
func (c *converter) fieldRow(f *ast.Field) (Row, []*ast.CommentGroup, int) {
	...
	// Position 1: between label and colon — appended to key inline.
	for _, cg := range slots.prefix {
		key = Cat(key, c.commentSep(cg, c.commentGroup(cg)))
		hasComment = true
	}
