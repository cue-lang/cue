...
type converter struct {
	// chainFieldTrailing carries a Field's same-line trailing comment
	// into a chained | or & value so the chain's Table-based
	// formatter can align it in the same comment column as the chain
	// arms' own trailing comments. Set by fieldRow before formatting
	// the value, consumed by chainTableExpr.
...
func (c *converter) node(n ast.Node) Doc {
	switch n := n.(type) {
	case *ast.File:
		return Cat(Group(c.file(n)), HardLine())
	case ast.Expr:
		return c.expr(n)
	case ast.Decl:
		return c.decl(n)
...
// file renders a *ast.File, preserving the RelPos of the first decl
// when placing leading File-level comments. In particular a blank
// line between a header comment and the package declaration (a very
// common pattern for copyright / license notices) must be preserved:
// withComments always emits a plain HardLine after a Position=0
// comment, which would collapse that blank line into a single
// newline.
func (c *converter) file(f *ast.File) Doc {
	...
	if len(f.Decls) > 0 {
		if pos := f.Decls[0].Pos(); pos.HasRelPos() && pos.RelPos() == token.NewSection {
		...
		if f, ok := decl.(*ast.Field); ok && c.isSimpleField(f) {
			...
			if len(postComments) > 0 {
				...
				for _, cg := range postComments {
					cgSep := HardLine()
					if cg.Pos().HasRelPos() && cg.Pos().RelPos() == token.NewSection {
						cgSep = BlankLine()
					}
...
// leaf and checks the leaf's value.
...
func unchainField(f *ast.Field) (chain []*ast.Field, collapsible bool) {
	...
	for _, cf := range chain[1:] {
		...
		if pos := cf.Pos(); pos.HasRelPos() {
			if rel := pos.RelPos(); rel == token.Newline || rel == token.NewSection {
				return chain, false
			}
		}
	}
...
func (c *converter) expr(x ast.Expr) Doc {
	...
	doc := c.exprCore(x)
	// Nodes that manage their own comments (so we skip withComments):
	//   - BinaryExpr: chained (| and &) via flattenChain; non-chained
	//     via binaryExprPrec.
	//   - StructLit / ListLit: interior comments (e.g. "{ // c }")
	//     must be rendered inside the braces, not after them.
	switch x.(type) {
...
func (c *converter) structLit(x *ast.StructLit) Doc {
	...
	var body Doc
	if len(x.Elts) == 0 && len(interior) == 0 {
		body = Text("{}")
	} else {
		openPos := x.Lbrace
		if len(x.Elts) > 0 {
			openPos = x.Elts[0].Pos()
		} else if len(interior) > 0 {
			openPos = interior[0].Pos()
		}
		openBreak := c.bracketBreak(openPos)
		closeBreak := c.bracketBreak(x.Rbrace)
		// Interior // comments would swallow '}' if rendered flat;
		...
		inner = prependInteriorComments(c, interior, inner)
		body = Group(Cats(
			lBrace,
	...
	return c.applyBeforeAfter(before, body, after)
}
...
func (c *converter) listLit(x *ast.ListLit) Doc {
	...
	case len(unelided) == 1 && len(interior) == 0 &&
		!(unelided[0].Pos().HasRelPos() && unelided[0].Pos().RelPos() >= token.Newline):
		// Single-element list whose element does not demand a
		// newline: hug the brackets around the element ("[...{...}]"
		// rather than "[\n    ...{...},\n]"). The element's own
		// layout (e.g. a struct that breaks internally) renders
		// between '[' and ']' without the list inserting its own
		// breaks, its own Nest, or a trailing comma.
...
// The effective RelPos is the maximum of the element's own RelPos
// and the RelPos of any doc comment attached to it — a doc comment
// with NewSection RelPos means the user wrote a blank line before
// the comment, and that blank line belongs before the comment (i.e.
// between elements), not between the comment and the element.
// Matches declSep's treatment of doc-commented fields.
func (c *converter) elemBreak(e ast.Expr) Doc {
	rel := token.NoRelPos
	if pos := e.Pos(); pos.HasRelPos() {
		rel = pos.RelPos()
	}
...
// binaryExpr converts a BinaryExpr. For | and & chains that carry any
// trailing // comment (either on an intermediate BinaryExpr, attached
// as a Position=1 interior comment on a braced empty-struct arm, or
// passed in via chainFieldTrailing from an enclosing field), we route
// to a Table-based formatter so trailing comments line up in a single
// column. For all other binary expressions, including comment-free
// chains, the per-node binaryExprPrec formatter is used.
func (c *converter) binaryExpr(x *ast.BinaryExpr) Doc {
	if x.Op == token.OR || x.Op == token.AND {
		if c.chainFieldTrailing != nil || chainHasTrailingComment(x) {
			return c.chainTableExpr(x)
		}
	}
...
// attached at Position >= 2 — i.e. after its operator. Interior
// (Position == 1) comments do not count: those represent comments
// written inside a braced RHS and are handled by binaryExprPrec's
// struct-injection path, which produces cleaner output than the
// Table-based chain formatter for that case.
...
func (c *converter) binaryExprPrec(x *ast.BinaryExpr, co, depth int) Doc {
	...
	if len(interior) > 0 {
		if s, ok := x.Y.(*ast.StructLit); ok && s.Lbrace.IsValid() {
			...
			if len(opInline) > 0 || len(midBlock) > 0 ||
				(x.Y.Pos().HasRelPos() && x.Y.Pos().RelPos() >= token.Newline) {
	...
	breakRHS := (x.Y.Pos().HasRelPos() && x.Y.Pos().RelPos() >= token.Newline) ||
		len(opInline) > 0 || len(midBlock) > 0
...
// that column-aligns across arms. Used only when there is at least one
// trailing comment somewhere in the chain (or an enclosing field has
// passed one via chainFieldTrailing). Without comments, the plain
// binaryExprPrec path gives a cleaner result.
func (c *converter) chainTableExpr(x *ast.BinaryExpr) Doc {
	fieldTrailing := c.chainFieldTrailing
	...
	if fieldTrailing != nil {
		...
		if last.comment == nil {
			last.comment = fieldTrailing
		} else {
			last.comment = Cats(last.comment, HardLine(), fieldTrailing)
		}
	}
...
// field, preserving their original vertical position.
func (c *converter) fieldRow(f *ast.Field) (Row, []*ast.CommentGroup) {
	...
	// trailing comments, pass the field's own trailing comment into
	// the chain's Table so it column-aligns with the chain's arm
	// comments. Otherwise keep it as a separate cell in the field row
	// (so it aligns with simple fields' trailing comments).
	if trailingComment != nil {
		if bin, ok := leaf.Value.(*ast.BinaryExpr); ok &&
...
func valNeedsLeadingBreak(v ast.Expr) bool {
	...
	rel := pos.RelPos()
	return rel == token.Newline || rel == token.NewSection
}
...
func prependInteriorComments(c *converter, interior []*ast.CommentGroup, inner Doc) Doc {
	if len(interior) == 0 {
		return inner
	}
...
// comments after.
func (c *converter) applyBeforeAfter(before []*ast.CommentGroup, body Doc, after []*ast.CommentGroup) Doc {
	if len(before) == 0 && len(after) == 0 {
		return body
	}
	parts := make([]Doc, 0, len(before)*2+1+len(after)*2)
	for _, cg := range before {
		parts = append(parts, c.commentGroup(cg), HardLine())
	}
	parts = append(parts, body)
