...
// Delegates to withComments which handles all comment positions.
func (c *converter) exprComments(n ast.Node, body *Doc) *Doc {
	return c.withComments(n, body)
}
...
func (c *converter) fieldRow(f *ast.Field) Row {
	...
	for _, cg := range ast.Comments(f) {
		...
		case cg.Line || cg.Position >= 3:
			hasComment = true
			cd := c.commentGroup(cg)
			// Same-line trailing comments go into TrailingComment for
			// cross-row alignment. Comments with Newline/NewSection RelPos
			// go on their own line appended to the value.
			if cg.Pos().HasRelPos() && cg.Pos().RelPos() >= token.Newline {
				val = Cat(val, c.commentSep(cg, cd))
			} else {
...
// fieldVal builds the value portion of a field without doc-comment nesting.
func (c *converter) fieldVal(f *ast.Field) *Doc {
	return c.fieldValDoc(f, false)
}
