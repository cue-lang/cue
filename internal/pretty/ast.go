...
func (c *converter) computeBracketedPolicy(b bracketedLayout) bracketedPolicy {
	...
	// is left to its own RelPos and is not forced here.  leadRel is
	// the first element's leading RelPos, or the opener's own when the
	// body is empty.
	leadRel := b.openerRel
	if b.firstElem != nil {
...
func (c *converter) indexExpr(x *ast.IndexExpr) Doc {
	...
	// b)[i]`).
	return c.maybeGroup(x, Cats(
		wrapForPrecedence(c.expr(x.X), x.X, token.HighestPrec),
		lBracketLit,
...
func (c *converter) sliceExpr(x *ast.SliceExpr) Doc {
	...
	// parens.
	return Cats(wrapForPrecedence(c.expr(x.X), x.X, token.HighestPrec), lBracketLit, low, colonLit, high, rBracketLit)
}
