...
func flattenDisjunction(x *ast.BinaryExpr) []disjunct {
	...
	walkLeft = func(e ast.Expr) {
		if bin, ok := e.(*ast.BinaryExpr); ok && bin.Op == token.OR {
			walkLeft(bin.X)
			// Collect right operands from left-associative chain.
			walkRight(bin.Y, bin.OpPos, &result)
			return
		}
	...
	walkLeft(x.X)
	walkRight(x.Y, x.OpPos, &result)
	return result
}

func walkRight(e ast.Expr, opPos token.Pos, result *[]disjunct) {
	if bin, ok := e.(*ast.BinaryExpr); ok && bin.Op == token.OR {
		// Right-associative: a | (b | c) — flatten b then c.
		walkRight(bin.X, opPos, result)
		walkRight(bin.Y, bin.OpPos, result)
		return
	}
	*result = append(*result, disjunct{expr: e, opPos: opPos, exprPos: e.Pos()})
}
...
func (c *converter) disjunction(x *ast.BinaryExpr) *Doc {
	...
	// Build the doc for each disjunct with its separator.
	var parts []*Doc
	...
	for i, d := range disjs {
		...
		// Flat: " | disjunct"; broken: " |" + newline + disjunct.
		// The "|" stays at end of previous line so the output re-parses
		// correctly ("|" is infix and needs its LHS visible).
		var sep *Doc
		switch {
...
func (c *converter) comprehension(x *ast.Comprehension) *Doc {
	...
	for _, clause := range x.Clauses {
		parts = append(parts, c.clause(clause))
	}
...
func (c *converter) fieldRow(f *ast.Field) Row {
	...
	// We attach them to the Row's Sep, which is rendered before the row.
	var docComment *Doc
...
func (c *converter) appendTrailingComments(n ast.Node, d *Doc) *Doc {
	for _, cg := range ast.Comments(n) {
		// Line comments, or position-based comments after the value
		// (position >= 3 for fields: <0>Label <1>":" <2>Expr <3>"," <4>).
		if cg.Line || (!cg.Doc && cg.Position >= 3) {
			d = Cats(d, Text(" "), c.commentGroup(cg))
		}
	}
	return d
}
...
func (c *converter) withComments(n ast.Node, body *Doc) *Doc {
	...
	for _, cg := range cgs {
		...
		} else if cg.Line || (!cg.Doc && cg.Position >= 3) {
			// Line comments and position-based comments after the value.
			after = append(after, Text(" "), cdoc)
		} else if cg.Position == 0 {
			// Position 0: before the first token, treat like doc.
			before = append(before, cdoc, HardLine())
		}
