...
// per-subtree:
//
//   - authored-mode: the formatter respects the authored layout.
//     The converted [Doc] is wrapped in a [docInfiniteWidth] scope
//     (see "finite vs infinite width" in the package doc), so
//     width-driven [docGroup] breaks are suppressed and only hard
//     line breaks produce newlines.
//
//   - programmatic-mode: width-driven Wadler-Lindig. The converted
//     [Doc] is wrapped in a [docFiniteWidth] subtree so that, when
//     the subtree is itself nested inside a [docInfiniteWidth]
//     scope, its inner [docGroup]s still pick flat-vs-broken
//     against the real width budget rather than inheriting the
//     outer unlimited budget.
//
...
// on. See the package doc for [docTable] semantics and render.go
...
//     it's then eligible for table alignment as `key: value`. If
//     not, it's "complex": multi-line values, doc comments on the
//     value, values on their own lines, or a non-collapsible chain.
//
// # Limits of Scope
//
...
func (c *converter) computeBracketedPolicy(b bracketedLayout) bracketedPolicy {
	...
	//     [docSwitchMode] inside TrailingComma() decides at render
	//     time, so we set WantTrailingComma=true unconditionally and
	//     let the [docSwitchMode] resolve to "" when the bracket fits
	//     flat.
	//   - In authored-mode the body passes through asInfiniteWidth and
	//     the [docSwitchMode] is resolved statically to its broken
	//     branch - so we
...
// inside TrailingComma() when a width-driven break occurs (programmatic
// mode).
		...
		if lit, ok := e.(*ast.BasicLit); ok && lit.Kind == token.STRING {
			...
			for i, p := range parts {
				if i > 0 {
					...
					// starts with the strip prefix uses [docLineBreakHard] -
					...
					// AtIndent's prefix; we render `p` verbatim. This
					// preserves exactly what the user wrote - we never
					// *add* indentation to a line that didn't have it.
					inBody = true
...
func (c *converter) fieldRow(chain []*ast.Field) (Row, []*ast.CommentGroup, int) {
	...
	// complex ([docGroup]/[docTable]/[docLineBreakBare] inside) we leave MergedFirstCell
...
func (c *converter) withComments(n ast.Node, body Doc) Doc {
	...
	for _, cg := range trailing {
		...
		// brackets/braces in flat-mode. SwitchMode(nil, HardLine()) is
