...
func (c *converter) declSep(d ast.Decl, prev ast.Decl) Doc {
	...
	// decl is a definition or a non-field, fill in a blank line —
	// but only if no leading layout intent was specified at all
	// (rel == NoRelPos). Authored intent (Newline / NewSection /
	// Blank) is respected as-is. The upgrade is a heuristic for
	// "the user/programmer didn't say anything, default to a
	// section break"; when they did say something, we don't
	// override it.
	if rel == token.NoRelPos && prev != nil && hasDocComment(d) {
		if field, ok := prev.(*ast.Field); ok {
			...
			// Non-field decls (let, embed, comprehension, etc.)
			// get a section break before a doc-commented sibling.
			rel = token.NewSection
		}
	}
...
//     hard line break. The comment must not be inlined — CUE's
//     `//` runs to end-of-line, so squashing onto a shared line
//     would absorb subsequent tokens. We pick a single newline
//     rather than a blank line because declSep no longer forces
//     a section upgrade for plain Newline RelPos (see declSep);
//     emitting a blank line preemptively would diverge from the
//     round-trip output where the parser re-attaches the comment
//     as a doc of the next decl with Newline RelPos.
