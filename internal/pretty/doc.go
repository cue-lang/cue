...
// docNoWidth marks Child as not subject to the line-width limit.
// Applied (together with preFlatten) at the smallest wrap-eligible
// subtree of an AST that carries authored layout — the user has
// already decided where lines break, so the formatter should not
// insert additional width-driven breaks. The no-width state
// propagates through every Doc constructor: Groups inside a
// docNoWidth see an unlimited budget too, so a chain or call
// inside the wrapped subtree stays flat unless a HardLine forces a
// break. To opt back into multi-line layout the user must place a
// Newline RelPos somewhere inside the inner construct.
//
// In a mixed AST (programmatic surroundings with a parsed pocket,
// or vice versa) docNoWidth scopes parsed-mode treatment to the
// pocket; programmatic siblings outside the wrap retain width-
// driven Wadler-Lindig layout. A programmatic pocket nested INSIDE
// a parsed wrap is restored to width-driven layout via
// [docProgrammatic]. See ast.go's package overview for the wrap-
// site selection algorithm.
type docNoWidth struct {
	docBase
	Child Doc
}

// docProgrammatic marks Child as a programmatic-mode subtree. It is
// opaque to preFlatten — soft Lines and IfBreak alternates inside
// Child are preserved for render-time decisions — and at render time
// the no-width flag is reset to false and the rendering mode is
// reset to broken for Child's subtree, so an enclosing parsed wrap
// (NoWidth + preFlatten) doesn't freeze Child's inner Groups at
// flat. Used at wrap-eligible AST nodes that have no RelPos
// descendants of their own: when such a node sits inside a parsed
// wrap, its content's layout is the formatter's job (width-driven),
// not the user's. Outside any parsed wrap, docProgrammatic is a
// no-op.
type docProgrammatic struct {
	docBase
	Child Doc
}

// docIfBreak emits Broken when in broken mode and Flat when in flat
// mode.
...
// are rendered inline separated by their Sep. In broken mode, columns
// are padded to their maximum widths.
...
type Row struct {
	...
	// second cell when the row's flat width would exceed the
	// available budget. Cells beyond the second stay on the second
	// cell's line. Used by field rows so that a wide `key: value`
	// pair can fall back to `key\n\tvalue` instead of overflowing
	// the line. List/call rows leave it false because their first
	// cell is the value itself.
	//
	// The break-point indent is determined dynamically: the renderer
	// reads its lastIndent state after rendering cell 0, so when a
	// chain Group inside cell 0 partially breaks the value lands
	// one level deeper than the chain's actual deepest break (rather
	// than at a fixed depth). When nothing inside cell 0 breaks the
	// value goes to indent + 1.
	AllowRowBreak bool
...
// broken mode. A docGroup is opaque to preFlatten, so flatten stays
...
// NoWidth returns a Doc that suppresses width-driven line breaks in
// d's rendering. The no-width state propagates through every nested
// constructor: Groups, Tables, Cats, Nests, IfBreaks all inherit it.
// Only HardLine / LitLine / BlankLine can introduce a newline inside
// the subtree, unless [Programmatic] is used to escape the no-width
// scope.
func NoWidth(d Doc) Doc {
	if d == nil {
		return nil
	}
	return &docNoWidth{
...
// Programmatic returns a Doc that renders d in programmatic
// (width-driven) mode regardless of any enclosing NoWidth +
// preFlatten parsed wrap. d's soft Lines and IfBreak alternates are
// preserved (preFlatten leaves docProgrammatic verbatim because its
// flatten is false), and at render time the inner mode is reset so
// the innermost Group does its own flat-vs-broken decision against
// the real width budget.
//
// Outside an enclosing NoWidth scope, Programmatic is a no-op. See
// docProgrammatic.
func Programmatic(d Doc) Doc {
	if d == nil {
		return nil
	}
	return &docProgrammatic{Child: d}
}

// docAtIndent anchors Child at a fixed visual indent independent of
// the surrounding nest level. Newlines emitted inside Child render
// as Prefix followed by the renderer's normal Indent times the local
// nest level — Prefix is verbatim, so any whitespace mixture (tabs,
// spaces, or a combination) is honoured exactly.
//
// Used for multi-line string bodies: the strip prefix (whitespace
// before the closing quote) is what the user wrote, and we preserve
// it byte-for-byte rather than trying to remap it to nest levels.
...
// AtIndent returns a Doc that renders d with newlines emitting
// prefix verbatim before the nest-driven indentation. Inside d the
// nest level resets to 0, so a Nest one level deep emits prefix +
