...
// Used at AST nodes whose subtrees carry Newline or NewSection RelPos:
// the user has already decided where lines break, so the formatter
// should not insert additional width-driven breaks. The no-width
// state propagates through every Doc constructor — Groups inside a
// docNoWidth see an unlimited budget too, so a chain or call inside
// a RelPos-bearing struct stays flat unless a HardLine forces a
