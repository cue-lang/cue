...
type Row struct {
	...
	// before the field. It is rendered before the key in broken mode
	// and does not participate in key-width measurement. Its presence
	// forces the enclosing group to break (a // comment cannot appear
	// inline without swallowing the rest of the line).
	//
	// Example input:
	//
	...
	//   x: 1
	//
	// Here DocComment holds "// this is a doc comment".
	DocComment *Doc

	// TrailingComment holds a same-line // comment that appears after
	// the field's value. It is rendered after the value with padding
	// so that trailing comments are vertically aligned across rows.
	//
	// Example input:
	//
	//   x: "hello"   // greeting
	//   y: "goodbye" // farewell
	//
	// Here TrailingComment holds "// greeting" and "// farewell"
	// respectively, and the renderer pads between the value and
	// comment so the "//" columns line up.
	TrailingComment *Doc

	// HasComment is true if the row contains any // comment (whether
	// in DocComment, TrailingComment, or embedded in Key/Val at
	// positions 1-2). When true, the enclosing group is forced to
	// break because a // comment runs to end of line and would
	// swallow subsequent tokens (like a closing brace) in flat mode.
	HasComment bool
}
