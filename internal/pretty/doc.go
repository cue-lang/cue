...
//     [docSwitchMode] can contain hard line breaks (e.g for trailing
//     comments) which would then cause the flat-fit test to fail even
//     with infinite width. Consequently, we always want the
//     broken-mode content of [docSwitchMode] nodes.
//
	...
	case *docTable:
		...
		for i, row := range d.rows {
			...
			rows[i] = Row{
				...
				AllowRowBreak:   row.AllowRowBreak,
				MergedFirstCell: asInfiniteWidth(row.MergedFirstCell),
			}
		}
		return Table(rows)
	}
