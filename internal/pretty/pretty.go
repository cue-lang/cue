...
type Config struct {
	...
	// Use "\t" for tabs or a string of spaces (e.g. "    ") for space-based
	// indentation. If empty, defaults to "\t".
	Indent string

	// Width is the target line width for line-breaking decisions.
...
func (cfg *Config) Node(n ast.Node) []byte {
	c := &converter{}
	doc := c.node(n)
	return Render(cfg.width(), cfg.indent(), doc)
}

// Node formats an AST node using default settings (tab indent, 80-column width).
func Node(n ast.Node) []byte {
	cfg := &Config{}
	return cfg.Node(n)
}
