...
// Node formats an AST node as pretty-printed bytes. It returns a non-nil
// error if the node could not be formatted faithfully (for example, it
// contains an invalid identifier in expression position); the returned
// bytes are still a best-effort rendering.
