...
func TestPostfixTrailingComment(t *testing.T) {
	// A trailing comment on the operand of a PostfixExpr should appear
	// after the operator, not between the operand and the operator.
	// Built programmatically because the parser can't produce a trailing
	// comment on the operand of ?.
	...
	testAST(t, f, &pretty.Config{Width: 80}, `x: value? // trailing`)
}
