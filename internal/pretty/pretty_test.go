...
func TestTableAlignment(t *testing.T) {
	rows := []pretty.Row{
		...
		{Cells: []*pretty.Doc{pretty.Text("barbaz:"), pretty.Text("2")}, Sep: pretty.SoftComma()},
		{Cells: []*pretty.Doc{pretty.Text("x:"), pretty.Text("3")}, Sep: pretty.SoftComma()},
	}
...
func TestTableAlignmentFlat(t *testing.T) {
	rows := []pretty.Row{
		...
		{Cells: []*pretty.Doc{pretty.Text("b:"), pretty.Text("2")}, Sep: pretty.SoftComma()},
	}
...
func TestUnicodeFieldAlignment(t *testing.T) {
	// Multi-byte rune field names: alignment uses rune count, not byte count,
	// so values line up visually.
	input := `
{
	"💩": 3
	y:4
}`[1:]
	// "💩": is 4 runes (", 💩, ", :). y: is 2 runes. Pad = 4-2 =
	// 2. With 1 space separator = 3 spaces. Note: terminal display
	// width of 💩 varies (often 2 columns), but we count runes like
	// most code formatters.
	testPretty(t, input, 80, "\t", `
{
	"💩": 3
	y:   4
}`[1:],
		// Without RelPos the struct flattens to one line.
		`{"💩": 3, y: 4}`)
}
...
func TestCommentInsideConjunctionStruct(t *testing.T) {
	// A comment inside a struct that is the RHS of a conjunction.
	// The parser attaches the comment to the BinaryExpr at Position 1,
	// but the printer injects it inside the struct body so it re-parses
...
foo: #X &
	{
		...
		// Without RelPos, blank lines between declarations are lost
		// (NewSection RelPos is stripped).
		`
package p
#X: {a?: int}
foo: #X &
...
func TestCommentInsideConjunctionStructSameLine(t *testing.T) {
	// A comment on the same line as { inside a conjunction's RHS struct.
	// The parser attaches it to the BinaryExpr at Position 1 with Line=true,
	// but it should still be injected into the struct body (not treated as
	// a trailing comment on the operator line).
	input := `foo: #X & {//a: 5
}
`
	testPretty(t, input, 80, "\t", `
...
func TestChainCommentAlignment(t *testing.T) {
	// Trailing comments on chain arms should be aligned. The struct arm
	// (which renders multi-line) should NOT inflate the comment column.
	// The field's trailing comment ("// here") should be integrated into
	// the chain table for alignment with the chain's other comments.
	input := `
#X: {a?: int}
foo: #X |        // comment
    {
...
func TestBlankLineAfterDefinition(t *testing.T) {
	...
	// Table alignment does not cross blank lines, so #Foo: and x:
	// are in separate tables and x: is not padded.
	input := `
{
	#Foo: string
	// doc comment
...
func TestDisjunctionComments(t *testing.T) {
	...
	// The last comment ("fourth letter") is a field-level trailing comment
	// that is passed into the chain table for alignment with the other
	// trailing comments.
	input := `
out: "A" | // first letter
    "B" | // second letter
