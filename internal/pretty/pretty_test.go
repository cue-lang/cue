...
func TestDocCombinators(t *testing.T) {
	...
	a := pretty.StringLit("a")
	b := pretty.StringLit("b")
	c := pretty.StringLit("c")
	x := pretty.StringLit("x")
	lBrace := pretty.StringLit("{")
	rBrace := pretty.StringLit("}")
	...
	tests := []struct {
		...
		{
			name: "cats_with_nil",
			doc:  pretty.Cats(a, nil, b, nil, c),
			want: "abc",
		},
		{
			name: "sep",
			doc:  pretty.Sep(pretty.StringLit(", "), a, b, c),
			want: "a, b, c",
		},
		{
			name:  "group_fits",
			doc:   pretty.Group(pretty.Cats(a, pretty.SoftLineSpace(), b)),
			width: 80,
			want:  "a b",
		},
		{
...
b`[1:],
		},
		{
			...
			doc:    pretty.Group(pretty.Cats(lBrace, pretty.Nest(pretty.Cat(pretty.SoftLineSpace(), x)), pretty.SoftLineSpace(), rBrace)),
			width:  3,
			indent: "\t",
			want: `
{
	x
		...
		},
		{
			...
			doc:  pretty.Group(pretty.Cats(a, pretty.HardLine(), b)),
			want: `
a
b`[1:],
		},
		{
			...
			doc:   pretty.Group(pretty.Cats(a, pretty.SwitchMode(pretty.StringLit("!"), pretty.StringLit("?")), b)),
			width: 80,
			want:  "a?b",
		},
		{
			name:  "switch_mode_broken",
			doc:   pretty.Group(pretty.Cats(a, pretty.SwitchMode(pretty.StringLit("!"), pretty.StringLit("?")), pretty.SoftLineSpace(), b)),
			width: 2,
			want: `
a!
b`[1:],
		...
		{
			name: "blank_line",
			doc:  pretty.Cats(a, pretty.BlankLine(), b),
			want: `
a
    ...
    x
		...
		},
		{
			...
			doc: pretty.Table([]pretty.Row{
				{Cells: []pretty.Doc{pretty.StringLit("foo:"), a}},
				{Cells: []pretty.Doc{pretty.StringLit("barbaz:"), b}, Sep: pretty.SoftLineComma()},
				{Cells: []pretty.Doc{pretty.StringLit("x:"), c}, Sep: pretty.SoftLineComma()},
			}),
			want: `
foo:    a
barbaz: b
...
[
	...
	[
		...
		},
		{
			...
			doc:    pretty.Group(pretty.Cats(lBrace, pretty.Nest(pretty.Cat(pretty.HardLine(), x)), pretty.HardLine(), rBrace)),
			width:  10,
			indent: "",
			want: `
{
...
func TestTxtar(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(name, func(t *testing.T) {
			...
			// upgrades) set annotate: true in their config so the
			// pre-pass runs - mirroring how cue/format invokes pretty.
			// Tests that target the pretty layer in isolation leave it
			// off, so the renderer is exercised against the raw AST.
...
// sectionData returns the data for the named section, fataling if not
// found.
...
// mustFormat formats n with cfg, failing the test on a formatting error.
...
// printing the output of a print-parse cycle produces the same
// result.
...
// stripRelPos removes all parser-derived layout intent from the AST:
// every token.Pos's RelPos is reset to NoRelPos, its scanner-derived
// comma and "scanned" bits are cleared, and every CommentGroup's Line
// flag is reset to false. cg.Line is a redundant projection of
// Slash.RelPos = Blank (the parser sets the two together - see
// commentSep's docstring), so a "programmatic AST" simulation must
// clear it alongside RelPos. The comma/scanned bits drive
// [listOmitsCommas]'s source-preserving comma style; a genuine
// programmatically-built AST has neither set, so clearing them keeps
// this simulation faithful (lists keep their commas). Position and Doc
// are structural / semantic and are left intact.
...
{a: 1
	...
	b: 2}`[1:],
		...
		{
			// Exercises node()'s ast.Decl dispatch branch: a bare Field
			// decl passed directly rather than wrapped in a File.
			name: "node_as_decl",
			node: &ast.Field{
				Label: &ast.Ident{Name: "a"},
				Value: &ast.BasicLit{Kind: token.INT, Value: "1"},
		...
		},
		{
			...
			// separators remain soft (not converted by asInfiniteWidth
			// to ", "), proving that the outer is not in authored-mode.
...
after: 3`[1:],
		...
		{
			// Synthesised StructLit (Lbrace.IsValid() == false) holding
			...
			//
			// If [wrapEligibility] returned authored == true for the
			...
			node: func() *ast.File {
				// Lbrace/Rbrace deliberately left as NoPos. The parser
				// never produces this shape; only programmatic AST
				// construction does.
				inner := &ast.StructLit{
					Elts: []ast.Decl{
						&ast.Field{
...
x: {
	...
	b: 2
		...
		},
		{
			...
			// branch wins. style.Config.Annotate's B3 is what supplies
		...
		{
			// Same-line (Line=true) trailing comments attached to a
			// leaf value are lifted to the field's trailing-cell
			// column so they column-align with sibling fields'
			// trailing comments. [internal/core/export] attaches
			// error annotations to the value (e.g. an *ast.BottomLit
			// with Position=2, Line=true) - structurally different
			// from the parser, which attaches to the field at
			// Position>=3 - but both should render in the same
			// aligned trailing column.
			//
			// This case also exercises co-existence with a doc
			// comment on the same leaf value: the doc comment must
			// stay attached to the value (renders before it), while
			// the same-line comment is lifted.
...
err: _|_ // boom`[1:],
		},
		{
			...
			// AND its leaf value also carries one, both must survive:
			// they're joined into the same trailing-cell.
...
x: 1 // on-field
     // on-value`[1:],
		...
		},
		{
			...
			// a no-RelPos field has been spliced. The surrounding
			// StructLit is wrapped in [infiniteWidth] so its authored
			// children are preserved as written, but the spliced field -
			// which has no RelPos descendants of its own - is wrapped in
			// a [finiteWidth] boundary that escapes the infinite-width
			// and asInfiniteWidth layers for its content. Width-driven
			// Wadler-Lindig therefore applies inside the spliced field:
			// its wide chain breaks, and render-time row segmentation
			// isolates the broken row so the surrounding fields no
			// longer share alignment with it.
			//
			// The "programmatic" struct sitting alongside has the same
			// content but no RelPos anywhere - its StructLit is its own
			// programmatic-mode subtree. The output is identical: option
			// (B) treats programmatic content the same way regardless of
			// whether it's nested inside an [infiniteWidth] wrap or
			// stands alone.
...
programmatic: {
	...
	z: 9
		...
		},
		{
			...
			// and a doc-commented field - extended to fire when prev's
			// trailing `//` is going to migrate into the next decl's doc
			// on reparse, which keeps the parse/format cycle idempotent.
...
bar: 2`[1:],
		...
		{
			// A chain with a trailing comment whose Slash RelPos !=
			// Blank (and cg.Line=false) must not lose the comment.
