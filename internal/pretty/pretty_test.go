...
//
// TODO: cg.Line is a projection of Slash.RelPos = Blank (the parser
// sets the two together — see commentSep's docstring) and is, by
// the same argument, parser-derived layout intent that should be
// stripped here. We don't strip it today because cg.Line is read
// in ~10 sites across ast.go to make layout decisions that aren't
// yet derived from RelPos alone; stripping it surfaces several
// regressions, including outright comment loss in chain/disjunction
// rendering. Migrating those sites to consult RelPos is a separate
// refactor.
