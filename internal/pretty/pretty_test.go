...
x: a | // trailing
	b`[1:],
		...
		{
			// A comma-free list (Scanned Lbrack, no comma bits on its
			// elements) whose group is forced broken by a multi-line
			// element must still separate same-line elements with a
			// comma.  This is the shape the jsonschema encoder produces:
			// it builds the list via ast.NewList but copies Scanned
			// positions from the source, so listOmitsCommas sees a
			// comma-free list and the multi-line struct makes it
			// authored ([infiniteWidth]). The elements carry NoRelPos,
			// so the separators collapse to the same line; the comma
			// must travel with the separator (not a switchMode that
			// follows the broken group mode) or we emit `[number {...}
			// ...]` - invalid CUE. The parser can never produce this AST
			// (it always inserts a comma or a newline between elements),
			// so we build it directly.
			name: "comma_free_same_line_elements_keep_comma",
			node: func() *ast.File {
