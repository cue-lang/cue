...
[
	...
	2,
		...
		},
		{
			...
			doc:    pretty.Group(pretty.Cats(pretty.Text("{"), pretty.Nest(pretty.Cat(pretty.HardLine(), pretty.Text("x"))), pretty.HardLine(), pretty.Text("}"))),
			width:  10,
			indent: "",
			want: `
{
...
{a: 1
	...
	b: 2}`[1:],
		...
		{
			// Per-subtree wrap: a parsed StructLit (RelPos on its
			// inner fields) sits inside an otherwise programmatic
			// AST. The smallest wrap-eligible ancestor is the
			// StructLit itself, so only it gets NoWidth+preFlatten;
			// the surrounding File and sibling Fields go through
			// width-driven layout. The HardLines inside the parsed
			// StructLit propagate up and break the outer Group, so
			// each top-level decl lands on its own line — but the
			// inter-decl separators remain soft (not preFlattened
			// to ", "), proving that the outer is not in
			// parsed-mode.
			name: "wrap_per_subtree_parsed_struct_in_programmatic",
			node: func() *ast.File {
				inner := &ast.StructLit{
					Lbrace: token.Blank.Pos(),
...
after: 3`[1:],
		...
		{
			// Per-subtree wrap, inverse of the previous test: a
			// parsed struct (RelPos on its existing fields) into
			// which a no-RelPos field has been spliced. The
			// surrounding StructLit is wrapped in parsed mode
			// (NoWidth + preFlatten) so its parsed children are
			// preserved as authored, but the spliced field — which
			// has no RelPos descendants of its own — is wrapped in
			// a Programmatic boundary that escapes NoWidth and
			// preFlatten for its content. Width-driven Wadler-
			// Lindig therefore applies inside the spliced field:
			// its wide chain breaks, and render-time row
			// segmentation isolates the broken row so the
			...
			// inside a parsed wrap or stands alone.
			name: "wrap_per_subtree_programmatic_field_in_parsed_struct",
			node: func() *ast.File {
...
programmatic: {
	...
	z: 9
		...
		{
			// Regression: a `//` trailing comment on a decl, in a
			// parsed wrap, must not be followed on the same line by
			// the next decl. Previously the inter-decl softLineComma
			// was preFlattened to Text(", ") and emitted literally
			// after the comment — the `//` then ran to end of line
			// absorbing `bar: 2` onto the comment's line. declSep
			// now promotes the soft sep when prev's subtree carries
			// a non-doc comment (and the parent is in a parsed
			// wrap), so the next decl lands on a fresh line.
			//
