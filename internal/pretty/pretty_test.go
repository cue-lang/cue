...
func TestTxtar(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(name, func(t *testing.T) {
			...
			gotRelPos := trimTrailingNewline(string(cfg.Node(syntax)))
			...
			stripRelPos(syntax)
			styleCfg.Annotate(syntax)
			gotNoRelPos := trimTrailingNewline(string(cfg.Node(syntax)))
...
func checkIdempotent(t *testing.T, formatted string, cfg *pretty.Config) {
	...
	reformatted := trimTrailingNewline(string(cfg.Node(syntax)))
...
// every token.Pos's RelPos is reset to NoRelPos, and every
// CommentGroup's Line flag is reset to false. cg.Line is a redundant
// projection of Slash.RelPos = Blank (the parser sets the two
// together - see commentSep's docstring), so a "programmatic AST"
// simulation must clear it alongside RelPos. Position and Doc are
// structural / semantic and are left intact.
...
x: a | // trailing
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Helper()
			got := trimTrailingNewline(string(tt.cfg.Node(tt.node)))
