...
func (r *renderer) renderTableInMode(indent int, basePrefix string, m mode, noWidth bool, rows []Row) {
	...
	// In noWidth mode (parsed source — RelPos present) inner Groups
	// see an unlimited budget and never break on width, so a row that
	// looks wider than `avail` will still render flat at its full
	// width. Segmenting alignment around that row would just visibly
	// break the user's column layout for no gain — the over-budget
	// row already overflows whether we segment or not. Skip the
	// width gate so longestAlignedPrefix only segments at HardLines.
	if noWidth {
		avail = math.MaxInt
	}
...
func measureCell(doc Doc) (width int, broken, canWrap bool) {
	...
	for len(stack) > 0 {
		...
		case *docHard, *docLitLine:
			return 0, true, true
		case *docCat:
			stack = append(stack, frame{d: d.Right, inGroup: e.inGroup})
			stack = append(stack, frame{d: d.Left, inGroup: e.inGroup})
		case *docNest:
			stack = append(stack, frame{d: d.Child, inGroup: e.inGroup})
		case *docGroup:
			stack = append(stack, frame{d: d.Child, inGroup: true})
		case *docNoWidth:
			stack = append(stack, frame{d: d.Child, inGroup: e.inGroup})
		case *docIfBreak:
			canWrap = true
			if e.inGroup {
				stack = append(stack, frame{d: d.Flat, inGroup: e.inGroup})
			} else {
				stack = append(stack, frame{d: d.Broken, inGroup: e.inGroup})
			}
		case *docTable:
			canWrap = true
			...
			for i := len(d.Rows) - 1; i >= 0; i-- {
				row := d.Rows[i]
				if row.Raw != nil {
					stack = append(stack, frame{d: row.Raw, inGroup: e.inGroup})
				} else {
					for j := len(row.Cells) - 1; j >= 0; j-- {
						stack = append(stack, frame{d: row.Cells[j], inGroup: e.inGroup})
						if j > 0 {
							stack = append(stack, frame{d: spaceText, inGroup: e.inGroup})
						}
					}
				}
				if i > 0 && row.Sep != nil {
					stack = append(stack, frame{d: row.Sep, inGroup: e.inGroup})
				}
			}
		}
	}
