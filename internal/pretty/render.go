...
func (r *renderer) renderInMode(indent int, m mode, noWidth bool, doc Doc) {
	if doc == nil {
		return
	}
	base := len(r.renderStack)
	r.renderStack = append(r.renderStack, renderEntry{indent: indent, mode: m, noWidth: noWidth, doc: doc})
	for len(r.renderStack) > base {
		...
		case *docCat:
			r.renderStack = append(r.renderStack,
				renderEntry{indent: e.indent, mode: e.mode, noWidth: e.noWidth, doc: d.Right},
				renderEntry{indent: e.indent, mode: e.mode, noWidth: e.noWidth, doc: d.Left})

		case *docNest:
			r.renderStack = append(r.renderStack, renderEntry{indent: e.indent + 1, mode: e.mode, noWidth: e.noWidth, doc: d.Child})
		...
		case *docGroup:
			...
			r.renderStack = append(r.renderStack, renderEntry{indent: e.indent, mode: mode, noWidth: e.noWidth, doc: d.Child})

		case *docNoWidth:
			r.renderStack = append(r.renderStack, renderEntry{indent: e.indent, mode: e.mode, noWidth: true, doc: d.Child})

		case *docIfBreak:
			if e.mode == modeBreak {
				r.renderStack = append(r.renderStack, renderEntry{indent: e.indent, mode: e.mode, noWidth: e.noWidth, doc: d.Broken})
			} else {
				r.renderStack = append(r.renderStack, renderEntry{indent: e.indent, mode: e.mode, noWidth: e.noWidth, doc: d.Flat})
			}

		case *docTable:
			r.renderTableInMode(e.indent, e.mode, e.noWidth, d.Rows)
		}
	}
}
...
func (r *renderer) renderTableInMode(indent int, m mode, noWidth bool, rows []Row) {
	if m == modeFlat {
		for i, row := range rows {
			if i > 0 {
				r.renderInMode(0, modeFlat, noWidth, row.Sep)
			}
			if row.Raw != nil {
				r.renderInMode(0, modeFlat, noWidth, row.Raw)
				continue
			}
			for j, cell := range row.Cells {
				...
				r.renderInMode(0, modeFlat, noWidth, cell)
			}
		}
		return
	}
	...
	// In noWidth mode the segmentation pass still uses real width:
	// segment boundaries do not add newlines, they just keep alignment
	// from spanning a row whose inner Group (the per-pocket boundary)
	// will visibly break on width. renderTableSegment ignores width for
	// the row-break and useMerged decisions, since those would add new
	// newlines.
	...
	for len(rows) > 0 {
		...
		r.renderTableSegment(indent, noWidth, rows[:n], info, !first)
		first = false
		rows = rows[n:]
	}
}
...
func (r *renderer) renderTableSegment(indent int, noWidth bool, rows []Row, info []rowInfo, emitFirstSep bool) {
	numCols := 0
	...
	for i, row := range rows {
		if i > 0 || emitFirstSep {
			r.renderInMode(indent, modeBreak, noWidth, row.Sep)
		}
		if row.Raw != nil {
			r.renderInMode(indent, modeBreak, noWidth, row.Raw)
			continue
		}
		if row.DocComment != nil {
			...
			r.renderInMode(indent, modeBreak, noWidth, row.DocComment)
		}
		...
		if useMerged && row.MergedFirstCell != nil {
			r.renderInMode(indent, modeBreak, noWidth, row.MergedFirstCell)
			for c := 2; c < len(row.Cells); c++ {
				...
				r.buf.WriteByte(' ')
				r.col++
				r.renderInMode(indent, modeBreak, noWidth, cell)
			}
			continue
		}
		for c, cell := range row.Cells {
			...
			colStart := r.col
			r.renderInMode(cellIndent, modeBreak, noWidth, cell)
			if c == lastNonNilIdx {
				continue
			}
