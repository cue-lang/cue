...
func (r *renderer) renderTableSegment(indent int, rows []Row, info []rowInfo, emitFirstSep bool) {
	...
	// Count how many rows contribute a cell to each column, and
	// derive colMaxW from the cached cell widths. A column with only
	// one row has no alignment target, so padding the preceding
	...
	// to column widths (but they still count towards colCount).
