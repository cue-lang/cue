...
func (r *renderer) renderTableInMode(f renderFrame, d *docTable) {
	rows := d.rows
	if f.mode == modeFlat {
		// We pass f.indent (not 0) even though a flat table emits no
		// newlines of its own: a cell may contain a [docFiniteWidth]
		// pocket (or other break-capable subtree) that does break - e.g.
		// a programmatic, width-driven list nested inside an authored
		// (infinite-width) call that itself fits flat. That pocket
		// anchors its own break indentation off the indent we hand it
		// here, so handing it 0 would dedent its body to the left margin.
		for i, row := range rows {
			if i > 0 {
				r.renderInMode(f.indent, f.basePrefix, modeFlat, f.infWidth, row.sep)
			}
			if row.raw != nil {
				r.renderInMode(f.indent, f.basePrefix, modeFlat, f.infWidth, row.raw)
				continue
			}
			for j, cell := range row.cells {
				...
				r.renderInMode(f.indent, f.basePrefix, modeFlat, f.infWidth, cell)
			}
		}
		return
	}
