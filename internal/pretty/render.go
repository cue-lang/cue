...
// walkState is the mode/flag portion of a Doc-walker frame, shared by
// the renderer ([renderer.renderInMode]) and the measurer
// ([measureDoc]).
type walkState struct {
	mode mode
	// infWidth is set inside a [docInfiniteWidth] scope: width-based
	// break decisions see an unlimited budget there, so docGroups pick
	// flat unless a hard line break forces them broken.
	infWidth bool
	...
	// functionally a no-op. This flag is carried by all other Doc
	// types to their children when descending (except docTable, whose
	// rows and cells start fresh), so the "next group encountered
	// while descending" semantics work even when the wrapped subtree's
	// root is not literally a group node.
	nextGroupNoop bool
}

// enterNextGroupNoop updates s for descending into a
// [docNextGroupNoop]'s child: arm the flag so the next [docGroup] on
// the descent is neutralised.
//
// The flag is only armed in modeBreak. In modeFlat a docGroup is
// already effectively a no-op on its own (no flat-fit retry fires),
// and the flag must NOT survive a descent in modeFlat: an
// intermediate [docFiniteWidth] on the path resets the mode to
// modeBreak before the docGroup runs, and that downstream docGroup
// should pick modeFlat for content that fits at the current column -
// so it must not be a no-op.
...
// nextGroupNoop neutralises the group: the flag is consumed and the
// mode (necessarily modeBreak - the flag is only ever armed in
// modeBreak) is left untouched. A group entered in modeFlat stays
// flat with no retry. Otherwise - modeBreak with no armed flag - the
// group may flip to modeFlat and the caller supplies the decision:
// the renderer runs the real flat-fit test, while [measureDoc] is
// optimistic and always assumes flat (mirroring the assumption the
// fit test itself makes about nested groups).
...
// child: both infWidth and mode are reset so inner docGroups do their
...
// throughout via the unlimited budget). An armed nextGroupNoop is
// deliberately left armed: the [docNextGroupNoop] wrapping of a
// list/call cell targets exactly the docGroup that [finiteWidth]
// wraps around the cell's body, one level below the docFiniteWidth.
...
func (r *renderer) renderInMode(indent int, basePrefix string, m mode, infWidth bool, doc Doc) {
	...
	for len(r.renderStack) > base {
		...
		case *docGroup:
			// The fit test only runs in modeBreak with budget to spare
...
// column's running max. A cell only widens its column when the row
// has content after it: a column's max width exists to align the
// cells that follow it across rows, so the row's last non-nil cell
// (and anything past it) contributes 0 - e.g. a long comment-less
// value sitting among rows that do carry a trailing comment must not
// push those later columns out. (On a broken row, ri.cells is
// truncated right before the broken cell, so later cells contribute
// nothing here either.)
//
// Shared by [renderer.renderTableSegment]'s colMaxW accumulation and
// [renderer.longestAlignedPrefix]'s prediction of the same maxes, so
// the cell positions predicted during segmentation match what the
// segment renderer actually pads to, by construction.
...
// column past the first. Shared by [renderer.renderTableSegment]'s
// breakRow test and [renderer.longestAlignedPrefix]'s rowFits so the
// two agree on which rows overflow, by construction.
...
func (r *renderer) renderTableSegment(indent int, basePrefix string, infiniteWidth bool, rows []Row, info []rowInfo, emitFirstSep bool) {
	...
	for i, row := range rows {
		...
		// while colCount above still counts it.
...
func measureDoc(doc Doc, startMode mode, budget int) (width int, broken, canBreak bool) {
	...
	stack := stackBuf[:1]
	stack[0] = frame{doc: doc, walkState: walkState{mode: startMode}}
	for len(stack) > 0 {
