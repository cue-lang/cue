...
type renderEntry struct {
	...
	mode       mode
	noWidth    bool // child of a docNoWidth, no width-driven breaks
	doc        Doc
}

// renderInMode renders a doc using the given indent, mode, and
// no-width flag. noWidth is true when the doc sits inside a
// docNoWidth wrapper: docTable's row-break / useMerged decisions
// see an unlimited budget, and docGroup's flat-fit check sees an
// unlimited budget too — so a Group always picks flat unless its
// child contains a HardLine. The flag propagates through every
// constructor; only docNoWidth toggles it on.
//
// basePrefix is a verbatim string prepended before the nest-driven
// indent on every newline emitted inside doc. It propagates through
// every constructor; only docAtIndent overrides it (and resets the
// nest level to 0 for its child).
func (r *renderer) renderInMode(indent int, basePrefix string, m mode, noWidth bool, doc Doc) {
	if doc == nil {
		return
	}
	base := len(r.renderStack)
	r.renderStack = append(r.renderStack, renderEntry{indent: indent, basePrefix: basePrefix, mode: m, noWidth: noWidth, doc: doc})
	for len(r.renderStack) > base {
		...
		case *docCat:
			r.renderStack = append(r.renderStack,
				renderEntry{indent: e.indent, basePrefix: e.basePrefix, mode: e.mode, noWidth: e.noWidth, doc: d.Right},
				renderEntry{indent: e.indent, basePrefix: e.basePrefix, mode: e.mode, noWidth: e.noWidth, doc: d.Left})

		case *docNest:
			r.renderStack = append(r.renderStack, renderEntry{indent: e.indent + 1, basePrefix: e.basePrefix, mode: e.mode, noWidth: e.noWidth, doc: d.Child})

		case *docGroup:
			mode := e.mode
			if mode != modeFlat {
				remaining := r.width - r.col
				// In noWidth mode the fit-test sees an unlimited
				// budget so the Group always picks flat unless its
				// child contains a HardLine. The rule: any subtree
				// under an AST node whose descendants carry a
				// Newline/NewSection RelPos is not subject to the
				// line-width limit.
				if e.noWidth {
			...
			r.renderStack = append(r.renderStack, renderEntry{indent: e.indent, basePrefix: e.basePrefix, mode: mode, noWidth: e.noWidth, doc: d.Child})

		case *docNoWidth:
			r.renderStack = append(r.renderStack, renderEntry{indent: e.indent, basePrefix: e.basePrefix, mode: e.mode, noWidth: true, doc: d.Child})

		case *docProgrammatic:
			// Reset both noWidth and mode for Child so its inner
			// Groups do their own flat-fit against the real width
			// budget, even when enclosed by a parsed wrap (which
			// would otherwise force modeFlat throughout via
			// NoWidth's MaxInt budget). preFlatten doesn't enter
			// docProgrammatic, so soft Lines and IfBreak alternates
			// inside Child remain available for those decisions.
			r.renderStack = append(r.renderStack, renderEntry{indent: e.indent, basePrefix: e.basePrefix, mode: modeBreak, noWidth: false, doc: d.Child})

		case *docAtIndent:
			r.renderStack = append(r.renderStack, renderEntry{indent: 0, basePrefix: d.Prefix, mode: e.mode, noWidth: e.noWidth, doc: d.Child})

		case *docIfBreak:
			if e.mode == modeBreak {
				r.renderStack = append(r.renderStack, renderEntry{indent: e.indent, basePrefix: e.basePrefix, mode: e.mode, noWidth: e.noWidth, doc: d.Broken})
			} else {
				r.renderStack = append(r.renderStack, renderEntry{indent: e.indent, basePrefix: e.basePrefix, mode: e.mode, noWidth: e.noWidth, doc: d.Flat})
			}

		case *docTable:
			r.renderTableInMode(e.indent, e.basePrefix, e.mode, e.noWidth, d.Rows)
		}
	}
}
...
func (r *renderer) renderTableInMode(indent int, basePrefix string, m mode, noWidth bool, rows []Row) {
	if m == modeFlat {
		for i, row := range rows {
			if i > 0 {
				r.renderInMode(0, basePrefix, modeFlat, noWidth, row.Sep)
			}
			if row.Raw != nil {
				r.renderInMode(0, basePrefix, modeFlat, noWidth, row.Raw)
				continue
			}
			for j, cell := range row.Cells {
				...
				r.renderInMode(0, basePrefix, modeFlat, noWidth, cell)
			}
		}
		return
	}
	...
	// an enclosing NoWidth scope without a Programmatic boundary
	// to escape it report canWrap=false — their inner Groups go
	// flat regardless of width, so segmenting around them would
	// just visibly break the user's column layout for no gain.
	// Cells that escape the NoWidth scope via Programmatic
	// (option B: programmatic pockets nested in a parsed wrap)
	// report canWrap=true and properly segment around overflow.
	...
	for len(rows) > 0 {
		...
		r.renderTableSegment(indent, basePrefix, noWidth, rows[:n], info, !first)
...
func (r *renderer) renderTableSegment(indent int, basePrefix string, noWidth bool, rows []Row, info []rowInfo, emitFirstSep bool) {
	numCols := 0
	...
	// Carve colCount and colMaxW from r.colScratch using stack
	// semantics so a nested renderTableSegment (via cell content)
	// doesn't clobber our slot. The scratch length is restored at
	// the function's single exit point.
	...
	// In noWidth mode, the breakRow check below must not fire:
	// parsed-mode rows preserve the user's layout regardless of
	// overflow. Cells that escape the NoWidth scope via
	// Programmatic carry canWrap=true and the breakRow guard
	// `!cells[1].canWrap` already excludes them. So MaxInt here
	// only suppresses the row-break for genuine parsed-mode
	// rows, not for programmatic pockets nested inside.
	if noWidth {
		avail = math.MaxInt
	}
	alignedWidth := 0
	...
	// for single-row segments (no alignment to disrupt) where the
	// row opts in via AllowRowBreak, has no merged alternative
	// (chain rows with atomic val go through the merged-render path
	// instead), and the total flat width would exceed the budget.
	// We also refuse to row-break when cell 1 itself can wrap (its
	// Doc contains a Group / soft Line / Table) — keeps `key: {…}`
	...
	breakRow := false
	if len(rows) == 1 && !info[0].broken && rows[0].Raw == nil &&
		rows[0].AllowRowBreak && rows[0].MergedFirstCell == nil &&
		len(info[0].cells) >= 2 && !info[0].cells[1].canWrap {
		totalFlat := 0
		for c, ci := range info[0].cells {
			if c > 0 {
				totalFlat++
			}
			totalFlat += ci.w
	...
	for i, row := range rows {
		if i > 0 || emitFirstSep {
			r.renderInMode(indent, basePrefix, modeBreak, noWidth, row.Sep)
		}
		if row.Raw != nil {
			r.renderInMode(indent, basePrefix, modeBreak, noWidth, row.Raw)
			continue
		}
		if row.DocComment != nil {
			...
			r.renderInMode(indent, basePrefix, modeBreak, noWidth, row.DocComment)
		}
		...
		if useMerged && row.MergedFirstCell != nil {
			r.renderInMode(indent, basePrefix, modeBreak, noWidth, row.MergedFirstCell)
			for c := 2; c < len(row.Cells); c++ {
				...
				r.buf.WriteByte(' ')
				r.col++
				r.renderInMode(indent, basePrefix, modeBreak, noWidth, cell)
			}
			continue
		}
		for c, cell := range row.Cells {
			...
			colStart := r.col
			r.renderInMode(cellIndent, basePrefix, modeBreak, noWidth, cell)
			if c == lastNonNilIdx {
				continue
			}
...
func (r *renderer) longestAlignedPrefix(rows []Row, avail int) (int, []rowInfo) {
	...
	// is fine (e.g. a lone trailing // comment). AllowRowBreak rows
	// are deliberately NOT made stricter here — multi-row alignment
	// is preferred even when the resulting line is a few characters
	// wider than the budget. The row-break feature only kicks in for
	// single-row segments, see renderTableSegment.
...
func measureCell(doc Doc) (width int, broken, canWrap bool) {
	...
	// inNoWidth tracks whether we are inside a NoWidth scope that
	// has not been escaped by a Programmatic boundary. Soft Lines,
	// IfBreaks, and inner Tables only mark the cell as canWrap if
	// they could ACTUALLY break at render time — under an
	// effective NoWidth scope they cannot (the surrounding Group's
	// flat-fit succeeds with MaxInt budget, so it stays flat and
	// soft Lines emit their alts). Programmatic resets the scope
	// so its inner content's canWrap is reported truthfully.
	type frame struct {
		d         Doc
		inGroup   bool
		inNoWidth bool
	...
	for len(stack) > 0 {
		...
		case *docHard, *docLitLine:
			return 0, true, true
		case *docCat:
			stack = append(stack, frame{d: d.Right, inGroup: e.inGroup, inNoWidth: e.inNoWidth})
			stack = append(stack, frame{d: d.Left, inGroup: e.inGroup, inNoWidth: e.inNoWidth})
		case *docNest:
			stack = append(stack, frame{d: d.Child, inGroup: e.inGroup, inNoWidth: e.inNoWidth})
		case *docGroup:
			stack = append(stack, frame{d: d.Child, inGroup: true, inNoWidth: e.inNoWidth})
		case *docNoWidth:
			stack = append(stack, frame{d: d.Child, inGroup: e.inGroup, inNoWidth: true})
		case *docProgrammatic:
			stack = append(stack, frame{d: d.Child, inGroup: e.inGroup, inNoWidth: false})
		case *docIfBreak:
			if !e.inNoWidth {
				canWrap = true
			}
			if e.inGroup {
				stack = append(stack, frame{d: d.Flat, inGroup: e.inGroup, inNoWidth: e.inNoWidth})
			} else {
				stack = append(stack, frame{d: d.Broken, inGroup: e.inGroup, inNoWidth: e.inNoWidth})
			}
		case *docTable:
			if !e.inNoWidth {
				canWrap = true
			...
			for i := len(d.Rows) - 1; i >= 0; i-- {
				row := d.Rows[i]
				if row.Raw != nil {
					stack = append(stack, frame{d: row.Raw, inGroup: e.inGroup, inNoWidth: e.inNoWidth})
				} else {
					for j := len(row.Cells) - 1; j >= 0; j-- {
						stack = append(stack, frame{d: row.Cells[j], inGroup: e.inGroup, inNoWidth: e.inNoWidth})
						if j > 0 {
							stack = append(stack, frame{d: spaceText, inGroup: e.inGroup, inNoWidth: e.inNoWidth})
						}
					}
				}
				if i > 0 && row.Sep != nil {
					stack = append(stack, frame{d: row.Sep, inGroup: e.inGroup, inNoWidth: e.inNoWidth})
				}
			}
		}
	}
