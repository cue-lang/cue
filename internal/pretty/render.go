...
func (r *renderer) render(doc *Doc) {
	// Use a slice as a LIFO stack. We process from the end (top).
	stack := []entry{{ind: 0, mode: modeBreak, doc: doc}}
	...
	for len(stack) > 0 {
		...
		case tagGroup:
			if r.fits(r.width-r.col, e.ind, e.doc.right) {
		...
		case tagTable:
			...
			for i := len(e.doc.rows) - 1; i >= 0; i-- {
				...
				if row.Raw != nil {
					...
					stack = append(stack, entry{e.ind, modeFlat, Text(" ")})
