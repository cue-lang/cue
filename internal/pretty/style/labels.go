...
// algorithm mirrors [cuelang.org/go/cue/format]'s labelSimplifier and
// runs in three sub-passes per struct body:
//
...
// labelSimplifier mirrors the v1 formatter's labelSimplifier. Each
// scope holds a map from candidate names to whether they are still
// eligible for unquoting (true means "no reference observed yet").
