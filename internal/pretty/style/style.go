...
func (w *walker) visit(n ast.Node) bool {
	...
	case *ast.File:
		...
		if w.cfg.RelPos {
			w.annotateBody(n.Decls, false, false)
		}
	...
	case *ast.StructLit:
		...
		if w.cfg.RelPos {
			...
			if w.annotateBody(n.Elts, n.Lbrace.IsValid(), false) {
...
func (w *walker) breakComprehensionBody(body *ast.StructLit) {
	...
	if w.annotateBody(body.Elts, true, true) {
...
func (w *walker) annotateBody(decls []ast.Decl, isStructBody, alwaysOpenFirst bool) (openFirstFired bool) {
	visibleCount := 0
	anyInlineHint := false
	...
	if anyInlineHint && !anyLineComment {
		return false
	}
