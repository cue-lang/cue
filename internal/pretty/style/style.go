...
// two or more non-elided decls, the first non-elided decl also gets a
// Newline target so the body breaks across lines after the opening
// brace. File bodies are excluded because their first decl's leading
// RelPos describes placement relative to BOF / leading file-level
// comments, not relative to an enclosing bracket. Single-element
// struct bodies are also excluded so single-field chain shapes (a: b:
// c) and single-element hug (`[{...}]`) still fire. Returns whether
