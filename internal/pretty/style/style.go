...
// two or more non-elided decls and no decl carries an explicit "stay
// inline" hint (Blank / NoSpace RelPos), the first non-elided decl
// also gets a Newline target so the body breaks across lines after
...
func (w *walker) annotateBody(decls []ast.Decl, isStructBody, alwaysOpenFirst, isFile bool) (openFirstFired bool) {
	...
	openFirst := isStructBody && (alwaysOpenFirst || visibleCount >= 2)
