...
// mirrors the single-line branch of cue/format's
// interpolationNormalize, but RelPos-only: it clears
...
// position. The newlines that matter for #2a live inside the `\(...)`
...
// posTrailingMin is the first comment Position counted as trailing -
// rendered after the node's content. It mirrors the constant of the
// same name in the pretty converter.
const posTrailingMin int8 = 3
...
// posTrailingMin) or a same-line comment that is not a doc comment.
func hasTrailingComment(n ast.Node) bool {
	for _, cg := range ast.Comments(n) {
		if cg.Position >= posTrailingMin {
			return true
		}
