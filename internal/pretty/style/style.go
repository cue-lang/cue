...
func (w *walker) annotateBody(decls []ast.Decl, isStructBody, alwaysOpenFirst, isFile bool) (openFirstFired bool) {
	...
	for _, d := range decls {
		...
		// A decl whose rendered output ends with a `//` comment makes a
		// compact body impossible: CUE line comments run to end-of-line,
		// so the comment would swallow the inter-decl `, ` separator and
		// the closing brace. Such a body only arises programmatically
		// (an authored same-line comment already forces the next decl
		// onto its own line), so this reshapes only programmatic bodies.
		if endsWithLineComment(d) {
			anyLineComment = true
		}
	}
	...
	// `import "x", {...}`.
	if anyInlineHint && !anyLineComment && !isFile {
		return false
	}
