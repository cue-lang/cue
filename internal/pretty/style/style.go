...
// RelPos - see [pretty.LeadingRelPos].
//
...
func (w *walker) visit(n ast.Node) bool {
	...
	case *ast.Comprehension:
		if w.cfg.RelPos {
			...
			// single-element body, where a plain struct would stay
			// inline. The same applies to the else/otherwise fallback
			// body.
...
// clears Newline/NewSection leading positions rather than inspecting
// source line numbers.
//
// A string is "multi-line" when one of its literal fragments contains
// a newline (the `"""..."""` body), which is content, not
// position. The newlines that matter here live inside the `\(...)`
// expressions, so they never appear in the fragments and a
// single-line string's fragments are newline-free.
func (w *walker) collapseSingleLineInterpolation(x *ast.Interpolation) {
	...
	// the string's own, so only the elements after it are cleared.
...
// breakComprehensionBody applies the comprehension-body house style
// to body: ensure it is braced and force a break after the opening
// brace, with the closer on its own line - even a single-element
// body, where a plain struct would stay inline.
//
// A comprehension body is always braced, but a programmatic one (e.g.
// from internal/core/export) is emitted without brace positions. That
// makes [ast.StructLit.Pos] fall back to the first element, so
// forcing that element onto its own line would also push the opening
// brace down (`for x in y\n{`). Giving the body explicit braces first
// - a Blank opener keeps `{` on the clause line - means only the body
// breaks. annotateBody's anyInlineHint guard leaves authored compact
// bodies untouched, so this only reshapes programmatic ones.
func (w *walker) breakComprehensionBody(body *ast.StructLit) {
	if body == nil {
		return
	}
	// The value's opening brace hugs the final clause - it never sits
	// on its own line. A braceless (programmatic) value gets a
	// synthesised Blank-positioned brace; a parsed brace that begins
	// on its own line (Newline / NewSection) is downgraded to Blank so
	// the value hugs (e.g. `if x > 9\n{x}` renders as `if x > 9 {x}`).
	switch {
...
// anchor; for every later decl we compute the target RelPos and
// upgrade if it is stronger than the existing leading RelPos.
//
// When isStructBody is true (the body is the Elts of a
// [*ast.StructLit], not the Decls of a [*ast.File]) and the body has
// two or more decls and no decl carries an explicit "stay inline"
// hint (Blank / NoSpace RelPos), the first decl also gets a Newline
// target so the body breaks across lines after
// the opening brace. A Blank/NoSpace on any decl signals that the
// builder placed the decl on the same line as its predecessor (e.g. a
// comma-separated body like `{a: 1, b: 2}`), so the openFirst upgrade
// is suppressed and the compact shape is preserved. File bodies are
// excluded because their first decl's leading RelPos describes
// placement relative to BOF / leading file-level comments, not
// relative to an enclosing bracket. Single-element struct bodies are
// also excluded so single-field chain shapes (a: b: c) and
// single-element hug (`[{...}]`) still fire - unless alwaysOpenFirst
// is set, which fires the openFirst upgrade at one-or-more
// elements. That is used for comprehension bodies, which always break
// after the opening brace as a matter of house style. Returns whether
// the first-elt Newline upgrade fired, so the StructLit caller knows
// it also needs to break before the closing `}`.
func (w *walker) annotateBody(decls []ast.Decl, isStructBody, alwaysOpenFirst, isFile bool) (openFirstFired bool) {
	...
	for _, d := range decls {
		...
		// carries that hint, the body is taken to be authored compact
		// and we leave inter-decl RelPos alone for the whole body -
		// openFirst stays false, and the B1/B2/A4 upgrades below are
		// also skipped.
		...
		// swallow the inter-decl `, ` separator and the closing
		// brace. Either way the body must break, defeating the
		// authored-compact heuristic below. Such a body only arises
		// programmatically (the parser already forces an own-line break
		// around any comment), so this reshapes only programmatic
		// bodies.
		if endsWithLineComment(d) || hasLeadingDocComment(d) {
			anyComment = true
		}
	}
	// The "authored compact body" heuristic only makes sense for a
	// struct body, which can legitimately render inline as `{a: 1, b:
	// 2}`. The file's top-level decls are never comma-joined, so a
	// Blank/NoSpace RelPos there (e.g. an EmbedDecl whose
	// ast.NewStruct value carries a NoSpace Lbrace) is not an inline
	// hint - honouring it would skip the blank-line-after-import (A2)
	// and one-decl-per-line (B1) rules and leave the converter to emit
	// `import "x", {...}`.
	...
	// still applies, so an authored compact body is left alone - this
	...
	for _, curr := range decls {
		if prev == nil {
			...
			// otherwise it is left alone (its leading RelPos describes
			// spacing from BOF or leading file-level comments).
			//
			// A leading doc comment on the first element also forces the
			// open break, even when openFirst wouldn't otherwise fire (a
			// single-element or programmatic struct): a `//` comment
			// must sit on its own line, so the opener cannot share
			// it. This is scoped to struct bodies (!isFile) - a file's
			// first decl sits at BOF with no opener to break after.
...
// "Effective" here means the position [pretty.LeadingRelPos] reads:
...
func (w *walker) upgradeLeading(n ast.Node, target token.RelPos) {
	...
	if cg := pretty.FirstCommentAt(n, pretty.PosDoc); cg != nil {
		...
		// element. Materialise the absent opening brace and put the
		// RelPos there, so the renderer reads it as "break before the
		// struct" rather than "break after the opening brace". A
		// Blank/NoSpace opener would not have reached here: it is
		// caught by annotateBody's anyInlineHint guard.
...
// the token that an [ast.SetRelPos] on n would otherwise target - or
// nil when n's leading token is a real token (a brace, an identifier,
// an operator) that can carry the RelPos directly.
//
// A node's leading position is transparent through wrappers that have
// no token of their own: an [*ast.EmbedDecl] reports its Expr's
// position, and an [*ast.BinaryExpr] reports its left operand's. So
// the leading token of `{a: 1} & {b: 2}` embedded in a struct is the
// `{` of the first operand. When that operand is a braceless
// (brace-elided) struct it has no `{`, so [ast.StructLit.Pos] falls
// through to its first element; a leading RelPos written via
// [ast.SetRelPos] then lands inside the struct, where the renderer
// reads it as "break after the opening brace" instead of the intended
// "break before the struct". Returning the struct lets
// [walker.upgradeLeading] attach the RelPos to a materialised brace
// instead.
//
// A braced struct (or any other leading token) returns nil: its own
// position carries the RelPos correctly, so no materialisation is
// needed.
...
// joining the originals with an empty `//` line. It reports whether
// it changed anything.
//
// House style: multiple doc comment groups on one node only arise in
// programmatically built ASTs - chiefly from
// [cuelang.org/go/internal/core/export], where unifying two fields
// merges each one's doc comment onto the result. The CUE parser never
// produces two doc groups for a single node: adjacent `//` lines fold
// into one group, and a blank line detaches the earlier comment to
// the enclosing scope. Rendering the groups adjacent would therefore
// fold them into one group on reparse (losing that they were
// distinct); rendering them blank-separated would detach the
// first. Joining them with a `//` line keeps both as doc comments,
// visually separated, and makes the output reparse to exactly this
// shape - so format/parse is stable at the AST level. The merged
// group keeps the first group's leading position. Because parsed
// sources never reach two doc groups, this is a no-op for cue fmt and
// only reshapes export-style output.
//
// File- and package-level leading comments are exempt: a file
// commonly carries several distinct top-level blocks (a license
// header, a package banner, the package doc) that the source kept
// apart with blank lines. Unlike a field's doc comment, these sit at
// the top of the file with nothing to detach to, so they round-trip
// fine blank-separated and must keep their section breaks rather than
...
// sectionSeparateDocComments is the file/package counterpart to
// [mergeDocComments]: rather than folding multiple doc-position
// comment groups together, it keeps them apart with blank lines. A
// file commonly carries several distinct top-level blocks - a license
// header, a package banner, the package doc - and a programmatically
// built file (e.g. from [cuelang.org/go/internal/core/export] or the
// protobuf extractor) leaves them with no RelPos, so they would
// otherwise render run together. This promotes every group after the
// first to NewSection so the section breaks are restored. It never
// weakens an existing RelPos, so parsed files - whose multi-group
// file comments are already blank-separated - are untouched.
...
// child of non-bracketed nodes, mirroring how the pretty converter
// appends and propagates trailing comments.
//
// This is a deliberately independent approximation of the pretty
// converter's endsWithLineComment analyse flag, not a copy of it: the
// two predicates differ at the edges (e.g. the converter's counts
// interior Position 1/2 comments on nodes that don't manage their own
// interior; this one counts any same-line non-doc comment) and they
// do NOT need to agree. A miss here just leaves a compact body alone
// - the converter's declSep still promotes the separator to a newline
// when the previous decl's rendering ends with a `//`, so the output
// stays valid. A false positive merely annotates the body
// one-decl-per-line, which is a legitimate (conservative) style
// outcome.
