...
// n.Pos().RelPos(). This mirrors pretty.LeadingRelPos, which is the
// function the pretty package uses to decide a decl's inter-decl
// separator. The style package's heuristics target this effective
// value, so the tests assert on it rather than on the raw decl Pos.
...
func importDecl(specs ...*ast.ImportSpec) *ast.ImportDecl {
	...
	if len(specs) > 1 {
		// Multi-spec imports always come with parens. Give Lparen a
		// valid position so the renderer treats it as a block.
...
func bracelessStruct(elts ...ast.Decl) *ast.StructLit {
	// No Lbrace/Rbrace: mirrors the synthesised structs that
...
// TestAnnotateRelPosBracelessEmbed covers the rule that a leading
...
// first element. Without it, the second of two sibling embeds (which
// the B1/B2 "one decl per line" rule targets with Newline) would have
// the Newline land inside its body, and the renderer would explode
// the struct while its inline sibling stayed flat. This mirrors the
// shape internal/core/export emits for two yields into one field (see
// testdata/main/topocomp2.txtar).
func TestAnnotateRelPosBracelessEmbed(t *testing.T) {
	cfg := style.Config{RelPos: true}

	// topField wraps a braceless container struct (as export emits it)
	// holding the given embeds in a File, and annotates it.
	...
	t.Run("SingleFieldEmbed", func(t *testing.T) {
		...
		// The first embed is the anchor (no upgrade); it stays
		// braceless and renders flat without any materialised brace.
	...
	t.Run("BinaryConjunction", func(t *testing.T) {
		// top: { {a1: true} {b: true} & {c: true} } - the leading
		// operand of the conjunction carries the brace + Newline; the
		// trailing operand, not at the leading edge, is left alone.
