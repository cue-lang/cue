...
// n.Pos().RelPos(). This mirrors leadingRelPos in internal/pretty/ast.go,
...
func TestAnnotateRelPos(t *testing.T) {
	...
	t.Run("A4_BlankBeforeDocAfterDefinition", func(t *testing.T) {
		...
		// RelPos - that is what leadingRelPos reads - not the
		// decl's own Pos.
	...
	t.Run("Conservative_PreservesDocCommentRelPos", func(t *testing.T) {
		...
		// on the doc comment's Slash (where leadingRelPos reads it
	...
	t.Run("SkipsElidedDecls", func(t *testing.T) {
		// Elided decls are invisible at render time and must not
		// participate in the prev/curr pairing. The elided decl's
		// own RelPos is left untouched.
		elided := field("e", "int")
		ast.SetRelPos(elided, token.Elided)
		f := &ast.File{Decls: []ast.Decl{
			field("a", "int"),
			elided,
			field("b", "int"),
		...
		cfg.Annotate(f)
		if got, want := leadingRel(f.Decls[2]), token.Newline; got != want {
			t.Errorf("decl[2] leading RelPos = %v, want %v (elided decl[1] not the predecessor)",
				got, want)
		}
		if got, want := elided.Pos().RelPos(), token.Elided; got != want {
			t.Errorf("elided decl RelPos changed to %v, want %v", got, want)
		}
	})
	...
	t.Run("FirstEltInMultiStructBody", func(t *testing.T) {
		// {a: 1, b: 2} - inside a struct body of 2+ elts, the
		// first elt's leading RelPos becomes Newline so pretty's
		// openBreakDoc(firstElem=...) emits a HardLine between `{`
		// and a:.
	...
	t.Run("FirstEltInMultiStructBodyWithDoc", func(t *testing.T) {
		...
		// (matching what pretty's leadingRelPos reads).
...
func TestAnnotateEllipsis(t *testing.T) {
	...
	t.Run("RemovedCommentsAllTransfer", func(t *testing.T) {
		...
		// every removed marker, in source order.
		var got []string
