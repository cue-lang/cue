...
// ReadBytes loads the source bytes for the given arguments. If src != nil,
// ReadBytes converts src to a []byte if possible; otherwise it returns an
// error. If src == nil, ReadBytes returns the result of reading the file
// specified by filename.
func ReadBytes(filename string, src any) ([]byte, error) {
	if src != nil {
		switch src := src.(type) {
		case string:
			return []byte(src), nil
