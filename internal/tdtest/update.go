...
type info struct {
	...
	numFields   int  // the number of field in T
...
func (s *Set[T]) getInfo() *info {
	var zero T

	info := &info{
		t:       s.t,
		tcType:  reflect.TypeOf(zero),
	...
	a := info.findCalls(fn.Body, "New")
	if len(a) != 1 {
		// TODO: allow more than one.
		t.Fatalf("only one New function allowed per test")
	}
	...
	if !ok {
		t.Fatalf("%v: arg 2 of %s must be a reference to the table",
			fset.Position(call.Args[1].Pos()), "New")
	}
	def := ti.Uses[ident]
	pos := def.Pos()
	...
	if !ok {
		...
		t.Fatalf("expected composit literal, found %T", v)
	}
	info.table = v

	// Find and index assertion calls.
	a = info.findCalls(fn.Body, "Equal", "EqualWriter")
	for _, x := range a {
	...
	ast.Inspect(block, func(n ast.Node) bool {
		...
		for _, name := range names {
			if sel.Sel.Name == name {
				...
				} else if id, ok := info.Uses[sel.X.(*ast.Ident)].(*types.PkgName); ok && strings.Contains(id.String(), tdtestParen) {
				} else {
					return true
				}
				ci := &callInfo{
...
func (t *T) getCallInfo(file string, line int) *callInfo {
	// TODO: check error
	return t.info.calls[token.Position{Filename: file, Line: line}]
}

func (s *Set[TC]) update() {
	t := s.t
	fset := s.info.testPkg.Fset

	file := fset.Position(s.info.table.Pos()).Filename
	var f *ast.File
	for i, gof := range s.info.testPkg.GoFiles {
		if gof == file {
			f = s.info.testPkg.Syntax[i]
		}
	}
	if f == nil {
	...
	astutil.Apply(f, func(c *astutil.Cursor) bool {
		...
		switch x := s.info.patches[n]; x.(type) {
		case nil:
...
func (t *T) updateField(ci *callInfo, newValue any) {
	t.info.needsUpdate = true

	fset := t.info.testPkg.Fset

	e, ok := t.info.table.Elts[t.iter].(*ast.CompositeLit)
	if !ok {
	...
	if !isZero {
		...
		if len(e.Elts) > 0 {
			...
			old := t.info.patches[key]
			if old != nil {
				t.info.patches[kv] = old
			}
			t.info.patches[key] = kv
		} else {
			t.info.patches[e] = &ast.CompositeLit{Elts: []ast.Expr{kv}}
		}
	}
}
