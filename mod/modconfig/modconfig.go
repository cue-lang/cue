...
// ResolveToHost returns the host location for the given module path and version
// without creating a Registry instance for it.
func (r *Resolver) ResolveToHost(mpath string, vers string) (HostLocation, bool) {
	return r.resolver.ResolveToHost(mpath, vers)
}
...
func (r *Resolver) ResolveToRegistry(mpath string, vers string) (modregistry.RegistryLocation, error) {
	loc, ok := r.resolver.ResolveToHost(mpath, vers)
	if !ok {
