...
type Config struct {
	...
	// DefaultRegistry is used as the fallback registry when none
	// is applicable from CUERegistry. If it's empty, a
	// default central registry will be used.
	DefaultRegistry string

	// Authorizer is used to authorize registry requests.
	// If it's nil, a default authorizer will be used that consults
	// the information stored by "cue login" and also the
	// information in .docker/config.json.
...
func NewResolver(cfg0 *Config) (*Resolver, error) {
	...
	var err error
	if configPath != "" {
		resolver, err = modresolve.ParseConfig(configData, configPath, cfg.DefaultRegistry)
	} else {
		resolver, err = modresolve.ParseCUERegistry(cueRegistry, cfg.DefaultRegistry)
	}
	if err != nil {
