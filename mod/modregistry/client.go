...
// will have the given prefix except that if the prefix is empty, the prefix "cue/" will be used.
...
func (c *Client) GetModule(ctx context.Context, m module.Version) (*Module, error) {
	...
	if !isModule(&manifest) {
		return nil, fmt.Errorf("%v does not resolve to a manifest of the correct type (media type is %q)", m, modDesc.MediaType)
	}
...
// Question: do we want to provide the resolved module versions or have PutModule do that itself?
// Given that we're provided a zip file, the
...
func (d Dependency) GetZip(ctx context.Context) (io.ReadCloser, error) {
	return d.client.registry.Fetch(ctx, d.repo, d.desc)
}
