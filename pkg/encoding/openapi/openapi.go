...
func MarshalSchema(config cue.Value, version string, schema pkg.Schema) (string, error) {
	ctx := value.OpContext(schema)
	return marshalSchema(ctx, config, version, schema)
}

func marshalSchema(ctx *adt.OpContext, config cue.Value, version string, schema pkg.Schema) (string, error) {
