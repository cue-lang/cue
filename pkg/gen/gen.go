...
//
//OK, so once we've extracted the CUE from the comment, we need to put it
//into the declaration table somehow. But how do we express arbitrary CUE
//in that table. And how is that table used to make the builtins that can be used
//by the CUE runtime later?
//
//We should fill in Param.Value for each parameter.
//
//read cue decl text; load into CUE;
//go through each member, extracting parameter name
//find parameter of that name in the AST func decl (or fail)
