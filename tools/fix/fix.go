...
type closeInfo struct {
	...
	// isListElement indicates whether we are in a list element. There is no
	// equivalent ast.Field for list elements. Like with Fields, we need to
	// limit the blast radius of embeddings.
	//
	// TODO: For now we do this by disabling reclosing within list elements.
	// This is not correct, as we do want to reclose structs as list elements.
	// The problem is that the modification flag is not correctly tracked
	// across list elements. We need to fix that in astutil.Apply first.
	isListElement bool
