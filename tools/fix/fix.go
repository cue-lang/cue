...
// fixListCommas removes explicit commas from multiline list elements by
// clearing the HasComma bit on element positions. The formatter then uses
// the comma-free style for these lists.
//
// Element[0]'s HasComma bit is the CUE-parsed marker and is preserved.
// Only inter-element commas (element[1:]) and the trailing comma (rbrack)
// are cleared.
func fixListCommas(f *ast.File) {
	ast.Walk(f, func(n ast.Node) bool {
		...
		// Only process multiline lists (Rbrack on a different line than Lbrack).
		if list.Rbrack.RelPos() < token.Newline {
			return true
		}
		// Clear inter-element commas (skip element[0] which is the CUE-parsed marker).
		for _, elt := range list.Elts[1:] {
			p := elt.Pos()
			if p.HasComma() {
