...
func Files(files []*ast.File, dir string, val cue.Value, cfg *Config) error {
...
func (t *trimmer) logf(format string, args ...any) {
	w := t.trace
	if w == nil {
		return
	}
...
func (t *trimmer) inc() { t.depth++ }
func (t *trimmer) dec() { t.depth-- }

func (t *trimmer) getNodeMeta(n ast.Node) *nodeMeta {
	if n == nil {
		return nil
	}
	d, found := t.nodes[n]
...
func (t *trimmer) findStaticDependencies(files []*ast.File) {
	t.inc()
	defer t.dec()
...
// specific.
func (t *trimmer) findPatterns(v *adt.Vertex) {
	t.inc()
	defer t.dec()
...
// disjunction.
func (t *trimmer) findDisjunctions(v *adt.Vertex) {
	t.inc()
	defer t.dec()
...
func (t *trimmer) keepAllChildren(n ast.Node) {
...
// redundant conjuncts.
func (t *trimmer) findRedundancies(v *adt.Vertex, keepAll bool) {
	v = v.DerefDisjunct()
...
//     survive.
func (t *trimmer) linkResolvers(c adt.Conjunct, addInverse bool) {
	var origNm *nodeMeta
	if src := c.Field().Source(); src != nil {
...
func (t *trimmer) resolveElemAll(c adt.Conjunct, f func(adt.Resolver, *adt.Vertex)) {
...
func (t *trimmer) equallySpecific(v *adt.Vertex, cs ...adt.Conjunct) bool {
	t.inc()
	//	t.ctx.LogEval = 1
...
// less) in isolation.
func (t *trimmer) solveUndecideds() {
...
// required.
func (t *trimmer) trim(files []*ast.File, dir string) error {
	t.inc()
	defer t.dec()
