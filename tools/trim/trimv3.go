...
//
// Cases b and c are decided only once all vertices have been seen:
// if the conjuncts of a vertex that must be kept for other reasons
// are jointly as specific as the vertex, then no winner needs to be
// kept at all. This handles schemas which are split over several
// declarations, none of which covers the vertex on its own.
//
// Specificity is tested by subsumption, with default values applied
// on both sides: trim must only preserve the outcome of evaluation,
// in which defaults are resolved, so concrete values implied by a
// schema's defaults are redundant no matter how deeply the defaults
// are nested.
//
...
//
// Similarly, conjuncts generated by a comprehension whose clauses
// read from the vertex under consideration, or an ancestor of it,
// cannot win: see [nodeMeta.comprehensionDependsOn].
//
...
type nodeMeta struct {
	...
	// For nodes whose src is an [ast.Comprehension]: the vertices
	// that the comprehension's clauses resolve to. What the
	// comprehension produces depends on the content of these
	// vertices. See [nodeMeta.comprehensionDependsOn].
	reads []*adt.Vertex
}
...
// comprehensionDependsOn reports whether nm is (statically) within a
// comprehension whose clauses read from v or an ancestor of v. Such a
// conjunct cannot be relied on to keep the field for v in existence:
// the comprehension only produces it based on the current content of
// v or its ancestors, which removing other conjuncts would change.
// For example:
//
//	s: x: {port: 8080}
//	#d: port: 8080
...
//	}
//
// In the vertex for s.x, the conjunct for #d, generated by the
// comprehension, is as specific as the vertex. But if it were to
// cause the removal of `s: x: {port: 8080}`, the comprehension would
// no longer have anything to iterate over, and the field x would
// disappear entirely.
func (nm *nodeMeta) comprehensionDependsOn(v *adt.Vertex) bool {
	for ; nm != nil; nm = nm.parent {
		// reads is only ever populated on comprehension nodes.
		if len(nm.reads) == 0 {
			continue
		}
		for u := v; u != nil; u = u.Parent {
			if slices.Contains(nm.reads, u) {
				return true
			}
		}
	}
...
// pendingVertex records a vertex for which one or more winning
// conjuncts were found, along with all its non-ignored conjuncts.
// The choice of winner is deferred until all vertices have been
// traversed; see [trimmerV3.solvePending].
type pendingVertex struct {
	v         *adt.Vertex
	winners   nodeMetas
	conjuncts []pendingConjunct
}
...
func (t *trimmerV3) findRedundancies(v *adt.Vertex, keepAll bool) {
	...
	for c := range v.LeafConjuncts() {
		...
		if nm.ignoreConjunct {
			...
			conjuncts = append(conjuncts, pendingConjunct{c: c, nm: nm})
			if nm.comprehensionDependsOn(v) {
				// This conjunct cannot win: keeping only it would not
				// keep the field in existence.
				t.logf(" comprehension depends on vertex: %p::%T", field, field)
			} else {
	...
	if keepAll {
		...
		if len(winners) == 0 {
			...
			// Defer the choice of winner until all vertices have
			// been seen: conjuncts that other vertices require may
			// jointly cover this vertex, making all winners
			// redundant. See [trimmerV3.solvePending].
			t.logf("%d winners found; deferring", len(winners))
			for i := range conjuncts {
				conjuncts[i].winner = slices.Contains(winners, conjuncts[i].nm)
			}
			t.pending = append(t.pending, pendingVertex{
				v:         v,
				winners:   winners,
				conjuncts: conjuncts,
			})
...
func (t *trimmerV3) linkResolvers(c adt.Conjunct, addInverse bool) {
	...
	t.linkResolversOrig(c, addInverse, origNm, false)
}
...
// clause condition expression. If recordReads is set, the resolved vertices
// are also recorded in origNm.reads.
func (t *trimmerV3) linkResolversOrig(c adt.Conjunct, addInverse bool, origNm *nodeMeta, recordReads bool) {
	t.resolveElemAll(c, func(resolver adt.Resolver, resolvedTo *adt.Vertex) {
		if recordReads && !slices.Contains(origNm.reads, resolvedTo) {
			origNm.reads = append(origNm.reads, resolvedTo)
		}
...
func (t *trimmerV3) linkStructComprehension(childEnv *adt.Environment, compr *adt.Comprehension, ci adt.CloseInfo) {
	...
	for _, clause := range compr.Clauses {
		...
		t.linkResolversOrig(condConj, false, comprNm, comprNm != nil)
	}
}
...
// subsumeProfile tests whether a conjunct is as specific as the
// vertex it appears in, applying defaults on both sides; see the
// package overview.
var subsumeProfile = subsume.Profile{
	Defaults:    true,
	LeftDefault: true,
}
...
func (t *trimmerV3) equallySpecific(v *adt.Vertex, cs ...adt.Conjunct) bool {
	...
	if v.IsClosedList() && !conjVertex.IsClosedList() {
		// Subsumption with defaults ignores closedness, but removing
		// the conjuncts that close a list would reopen it. Structs do
		// not need this: their closedness comes from references to
		// definitions, which linkResolvers keeps alive.
		t.logf(" does not close list")
		t.dec()
		return false
	}
	err := subsumeProfile.Value(t.ctx, v, conjVertex)
	if err != nil {
...
// solvePending decides, for each vertex with winning conjuncts, which
// conjuncts to keep. This is done only after all vertices have been
// traversed so that all unconditional requirements (patterns,
// disjunctions, no-winner vertices, static dependencies) are known.
//
// If the vertex's conjuncts that are required anyway are jointly as
// specific as the vertex, then all its other conjuncts, including the
// winners, are redundant and nothing more needs to be kept. This
// handles schemas which are split over several declarations, such as
//
//	a: #Schema
//	a: opts: {x: 1, y: 2}
//	#Schema: opts: x: 1
//	#Schema: opts: y: 2
//
// where no single conjunct of #Schema covers a.opts, but the two
// conjuncts together, which must be kept as part of #Schema, do.
//
// Otherwise, a single winner is kept, and several winners are left
// for [trimmerV3.solveUndecideds] to choose from.
func (t *trimmerV3) solvePending() {
	t.inc()
	defer t.dec()

	for _, p := range t.pending {
		if p.winners.hasRequired() {
			// An already required winner is kept regardless, so
			// nothing more needs to be decided for this vertex.
			continue
		}

		var required []adt.Conjunct
		for _, pc := range p.conjuncts {
			// Conjuncts from comprehensions which depend on this
			// vertex cannot keep the field in existence, so they
			// cannot help cover the vertex. See
			// [nodeMeta.comprehensionDependsOn].
			if !pc.winner && pc.nm.isRequired() &&
				!pc.nm.comprehensionDependsOn(p.v) {
				required = append(required, pc.c)
			}
		}
		if len(required) > 0 && t.equallySpecific(p.v, required...) {
			t.logf("required conjuncts cover vertex %p; no winner needed", p.v)
			continue
		}
		...
		if len(p.winners) == 1 {
			...
			p.winners[0].markRequired()
		} else {
			t.logf("%d winners found, none already required", len(p.winners))
			t.undecided = append(t.undecided, p.winners)
		}
	}
}
