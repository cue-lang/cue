...
//
//  1. [Node]: the semantic layer. A Node is a single field (or the
//     package root), presenting the merged view of every declaration
//     of that field in the same scope. Nodes are canonical: the same
//     field is always represented by the same *Node.
//
...
// the post-merged view of the field: every declaration of the same
// field contributes to the same Node. For example, given:
//
//	x: a: 3
//	x: b: 4
//
// there is a single Node for `x`. [Node.Decls] yields one [Decl] per
// declaration of x (i.e. 2 distinct decls in this case), and
// [Node.Fields] yields both `a` and `b`.
//
// Nodes are canonical: within a single [Evaluator] (between calls to
// [Evaluator.Reset]), the same field is always represented by the
...
func (n *Node) Index() (int, bool) {
	rest, found := strings.CutPrefix((*navigable)(n).name, "__")
	if !found {
		return 0, false
	}
...
// nil, false.
//
// A node reached through an import has a path relative to the root
// of its own package: use [Node.Evaluator] to detect that a node
...
// Field returns the node for the named field, or nil if this node
// has no such field. The same containment rules apply as for
...
// Elements yields the nodes for this node's list elements, in index
// order. As with fields, the elements of all of this node's
...
// [Node.Ellipses].
func (n *Node) Elements() iter.Seq[*Node] {
	return func(yield func(*Node) bool) {
		...
		for name, nav := range (*navigable)(n).bindings {
...
// node's declarations, in declaration order. An ellipsis gets its own
// anonymous node, whose single [Decl] has kind [DeclEllipsis]: that
...
// reachable from it by resolution: the nodes whose contents this
// node includes. Resolution edges arise from embedded references and
// reference values:
//
//	x: y            // x resolves to y
//	z: {y, a: 1}    // z resolves to y
//
...
// a.b is in x.b's Expand. Such implied edges mean that expansion can
// discover strictly more than walking the declarations' syntax and
...
// b. The returned set (which always includes the receiver) is
// ordered by the source position of each node's earliest
// declaration.
//
// Because this evaluator is a MAY-analysis (see the package
// documentation), expansion follows all branches of disjunctions,
// and may include nodes that full evaluation would prove
// unreachable.
...
// A NodeSet is a set of [Node]s: deduplicated, in a deterministic
// order. NodeSets arise wherever several nodes can legitimately
// answer a question: most prominently from [Node.Expand], where the
// set as a whole represents the merged view of a node, and from
// [Decl.Resolve], where an expression can resolve to several
// declarations. The methods on NodeSet ask the same question of
// every member and merge the answers.
//
...
// [Decl.Resolve] deliberately returns an unexpanded set. For
// example, given:
//
//	x: {a: 1}
//	y: x
...
// first occurrence of each. Returns nil if the result would be
// empty.
...
// compareNodes provides a deterministic order for expanded sets: by
// the position of each node's earliest-created declaration, then by
// name.
func compareNodes(a, b *Node) int {
	aFilename, aOffset := nodeSortPos(a)
	bFilename, bOffset := nodeSortPos(b)
	return cmp.Or(
		strings.Compare(aFilename, bFilename),
		cmp.Compare(aOffset, bOffset),
		strings.Compare((*navigable)(a).name, (*navigable)(b).name),
	)
}

func nodeSortPos(n *Node) (string, int) {
	for _, fr := range (*navigable)(n).frames {
		if fr.start.IsValid() {
			pos := fr.start.Position()
			return pos.Filename, pos.Offset
		}
	}
	return "", -1
}
...
// nodes for one name, in the order of the members that declare it.
// The NodeSet ns is not implicitly expanded, nor are the yielded
// NodeSets.
//
...
// [Node.Decls]), in member order. The NodeSet ns is not implicitly
// expanded.
...
const (
	...
	//	x: {a: 1} & {b: 2}
	//
	// Each operand is a DeclConjunct contributing to x.
	DeclConjunct
	...
	//	x: {a: 1} | {b: 2}
	//
	// Each operand is a DeclDisjunct contributing to x.
	DeclDisjunct
...
//
// Note that Embedded describes a Decl's own relationship to its
// node, not its position in the wider file: in
//
//	x: 3
//	{x: 4}
//
...
type Decl interface {
	...
	// key, such as the file declarations exposed at the root, or
	...
	// field, the [ast.File] itself for a file-level declaration, or
	// the operand expression for embedding-like Decls. Returns nil for
	// declarations that have no associated value node (e.g. a package
	// clause, whose information is exposed via [Decl.Key] and
	// [Decl.DocComments] only, or a bare ellipsis).
	Value() ast.Node
	...
	// Node returns the node to which this declaration contributes.
	// Note that Decls merged into their node by embedding-like
	// constructs all report the same containing node: in
	//
	//	x: {a: 1} & {b: 2}
	//
	// Node returns x's node for all three of x's Decls. Use
	// [Decl.Kind], [Decl.Value] and [Decl.Fields] to tell such Decls
	// apart.
	Node() *Node

	// Kind reports the syntactic construct that this declaration
	// models. Use [DeclKind.Embedded] to test whether this
	// declaration is merged into its node by an embedding-like
	// construct.
	Kind() DeclKind

	// Fields yields the fields that this declaration alone contributes
	// to its node, in lexical order of name. Whereas [Node.Fields]
	// merges the contributions of every declaration, this is the
	// per-declaration view, which recovers the grouping that merging
	// discards. For example, given:
	//
	//	x: {a: 1} | {b: 2}
	//
	// x's node's Fields are a and b, but the [DeclDisjunct] Decl for
	// {a: 1} yields only a, and the one for {b: 2} yields only b:
	// distinguishing the branches of the disjunction.
	//
	// The same exclusions apply as for [Node.Fields]: fields reached
	// via references, pattern constraints, dynamic fields and list
	// elements are not yielded.
	Fields() iter.Seq2[string, *Node]

	// Ellipses returns the nodes for the ellipses declared directly
	// within this declaration, in declaration order. Whereas
	// [Node.Ellipses] merges the ellipses of every declaration,
	// this is the per-declaration view: in
	//
	//	x: {a: 1, ...} | {b: 2}
	//
	...
	// it refers. The element MUST be one of the [ast.Node] values from
	// this declaration's own syntax, otherwise the result will be
	// nil. Resolve also returns nil for elements that do not resolve
	// to anything: literals, operators, or expressions the evaluator
	// does not track. Only as much of the package as is needed is
	// evaluated.
	//
	// Within a path such as x.y.z, each component resolves
	// separately: given
	//
	//	x: y: z: 3
	//	w: x.y.z
	...
	// expression's constituent parts. The resolvable elements of a
	// path are its leading ident (`x`), each selector ident (or index
	// literal) along it (`y`, `z`), and the whole path expression
	// (`x.y.z`). So here, resolving the whole selector expression, or
	// equivalently its Sel (the ident z within it), yields the node
	// for x.y.z; resolving the ident y within it yields x.y; and
	// resolving the leading ident x yields x. Note that the interior
	// sub-expression x.y (an [ast.SelectorExpr] in its own right,
	// visited by the walk) resolves to nothing.
	//
	...
	//
	// Resolving a field declaration's key yields the node that the
	// field declares - so for a Decl d of kind [DeclField],
	// d.Resolve(d.Key()) returns a set containing d.Node().  Resolving
	// an ident whose binding is an alias, let, or import yields the
	// (anonymous) node of that binding: use [Node.Expand] to see
	// through it. Resolving an [ast.ImportSpec] yields the imported
	// package's [Evaluator.PackageClauses] node.
	//
...
func (fr *frameDecl) Node() *Node {
	return (*Node)(fr.navigable)
}

// Kind implements [Decl].
func (fr *frameDecl) Kind() DeclKind {
	return fr.kind
}

// Fields implements [Decl].
func (fr *frameDecl) Fields() iter.Seq2[string, *Node] {
	return func(yield func(string, *Node) bool) {
		f := (*frame)(fr)
		// Evaluating via the navigable (rather than f.eval directly)
		// preserves the invariants documented on [navigable.eval].
		f.navigable.eval()
		for _, name := range slices.Sorted(maps.Keys(f.bindings)) {
			if strings.HasPrefix(name, "__") {
				continue
			}
			nav := f.navigable.bindings[name]
			if nav == nil {
				continue
			}
			...
			isField := false
			for _, childFr := range f.bindings[name] {
...
// Ellipses implements [Decl].
func (fr *frameDecl) Ellipses() NodeSet {
	f := (*frame)(fr)
	f.navigable.eval()
	var result NodeSet
	for _, nav := range f.ellipses {
		result = append(result, (*Node)(nav))
	}
	return dedupeNodes(result)
}
...
func (fr *frameDecl) Resolve(el ast.Node) NodeSet {
	f := (*frame)(fr)
	fe := f.fileEvaluator
	if el == nil || fe == nil {
		return nil
	...
	// component (including field keys and whole path expressions),
	// so an element found by walking this declaration's syntax
	// matches exactly.
	var navs []*navigable
	for _, leafFr := range leafFrames {
		for _, p := range leafFr.childPaths {
			...
			for i := range comps {
				...
				// The results of resolving a component are stored in
				// the component that follows it. The final component of
				// a multi-component path exists only to hold the
				// results of the whole path, so a match on it (the
				// whole path expression) — or on the single component
				// of a length-1 path — finds its results in place.
				if i+1 < len(comps) {
	...
	result := make(NodeSet, 0, len(navs))
