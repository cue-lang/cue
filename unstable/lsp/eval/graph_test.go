...
// check, which expresses the case's expectations using the [graphTest]
// helpers.
...
// field navigates from the package root through the given dotted
// path of field names, asserting that every step exists.
...
// soleDecl asserts that n has exactly one decl of the given kind,
// and returns it.
...
// checkSetFields asserts the names yielded by ns.Fields().
func (gt *graphTest) checkSetFields(ns eval.NodeSet, want ...string) {
	gt.Helper()
	names := []string{}
...
// checkDeclKinds asserts the kind of each of n's decls, in order.
func (gt *graphTest) checkDeclKinds(n *eval.Node, want ...eval.DeclKind) {
	...
	qt.Assert(gt, qt.DeepEquals(kinds, notNil(want)))
}
...
// checkNodes asserts that got holds exactly the given nodes, in
...
`,
			check: func(t *graphTest) {
				...
				expanded := x.Expand()
				t.checkSetFields(expanded, "a", "b", "c", "e")
...
`,
			check: func(t *graphTest) {
				...
				t.checkSetFields(emb.Expand(), "e", "f")
			},
		},
...
`,
			check: func(t *graphTest) {
				...
				t.checkSetFields(xb.Expand(), "c", "d")
			},
		},
...
`,
			check: func(t *graphTest) {
				...
				// In a longer path, the resolvable elements are the
				// leading ident, each selector ident, and the whole
				// expression. The interior sub-expression o.p is not a
				// tracked element and resolves to nothing; the ident p
				// within it is how the o.p prefix resolves.
				...
				qt.Assert(t, qt.IsNil(dpDecl.Resolve(inner)))
...
`,
			check: func(t *graphTest) {
				...
				l := t.field("l")
				t.checkFields(l)
				elements := slices.Collect(l.Elements())
...
`,
			check: func(t *graphTest) {
				...
				// of the element node.
				mElements := slices.Collect(t.field("m").Elements())
...
`,
			check: func(t *graphTest) {
				...
				qt.Assert(t, qt.HasLen(slices.Collect(m.Elements()), 0))
