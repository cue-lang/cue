...
`,
			check: func(t *graphTest) {
				checkConstraint := func(n *eval.Node, want token.Token, wantOK bool) {
					t.Helper()
					got, ok := n.Constraint()
				...
				checkConstraint(t.field("k"), token.ILLEGAL, true)
				checkConstraint(t.field("opt"), token.OPTION, true)
				checkConstraint(t.field("req"), token.NOT, true)
				checkConstraint(t.field("blend"), token.NOT, true)
				checkConstraint(t.field("mix"), token.ILLEGAL, true)

				// Non-field decls take no part: ctrap's conjunction
				// operands (whose Decl.Constraint is token.ILLEGAL) do
				// not make it regular.
				checkConstraint(t.field("ctrap"), token.OPTION, true)

				// Nodes without field declarations have no constraint.
				pats := t.root().Patterns()
				qt.Assert(t, qt.HasLen(pats, 1))
				checkConstraint(pats[0], token.ILLEGAL, false)
				...
				checkConstraint(t.field("y"), token.OPTION, true)
			},
		},
...
`,
			check: func(t *graphTest) {
				...
				element := slices.Collect(t.field("l").Elements())[0]
...
func TestUnifyConstraints(t *testing.T) {
	...
	optional, required, regular := token.OPTION, token.NOT, token.ILLEGAL

	want := map[[2]token.Token]token.Token{
	...
	constraints := []token.Token{optional, required, regular}
	for _, a := range constraints {
		for _, b := range constraints {
			got := eval.UnifyConstraints(a, b)
			qt.Assert(t, qt.Equals(got, want[[2]token.Token{a, b}]),
